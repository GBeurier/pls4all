...
Implement a stateful X-based outlier filter (`c4a_filter_x_outlier_*`)
...
enum lands in `c4a.h §18` at the integration commit):
...
- `cpp/src/c_api/c_api_filter_x_outlier.cpp` — 5 extern "C" entry
  ...
  prototypes until c4a.h §18 lands at the integration commit.

### Tests (1 file)

- `cpp/tests/test_filter_x_outlier.cpp` — 12 tests (6 smoke + 6 parity).
  Lives in its own executable
  `chemometrics4all_tests_filter_x_outlier` so this worktree leaves
  `cpp/tests/main.cpp` and the expected-symbols files frozen.

### Parity (1 file)

- `parity/python_generator/src/c4a_parity_filters_ref/x_outlier.py` —
...
- `cpp/src/CMakeLists.txt` — append the new engine + vendored sources
  to `chemometrics4all_core` and the C ABI wrapper to
  `chemometrics4all_c{,_static}`.
...
- `cpp/include/chemometrics4all/c4a.h` (integration adds §18 enum).
- `cpp/include/chemometrics4all/c4a_version.h` (ABI still 1.7.0).
...
c4a_status_t c4a_filter_x_outlier_fit(c4a_filter_x_outlier_handle_t* h,
                                       c4a_matrix_view_t X);
...
- **Bit-exact sklearn `IsolationForest`**. sklearn uses NumPy
  RandomState; c4a uses PCG64. Tree structures differ. Counts agree
  within tolerance.
...
- 5 new ABI symbols exported via `c4a_*` version script.
...
```bash
cd /home/delete/nirs4all/chemometrics4all/.claude/worktrees/agent-a402c58f3cc0f19b4
cmake --preset dev-debug
cmake --build --preset dev-debug

build/dev-debug/cpp/tests/chemometrics4all_tests                       # 171 / 171
build/dev-debug/cpp/tests/chemometrics4all_tests_filter_x_outlier      # 12 / 12

ctest --test-dir build/dev-debug                                       # 2 / 2 pass

nm -D --defined-only build/dev-debug/cpp/src/libc4a.so.1.7.0 \
    | awk '$2=="T" {print $3}' | grep c4a_filter_x_outlier_ | sort
# c4a_filter_x_outlier_apply
# c4a_filter_x_outlier_create
# c4a_filter_x_outlier_destroy
# c4a_filter_x_outlier_fit
# c4a_filter_x_outlier_is_fitted
```
