...
#include "fixtures/cars_selection_fixtures.hpp"
#include "harness.hpp"
...
void check_indices(int& failures,
                   ...
                   const ::pls4all::test::fixtures::CarsSelectionIndexRef& expected) {
    if (actual.size() != expected.size) {
        ++failures;
        std::fprintf(stderr,
                     "  FAIL %s size: actual %lu expected %lu\n",
...
void check_fixture(int& failures,
                   const ::pls4all::test::fixtures::CarsSelectionFixture& fixture) {
    ...
    ::pls4all::core::CarsSelectionResult result;
    CHECK_EQ(::pls4all::core::select_by_cars(ctx,
                                             cfg,
                                             X,
                                             Y,
                                             plan,
...
TEST(cars_selection_phase5f, generated_fixture_matches_python_reference) {
    for (const auto& fixture : ::pls4all::test::fixtures::kCarsSelectionFixtures) {
        check_fixture(failures, fixture);
    }
}

TEST(cars_selection_phase5f, rejects_invalid_cars_requests) {
    const auto& fixture = ::pls4all::test::fixtures::kCarsSelectionFixtures[0];
    ::pls4all::core::Context ctx;
    ::pls4all::core::Config cfg;
    cfg.n_components = fixture.n_components;
