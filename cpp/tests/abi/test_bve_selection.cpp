...
void check_indices(int& failures,
                   ...
                   const ::n4m::test::fixtures::BveSelectionIndexRef& expected) {
    if (actual.size() != expected.size) {
        ++failures;
        std::fprintf(stderr,
                     "  FAIL %s size: actual %lu expected %lu\n",
...
void check_fixture(int& failures,
                   const ::n4m::test::fixtures::BveSelectionFixture& fixture) {
    ...
    ::n4m::core::BveSelectionResult result;
    CHECK_EQ(::n4m::core::select_by_bve(ctx,
                                            cfg,
                                            X,
                                            Y,
                                            plan,
...
TEST(bve_selection_phase5k, generated_fixture_matches_python_reference) {
    for (const auto& fixture : ::n4m::test::fixtures::kBveSelectionFixtures) {
        check_fixture(failures, fixture);
    }
}

TEST(bve_selection_phase5k, rejects_invalid_bve_requests) {
    const auto& fixture = ::n4m::test::fixtures::kBveSelectionFixtures[0];
    ::n4m::core::Context ctx;
    ::n4m::core::Config cfg;
    cfg.n_components = fixture.n_components;
