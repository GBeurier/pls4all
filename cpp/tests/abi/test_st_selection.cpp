...
void check_indices(int& failures,
                   ...
                   const ::n4m::test::fixtures::StSelectionIndexRef& expected) {
    if (actual.size() != expected.size) {
        ++failures;
        std::fprintf(stderr,
                     "  FAIL %s size: actual %lu expected %lu\n",
...
std::vector<double> thresholds(const ::n4m::test::fixtures::MatrixRef& ref) {
    return std::vector<double>(ref.values, ref.values + ref.size);
}

void check_fixture(int& failures,
                   const ::n4m::test::fixtures::StSelectionFixture& fixture) {
    ...
    ::n4m::core::StSelectionResult result;
    CHECK_EQ(::n4m::core::select_by_st(ctx,
                                           cfg,
                                           X,
                                           Y,
                                           plan,
...
TEST(st_selection_phase5u, generated_fixture_matches_python_reference) {
    for (const auto& fixture : ::n4m::test::fixtures::kStSelectionFixtures) {
        check_fixture(failures, fixture);
    }
}

TEST(st_selection_phase5u, rejects_invalid_st_requests) {
    const auto& fixture = ::n4m::test::fixtures::kStSelectionFixtures[0];
    ::n4m::core::Context ctx;
    ::n4m::core::Config cfg;
    cfg.n_components = fixture.n_components;
