...
#include "fixtures/st_selection_fixtures.hpp"
#include "harness.hpp"
...
void check_indices(int& failures,
                   ...
                   const ::pls4all::test::fixtures::StSelectionIndexRef& expected) {
    if (actual.size() != expected.size) {
        ++failures;
        std::fprintf(stderr,
                     "  FAIL %s size: actual %lu expected %lu\n",
...
std::vector<double> thresholds(const ::pls4all::test::fixtures::MatrixRef& ref) {
    return std::vector<double>(ref.values, ref.values + ref.size);
}

void check_fixture(int& failures,
                   const ::pls4all::test::fixtures::StSelectionFixture& fixture) {
    ...
    ::pls4all::core::StSelectionResult result;
    CHECK_EQ(::pls4all::core::select_by_st(ctx,
                                           cfg,
                                           X,
                                           Y,
                                           plan,
    ...
    CHECK_EQ(result.n_thresholds, fixture.n_thresholds);
    CHECK_EQ(result.min_selected, fixture.min_selected);
    CHECK_EQ(result.best_threshold_index, fixture.best_threshold_index);
    check_close_scalar(failures, "best_threshold", result.best_threshold, fixture.best_threshold);
    ...
    check_close_values(failures, "thresholds", result.thresholds, fixture.thresholds);
    check_close_values(failures, "rmse_by_threshold", result.rmse_by_threshold, fixture.rmse_by_threshold);
    check_indices(failures, "selected_counts", result.selected_counts, fixture.selected_counts);
    check_indices(failures, "selected_masks", result.selected_masks, fixture.selected_masks);
...
TEST(st_selection_phase5u, generated_fixture_matches_python_reference) {
    for (const auto& fixture : ::pls4all::test::fixtures::kStSelectionFixtures) {
        check_fixture(failures, fixture);
    }
}

TEST(st_selection_phase5u, rejects_invalid_st_requests) {
    const auto& fixture = ::pls4all::test::fixtures::kStSelectionFixtures[0];
    ::pls4all::core::Context ctx;
    ::pls4all::core::Config cfg;
    cfg.n_components = fixture.n_components;
    ...
    std::vector<double> bad_thresholds = thresholds(fixture.thresholds);
    bad_thresholds[0] = 1.5;
    CHECK_EQ(::pls4all::core::select_by_st(ctx,
                                           cfg,
                                           X,
                                           Y,
