...
c4a_status_t require_f64_rowmajor(const c4a_matrix_view_t& v,
...
c4a_status_t require_f64_rowmajor_mut(c4a_matrix_view_t& v,
...
c4a_status_t require_same_shape(const MatrixIn& a, const MatrixIn& b) noexcept {
    return (a.rows == b.rows && a.cols == b.cols) ? C4A_OK : C4A_ERR_SHAPE_MISMATCH;
}

c4a_status_t require_out_shape(const MatrixIn& x, const MatrixOut& out) noexcept {
    return (x.rows == out.rows && x.cols == out.cols) ? C4A_OK : C4A_ERR_SHAPE_MISMATCH;
}
    ...
    for (std::int64_t out_col = 0; out_col < y.cols; ++out_col) {
        ...
        if (!solve_linear(xtx, xty, params, sol)) {
            ...
            if (!solve_linear(regularized, xty, params, sol)) {
                throw C4A_ERR_NUMERICAL_FAILURE;
            }
        }
        for (std::int64_t a = 0; a < params; ++a) {
...
c4a_status_t ds_fit(DSState& s, const c4a_matrix_view_t& source_v,
                    const c4a_matrix_view_t& target_v) {
    MatrixIn source, target;
    c4a_status_t st = require_f64_rowmajor(source_v, source);
    if (st != C4A_OK) return st;
    st = require_f64_rowmajor(target_v, target);
    if (st != C4A_OK) return st;
    st = require_same_shape(source, target);
    if (st != C4A_OK) return st;
    if (source.rows < 1 || source.cols < 1) return C4A_ERR_INVALID_ARGUMENT;
...
c4a_status_t ds_transform(const DSState& s, const c4a_matrix_view_t& x_v,
                          c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    MatrixOut out;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
...
c4a_status_t pds_fit(PDSState& s, const c4a_matrix_view_t& source_v,
                     const c4a_matrix_view_t& target_v) {
    MatrixIn source, target;
    c4a_status_t st = require_f64_rowmajor(source_v, source);
    if (st != C4A_OK) return st;
    ...
    st = require_same_shape(source, target);
    if (st != C4A_OK) return st;
    if (source.rows < 1 || source.cols < 1 || s.window < 1) return C4A_ERR_INVALID_ARGUMENT;
    s.features = source.cols;
    s.windows.clear();
    s.coefs.clear();
...
c4a_status_t pds_transform(const PDSState& s, const c4a_matrix_view_t& x_v,
                           c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    MatrixOut out;
...
c4a_status_t saps_fit(SAPSState& s, const c4a_matrix_view_t& source_v,
                      const c4a_matrix_view_t& target_v) {
    MatrixIn source, target;
    c4a_status_t st = require_f64_rowmajor(source_v, source);
    if (st != C4A_OK) return st;
...
c4a_status_t saps_transform(const SAPSState& s, const c4a_matrix_view_t& x_v,
                            c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    MatrixOut out;
...
c4a_status_t slope_fit(SlopeBiasState& s, const double* source,
                       const double* target, std::int64_t n) {
    if (source == nullptr || target == nullptr) return C4A_ERR_NULL_POINTER;
    if (n < 1) return C4A_ERR_INVALID_ARGUMENT;
...
c4a_status_t normalize_weights(std::vector<double>& w) {
    double total = 0.0;
    for (double value : w) total += value;
    if (!(total > 0.0)) return C4A_ERR_INVALID_ARGUMENT;
    for (double& value : w) value /= total;
    return C4A_OK;
}

c4a_status_t weighted_snv_fit(VectorWeightsState& s, const c4a_matrix_view_t& x_v) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
    if (x.cols < 1) return C4A_ERR_INVALID_ARGUMENT;
    if (s.has_initial) {
        if (static_cast<std::int64_t>(s.initial.size()) != x.cols) return C4A_ERR_SHAPE_MISMATCH;
        s.weights = s.initial;
    } else {
...
c4a_status_t vsn_fit(VectorWeightsState& s, const c4a_matrix_view_t& x_v) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
...
c4a_status_t weighted_snv_transform(const VectorWeightsState& s,
                                    const c4a_matrix_view_t& x_v,
                                    c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    MatrixOut out;
    ...
    if (st != C4A_OK) return st;
    if (static_cast<std::int64_t>(s.weights.size()) != x.cols) return C4A_ERR_SHAPE_MISMATCH;
    for (std::int64_t i = 0; i < x.rows; ++i) {
        double mean = 0.0;
...
c4a_status_t piecewise_snv_fit(PiecewiseSNVState& s, const c4a_matrix_view_t& x_v) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
    s.features = x.cols;
...
c4a_status_t piecewise_snv_transform(const PiecewiseSNVState& s,
                                     const c4a_matrix_view_t& x_v,
                                     c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    ...
    if (st != C4A_OK) return st;
    st = require_out_shape(x, out);
    if (st != C4A_OK || x.cols != s.features) return C4A_ERR_SHAPE_MISMATCH;
...
c4a_status_t msc_fit(MSCState& s, const c4a_matrix_view_t& x_v) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
    s.features = x.cols;
...
c4a_status_t msc_transform(const MSCState& s, const c4a_matrix_view_t& x_v,
                           c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    MatrixOut out;
...
c4a_status_t align_fit(AlignState& s, const c4a_matrix_view_t& x_v) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
    s.features = x.cols;
...
c4a_status_t align_transform(const AlignState& s, const c4a_matrix_view_t& x_v,
                             c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
    MatrixOut out;
...
c4a_status_t variance_fit(SelectorState& s, const c4a_matrix_view_t& x_v,
                          const double* y, std::int64_t y_len) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
    if (s.correlation && (y == nullptr || y_len != x.rows)) return C4A_ERR_SHAPE_MISMATCH;
...
c4a_status_t selector_transform(const SelectorState& s,
                                const c4a_matrix_view_t& x_v,
                                c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
...
c4a_status_t interval_fit(IntervalState& s, const c4a_matrix_view_t& x_v) {
    MatrixIn x;
    c4a_status_t st = require_f64_rowmajor(x_v, x);
    if (st != C4A_OK) return st;
    s.features = x.cols;
...
c4a_status_t interval_transform(const IntervalState& s,
                                const c4a_matrix_view_t& x_v,
                                c4a_matrix_view_t& out_v) {
    if (!s.fitted) return C4A_ERR_NOT_FITTED;
    MatrixIn x;
...
c4a_status_t make_handle(Handle** out, Args&&... args) noexcept {
...
c4a_status_t call_status(Fn&& fn) noexcept {
    try {
        return fn();
    } catch (c4a_status_t s) {
        return s;
    } catch (const std::bad_alloc&) {
        return C4A_ERR_OUT_OF_MEMORY;
    } catch (...) {
...
struct c4a_pp_direct_standardization_handle_t { DSState s; };
struct c4a_pp_robust_direct_standardization_handle_t {
    DSState s;
    double trim_quantile;
    std::int32_t max_iter;
};
struct c4a_pp_piecewise_direct_standardization_handle_t { PDSState s; };
struct c4a_pp_saps_handle_t { SAPSState s; };
struct c4a_pp_slope_bias_handle_t { SlopeBiasState s; };
struct c4a_pp_local_centering_handle_t {
    bool fitted = false;
    std::int64_t features = 0;
    std::vector<double> delta;
};
struct c4a_pp_weighted_snv_handle_t { VectorWeightsState s; };
struct c4a_pp_vsn_handle_t { VectorWeightsState s; };
struct c4a_pp_piecewise_snv_handle_t { PiecewiseSNVState s; };
struct c4a_pp_piecewise_msc_handle_t { MSCState s; };
struct c4a_pp_localized_msc_handle_t { MSCState s; };
struct c4a_pp_xcorr_align_handle_t { AlignState s; };
struct c4a_pp_icoshift_align_handle_t { AlignState s; };
struct c4a_pp_dtw_align_handle_t { AlignState s; };
struct c4a_pp_cow_align_handle_t { AlignState s; };
struct c4a_filter_variance_handle_t { SelectorState s; };
struct c4a_filter_correlation_handle_t { SelectorState s; };
struct c4a_interval_generator_handle_t { IntervalState s; };

extern "C" C4A_API c4a_status_t c4a_pp_direct_standardization_create(
    c4a_pp_direct_standardization_handle_t** out, int fit_intercept, double ridge) {
    c4a_pp_direct_standardization_handle_t init;
    init.s.fit_intercept = fit_intercept != 0;
    init.s.ridge = ridge;
    return make_handle(out, init);
}

extern "C" C4A_API void c4a_pp_direct_standardization_destroy(
    c4a_pp_direct_standardization_handle_t* h) { delete h; }

extern "C" C4A_API c4a_status_t c4a_pp_direct_standardization_fit(
    c4a_pp_direct_standardization_handle_t* h, c4a_matrix_view_t source,
    c4a_matrix_view_t target) {
    return call_status([&] { return h ? ds_fit(h->s, source, target) : C4A_ERR_NULL_POINTER; });
}

extern "C" C4A_API c4a_status_t c4a_pp_direct_standardization_transform(
    const c4a_pp_direct_standardization_handle_t* h, c4a_matrix_view_t x,
    c4a_matrix_view_t out) {
    return call_status([&] { return h ? ds_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}

extern "C" C4A_API c4a_status_t c4a_pp_direct_standardization_is_fitted(
    const c4a_pp_direct_standardization_handle_t* h, int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
}

extern "C" C4A_API c4a_status_t c4a_pp_robust_direct_standardization_create(
    c4a_pp_robust_direct_standardization_handle_t** out, int fit_intercept,
    double ridge, double trim_quantile, int32_t max_iter) {
    c4a_pp_robust_direct_standardization_handle_t init;
    init.s.fit_intercept = fit_intercept != 0;
    init.s.ridge = ridge;
...
extern "C" C4A_API void c4a_pp_robust_direct_standardization_destroy(
    c4a_pp_robust_direct_standardization_handle_t* h) { delete h; }

extern "C" C4A_API c4a_status_t c4a_pp_robust_direct_standardization_fit(
    c4a_pp_robust_direct_standardization_handle_t* h, c4a_matrix_view_t source_v,
    c4a_matrix_view_t target_v) {
    return call_status([&] {
        if (!h) return C4A_ERR_NULL_POINTER;
        MatrixIn source, target;
        c4a_status_t st = require_f64_rowmajor(source_v, source);
        if (st != C4A_OK) return st;
        st = require_f64_rowmajor(target_v, target);
        ...
        for (std::int32_t iter = 0; iter < std::max<std::int32_t>(1, h->max_iter); ++iter) {
            ...
            c4a_matrix_view_t out_v{mapped.data(), source.rows, source.cols, source.cols, 1, C4A_DTYPE_F64, 0};
...
extern "C" C4A_API c4a_status_t c4a_pp_robust_direct_standardization_transform(
    const c4a_pp_robust_direct_standardization_handle_t* h, c4a_matrix_view_t x,
    c4a_matrix_view_t out) {
    return call_status([&] { return h ? ds_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}

extern "C" C4A_API c4a_status_t c4a_pp_robust_direct_standardization_is_fitted(
    const c4a_pp_robust_direct_standardization_handle_t* h, int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
}

extern "C" C4A_API c4a_status_t c4a_pp_piecewise_direct_standardization_create(
    c4a_pp_piecewise_direct_standardization_handle_t** out, int32_t window_size,
    int fit_intercept, double ridge) {
    c4a_pp_piecewise_direct_standardization_handle_t init;
...
extern "C" C4A_API void c4a_pp_piecewise_direct_standardization_destroy(
    c4a_pp_piecewise_direct_standardization_handle_t* h) { delete h; }

extern "C" C4A_API c4a_status_t c4a_pp_piecewise_direct_standardization_fit(
    c4a_pp_piecewise_direct_standardization_handle_t* h, c4a_matrix_view_t source,
    c4a_matrix_view_t target) {
    return call_status([&] { return h ? pds_fit(h->s, source, target) : C4A_ERR_NULL_POINTER; });
}

extern "C" C4A_API c4a_status_t c4a_pp_piecewise_direct_standardization_transform(
    const c4a_pp_piecewise_direct_standardization_handle_t* h, c4a_matrix_view_t x,
    c4a_matrix_view_t out) {
    return call_status([&] { return h ? pds_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}

extern "C" C4A_API c4a_status_t c4a_pp_piecewise_direct_standardization_is_fitted(
    const c4a_pp_piecewise_direct_standardization_handle_t* h, int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
}

extern "C" C4A_API c4a_status_t c4a_pp_saps_create(
    c4a_pp_saps_handle_t** out, int32_t n_components, double score_weight,
    int fit_intercept, double ridge) {
    c4a_pp_saps_handle_t init;
...
extern "C" C4A_API void c4a_pp_saps_destroy(c4a_pp_saps_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_pp_saps_fit(c4a_pp_saps_handle_t* h,
    c4a_matrix_view_t source, c4a_matrix_view_t target) {
    return call_status([&] { return h ? saps_fit(h->s, source, target) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_saps_transform(const c4a_pp_saps_handle_t* h,
    c4a_matrix_view_t x, c4a_matrix_view_t out) {
    return call_status([&] { return h ? saps_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_saps_is_fitted(const c4a_pp_saps_handle_t* h,
    int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
...
extern "C" C4A_API c4a_status_t c4a_pp_slope_bias_create(c4a_pp_slope_bias_handle_t** out) {
    return make_handle(out, c4a_pp_slope_bias_handle_t{});
}
extern "C" C4A_API void c4a_pp_slope_bias_destroy(c4a_pp_slope_bias_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_pp_slope_bias_fit(c4a_pp_slope_bias_handle_t* h,
    const double* source, const double* target, int64_t n) {
    return call_status([&] { return h ? slope_fit(h->s, source, target, n) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_slope_bias_transform(const c4a_pp_slope_bias_handle_t* h,
    const double* y, int64_t n, double* out) {
    if (!h || !y || !out) return C4A_ERR_NULL_POINTER;
    if (!h->s.fitted) return C4A_ERR_NOT_FITTED;
    for (int64_t i = 0; i < n; ++i) out[i] = h->s.slope * y[i] + h->s.bias;
    return C4A_OK;
}
extern "C" C4A_API c4a_status_t c4a_pp_slope_bias_is_fitted(const c4a_pp_slope_bias_handle_t* h,
    int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
...
extern "C" C4A_API c4a_status_t c4a_pp_local_centering_create(c4a_pp_local_centering_handle_t** out) {
    return make_handle(out, c4a_pp_local_centering_handle_t{});
}
extern "C" C4A_API void c4a_pp_local_centering_destroy(c4a_pp_local_centering_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_pp_local_centering_fit(c4a_pp_local_centering_handle_t* h,
    c4a_matrix_view_t source_v, c4a_matrix_view_t target_v) {
    return call_status([&] {
        if (!h) return C4A_ERR_NULL_POINTER;
        MatrixIn source, target;
        c4a_status_t st = require_f64_rowmajor(source_v, source);
...
extern "C" C4A_API c4a_status_t c4a_pp_local_centering_transform(
    const c4a_pp_local_centering_handle_t* h, c4a_matrix_view_t x_v, c4a_matrix_view_t out_v) {
    return call_status([&] {
        if (!h) return C4A_ERR_NULL_POINTER;
        if (!h->fitted) return C4A_ERR_NOT_FITTED;
        MatrixIn x;
        MatrixOut out;
        c4a_status_t st = require_f64_rowmajor(x_v, x);
        if (st != C4A_OK) return st;
        ...
        if (st != C4A_OK) return st;
        st = require_out_shape(x, out);
        if (st != C4A_OK || x.cols != h->features) return C4A_ERR_SHAPE_MISMATCH;
...
extern "C" C4A_API c4a_status_t c4a_pp_local_centering_is_fitted(
    const c4a_pp_local_centering_handle_t* h, int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->fitted) ? 1 : 0;
    return C4A_OK;
}

extern "C" C4A_API c4a_status_t c4a_pp_weighted_snv_create(
    c4a_pp_weighted_snv_handle_t** out, const double* weights, int64_t n_weights,
    int32_t ddof, double eps) {
    c4a_pp_weighted_snv_handle_t init;
    init.s.ddof = ddof;
    init.s.eps = eps;
...
extern "C" C4A_API void c4a_pp_weighted_snv_destroy(c4a_pp_weighted_snv_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_pp_weighted_snv_fit(c4a_pp_weighted_snv_handle_t* h,
    c4a_matrix_view_t x) {
    return call_status([&] { return h ? weighted_snv_fit(h->s, x) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_weighted_snv_transform(const c4a_pp_weighted_snv_handle_t* h,
    c4a_matrix_view_t x, c4a_matrix_view_t out) {
    return call_status([&] { return h ? weighted_snv_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_weighted_snv_is_fitted(const c4a_pp_weighted_snv_handle_t* h,
    int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
...
extern "C" C4A_API c4a_status_t c4a_pp_vsn_create(c4a_pp_vsn_handle_t** out, double eps) {
    c4a_pp_vsn_handle_t init;
    init.s.eps = eps;
    return make_handle(out, init);
}
extern "C" C4A_API void c4a_pp_vsn_destroy(c4a_pp_vsn_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_pp_vsn_fit(c4a_pp_vsn_handle_t* h, c4a_matrix_view_t x) {
    return call_status([&] { return h ? vsn_fit(h->s, x) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_vsn_transform(const c4a_pp_vsn_handle_t* h,
    c4a_matrix_view_t x, c4a_matrix_view_t out) {
    return call_status([&] { return h ? weighted_snv_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_vsn_is_fitted(const c4a_pp_vsn_handle_t* h, int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
}

extern "C" C4A_API c4a_status_t c4a_pp_piecewise_snv_create(
    c4a_pp_piecewise_snv_handle_t** out, int32_t window, int32_t ddof, double eps) {
    c4a_pp_piecewise_snv_handle_t init;
    init.s.window = window;
    init.s.ddof = ddof;
    init.s.eps = eps;
    return make_handle(out, init);
}
extern "C" C4A_API void c4a_pp_piecewise_snv_destroy(c4a_pp_piecewise_snv_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_pp_piecewise_snv_fit(c4a_pp_piecewise_snv_handle_t* h,
    c4a_matrix_view_t x) {
    return call_status([&] { return h ? piecewise_snv_fit(h->s, x) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_piecewise_snv_transform(const c4a_pp_piecewise_snv_handle_t* h,
    c4a_matrix_view_t x, c4a_matrix_view_t out) {
    return call_status([&] { return h ? piecewise_snv_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_pp_piecewise_snv_is_fitted(const c4a_pp_piecewise_snv_handle_t* h,
    int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
...
extern "C" C4A_API c4a_status_t prefix##_create(HandleType** out, const double* reference, int64_t n_reference, int32_t window, double eps) { \
...
extern "C" C4A_API void prefix##_destroy(HandleType* h) { delete h; } \
extern "C" C4A_API c4a_status_t prefix##_fit(HandleType* h, c4a_matrix_view_t x) { \
    return call_status([&] { return h ? msc_fit(h->s, x) : C4A_ERR_NULL_POINTER; }); \
} \
extern "C" C4A_API c4a_status_t prefix##_transform(const HandleType* h, c4a_matrix_view_t x, c4a_matrix_view_t out) { \
...
DEFINE_MSC_API(c4a_pp_localized_msc, c4a_pp_localized_msc_handle_t, true)

#define DEFINE_ALIGN_API(prefix, HandleType, interval_flag, dtw_flag, cow_flag) \
extern "C" C4A_API c4a_status_t prefix##_create(HandleType** out, const double* reference, int64_t n_reference, int32_t interval_size, int32_t max_shift) { \
...
extern "C" C4A_API void prefix##_destroy(HandleType* h) { delete h; } \
extern "C" C4A_API c4a_status_t prefix##_fit(HandleType* h, c4a_matrix_view_t x) { \
    return call_status([&] { return h ? align_fit(h->s, x) : C4A_ERR_NULL_POINTER; }); \
} \
extern "C" C4A_API c4a_status_t prefix##_transform(const HandleType* h, c4a_matrix_view_t x, c4a_matrix_view_t out) { \
...
DEFINE_ALIGN_API(c4a_pp_icoshift_align, c4a_pp_icoshift_align_handle_t, true, false, false)
DEFINE_ALIGN_API(c4a_pp_cow_align, c4a_pp_cow_align_handle_t, false, false, true)
DEFINE_ALIGN_API(c4a_pp_dtw_align, c4a_pp_dtw_align_handle_t, false, true, false)

#define DEFINE_SELECTOR_CREATE(prefix, HandleType, corr_flag) \
extern "C" C4A_API c4a_status_t prefix##_create(HandleType** out, double threshold, int32_t top_k) { \
...
extern "C" C4A_API c4a_status_t c4a_filter_variance_fit(c4a_filter_variance_handle_t* h,
    c4a_matrix_view_t x) {
    return call_status([&] { return h ? variance_fit(h->s, x, nullptr, 0) : C4A_ERR_NULL_POINTER; });
}

DEFINE_SELECTOR_CREATE(c4a_filter_correlation, c4a_filter_correlation_handle_t, true)
extern "C" C4A_API c4a_status_t c4a_filter_correlation_fit(c4a_filter_correlation_handle_t* h,
    c4a_matrix_view_t x, const double* y, int64_t n_y) {
    return call_status([&] { return h ? variance_fit(h->s, x, y, n_y) : C4A_ERR_NULL_POINTER; });
}

extern "C" C4A_API c4a_status_t c4a_interval_generator_create(
    c4a_interval_generator_handle_t** out, int32_t interval_size, int32_t step) {
    c4a_interval_generator_handle_t init;
    init.s.width = interval_size;
    init.s.step = step;
    return make_handle(out, init);
}
extern "C" C4A_API void c4a_interval_generator_destroy(c4a_interval_generator_handle_t* h) { delete h; }
extern "C" C4A_API c4a_status_t c4a_interval_generator_fit(c4a_interval_generator_handle_t* h,
    c4a_matrix_view_t x) {
    return call_status([&] { return h ? interval_fit(h->s, x) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_interval_generator_transform(
    const c4a_interval_generator_handle_t* h, c4a_matrix_view_t x, c4a_matrix_view_t out) {
    return call_status([&] { return h ? interval_transform(h->s, x, out) : C4A_ERR_NULL_POINTER; });
}
extern "C" C4A_API c4a_status_t c4a_interval_generator_output_cols(
    const c4a_interval_generator_handle_t* h, int64_t* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    if (!h || !h->s.fitted) return C4A_ERR_NOT_FITTED;
    *out = interval_output_cols(h->s);
    return C4A_OK;
}
extern "C" C4A_API c4a_status_t c4a_interval_generator_is_fitted(
    const c4a_interval_generator_handle_t* h, int* out) {
    if (!out) return C4A_ERR_NULL_POINTER;
    *out = (h && h->s.fitted) ? 1 : 0;
    return C4A_OK;
}
