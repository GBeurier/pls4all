...
//
// When C4A_USE_BLAS is not defined the implementations are the same
...
//
// When C4A_USE_BLAS is defined we dispatch to the system CBLAS
...
#ifndef CHEMOMETRICS4ALL_CORE_LINALG_HPP
#define CHEMOMETRICS4ALL_CORE_LINALG_HPP
...
// ---------------------------------------------------------------------------
// When C4A_USE_CUDA is defined the dispatch goes to cuda_dispatch which
...
#if defined(C4A_USE_BLAS) && !defined(C4A_USE_CUDA)
...
#if defined(C4A_USE_CUDA)
    chemometrics4all::cuda_dispatch::gemv(
        (trans == Trans_No) ? 0 : 1,
        rows, cols, alpha, A, x, beta, y);
#elif defined(C4A_USE_BLAS)
...
#if defined(C4A_USE_CUDA)
    chemometrics4all::cuda_dispatch::gemm(
...
#if defined(C4A_USE_CUDA)
    chemometrics4all::cuda_dispatch::ger(M, N, alpha, x, y, A, lda);
#elif defined(C4A_USE_BLAS)
...
#endif  // CHEMOMETRICS4ALL_CORE_LINALG_HPP
