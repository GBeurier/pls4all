...
struct c4a_filter_leverage_state_t {
 ...
 * -------------------------------------------------------------------------- */
static c4a_status_t jacobi_eigh(double* A, int64_t n, double* eigvals,
                                  double* V) {
    if (n < 1) {
...
c4a_filter_leverage_state_t* c4a_filter_leverage_state_new(
    ...
    int32_t n_components,
    int     center) {
    if (method != C4A_FILTER_LEVERAGE_METHOD_HAT &&
        method != C4A_FILTER_LEVERAGE_METHOD_PCA) {
        return NULL;
    }
    if (use_absolute) {
    ...
    c4a_filter_leverage_state_t* s =
        (c4a_filter_leverage_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    memset(s, 0, sizeof(*s));
...
double c4a_filter_leverage_state_threshold(
 ...
 * state->tau. */
static c4a_status_t fit_hat(c4a_filter_leverage_state_t* state,
                              const double* Xc, int64_t rows, int64_t cols) {
    state->effective_method = C4A_FILTER_LEVERAGE_METHOD_HAT;
    ...
    const c4a_status_t qrst = c4a_householder_qr(M, cols, cols, tau);
    if (qrst != C4A_OK) {
        free(M); free(tau);
        return qrst;
    }
...
static c4a_status_t fit_pca(c4a_filter_leverage_state_t* state,
                             const double* Xc, int64_t rows, int64_t cols) {
    state->effective_method = C4A_FILTER_LEVERAGE_METHOD_PCA;
    ...
    compute_xtx(Xc, rows, cols, M);
    const c4a_status_t jst = jacobi_eigh(M, cols, eigvals, V);
    if (jst != C4A_OK) {
        free(M); free(V); free(eigvals);
        return jst;
    }
...
static c4a_status_t compute_leverages(const c4a_filter_leverage_state_t* state,
                                       const double* Xc, int64_t rows,
                                       int64_t cols, double* out) {
    if (state->effective_method == C4A_FILTER_LEVERAGE_METHOD_HAT) {
        ...
        for (int64_t r = 0; r < rows; ++r) {
            ...
            const c4a_status_t qst = c4a_apply_qt(state->M_qr, cols, cols,
                                                     state->tau, tmp);
            if (qst != C4A_OK) { free(tmp); free(sol); return qst; }
            const c4a_status_t bst = c4a_back_solve_R(state->M_qr, cols, cols,
                                                         tmp, sol);
            if (bst != C4A_OK) { free(tmp); free(sol); return bst; }
            double h = 0.0;
            for (int64_t j = 0; j < cols; ++j) {
                h += xrow[j] * sol[j];
            }
...
c4a_status_t c4a_filter_leverage_state_fit(c4a_filter_leverage_state_t* state,
    ...
    c4a_status_t st;
    if (state->method == C4A_FILTER_LEVERAGE_METHOD_PCA || cols >= rows) {
...
c4a_status_t c4a_filter_leverage_state_apply(
    ...
    const c4a_status_t st = compute_leverages(state, Xc, rows, cols, lev);
    if (st != C4A_OK) {
        free(Xc); free(lev);
        return st;
    }
