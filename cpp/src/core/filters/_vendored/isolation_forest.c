...
#define C4A_IFOREST_INITIAL_NODES 31
...
static int tree_ensure_capacity(iforest_tree_t* t, int32_t needed) {
    ...
    int32_t new_cap = (t->cap == 0) ? C4A_IFOREST_INITIAL_NODES : t->cap;
...
static double rng_uniform(c4a_rng_pcg64* rng) {
    return c4a_pcg64_engine_next_double(rng);
}

static int64_t rng_int_below(c4a_rng_pcg64* rng, int64_t upper) {
    /* Lemire-style rejection-free range (acceptable for small upper). */
    if (upper <= 1) return 0;
    const uint64_t mask = (uint64_t)upper;
    uint64_t x = c4a_pcg64_engine_next_uint64(rng) & 0x7FFFFFFFFFFFFFFFULL;
...
static c4a_status_t build_tree(iforest_tree_t* t,
    ...
    if (perm == NULL) return C4A_ERR_OUT_OF_MEMORY;
...
c4a_iforest_t* c4a_iforest_new(void) {
    c4a_iforest_t* f = (c4a_iforest_t*)malloc(sizeof(c4a_iforest_t));
...
c4a_status_t c4a_iforest_fit(c4a_iforest_t* f,
                              ...
                              uint64_t seed) {
    if (f == NULL) return C4A_ERR_NULL_POINTER;
    if (rows < 1 || cols < 1) return C4A_ERR_INVALID_ARGUMENT;
    if (X == NULL) return C4A_ERR_NULL_POINTER;
    if (n_estimators < 1) return C4A_ERR_INVALID_ARGUMENT;
    if (max_samples < 1) return C4A_ERR_INVALID_ARGUMENT;
    ...
    if (f->trees == NULL) return C4A_ERR_OUT_OF_MEMORY;
    ...
    for (int i = 0; i < n_estimators; ++i) {
        const c4a_status_t st = build_tree(&f->trees[i], X, rows, cols,
                                              f->max_samples, &rng);
...
c4a_status_t c4a_iforest_score(const c4a_iforest_t* f,
                                const double* X, int64_t rows, int64_t cols,
                                double* scores) {
    if (f == NULL || scores == NULL) return C4A_ERR_NULL_POINTER;
    if (rows < 0 || cols < 0) return C4A_ERR_INVALID_ARGUMENT;
    if (rows == 0) return C4A_OK;
    if (X == NULL) return C4A_ERR_NULL_POINTER;
    if (cols != f->cols) return C4A_ERR_SHAPE_MISMATCH;
    if (f->n_trees == 0) return C4A_ERR_NOT_FITTED;
