...
[[nodiscard]] n4m_status_t validate_request(::n4m::core::Context& ctx,
    ...
    if (X.rows == 0 || X.cols == 0 || Y.cols == 0) {
        ctx.set_error("shaving matrices must be non-empty");
    ...
    if (plan.folds.empty()) {
        ctx.set_error("shaving requires at least one validation fold");
        return N4M_ERR_INVALID_ARGUMENT;
    }
    if (n_steps <= 0) {
    ...
    if (!std::isfinite(shave_fraction) || shave_fraction <= 0.0 || shave_fraction >= 1.0) {
        ctx.set_error("shave_fraction must be finite and in (0, 1)");
        return N4M_ERR_INVALID_ARGUMENT;
    }
    if (X.cols > static_cast<std::int64_t>(std::numeric_limits<std::int32_t>::max()) ||
        ...
        ctx.set_error("shaving matrix dimensions exceed int32 result storage");
        return N4M_ERR_INVALID_ARGUMENT;
    }
    return N4M_OK;
}
...
[[nodiscard]] n4m_status_t copy_columns(::n4m::core::Context& ctx,
    ...
    if (columns.empty()) {
        ctx.set_error("shaving column selection must not be empty");
        return N4M_ERR_INVALID_ARGUMENT;
    }
    std::size_t n_values = 0;
    if (!checked_matrix_size(X.rows, static_cast<std::int64_t>(columns.size()), n_values)) {
        ctx.set_error("shaving subset matrix shape is too large");
        return N4M_ERR_INVALID_ARGUMENT;
    }
    ...
    for (std::size_t local_col = 0; local_col < cols; ++local_col) {
        ...
        if (original_col < 0 || original_col >= X.cols) {
            ctx.set_error("shaving column index out of range");
            return N4M_ERR_INVALID_ARGUMENT;
        }
        ...
        for (std::size_t row = 0; row < rows; ++row) {
            ...
            if (!std::isfinite(value)) {
                ctx.set_error("shaving X contains NaN or Inf");
                return N4M_ERR_INVALID_ARGUMENT;
            }
...
[[nodiscard]] std::vector<std::int64_t> shave_active(
    ...
    next.reserve(active.size() - static_cast<std::size_t>(remove_count));
...
n4m_status_t select_by_shaving(Context& ctx,
                               const Config& cfg,
                               const n4m_matrix_view_t& X,
                               const n4m_matrix_view_t& Y,
    ...
    try {
        ...
        for (std::int32_t step = 0; step < n_steps; ++step) {
            ...
            if (active_scores.size() != active.size()) {
                ctx.set_error("shaving active score count is inconsistent");
                out = ShavingSelectionResult{};
                return N4M_ERR_INTERNAL;
            }
        ...
        if (best_step < 0) {
            ctx.set_error("shaving did not produce a candidate subset");
            out = ShavingSelectionResult{};
            return N4M_ERR_INTERNAL;
        }
        ...
        ctx.set_error("out of memory while running shaving selection");
        out = ShavingSelectionResult{};
        return N4M_ERR_OUT_OF_MEMORY;
    } catch (...) {
        ctx.set_error("unexpected exception while running shaving selection");
        out = ShavingSelectionResult{};
        return N4M_ERR_INTERNAL;
    }
}
