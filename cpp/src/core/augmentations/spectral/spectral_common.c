 ...
 * via the shared `c4a_pad_resolve_index` helper from padding.h.
 *
...
static inline void c4a_aug_umul128(uint64_t a, uint64_t b,
 ...
 * engine before each apply (via `c4a_rng_pcg64_set_seed`) so buffer state
 ...
 * `c4a_rng_pcg64_state_t` is opaque we instead refill on every call and
 ...
 * augmenter ABI: `c4a_aug_randint_state_t`. Each operator builds one in
...
static uint32_t c4a_aug_next_uint32(c4a_aug_randint_state_t* state) {
...
int64_t c4a_aug_randint(c4a_aug_randint_state_t* state,
    ...
    if (rng_excl <= 0xFFFFFFFFu) {
        ...
        uint32_t x = c4a_aug_next_uint32(state);
    ...
    uint64_t x = c4a_pcg64_engine_next_uint64(state->rng);
    uint64_t m_hi = 0;
    uint64_t m_lo = 0;
    c4a_aug_umul128(x, rng_excl, &m_hi, &m_lo);
    if (m_lo < rng_excl) {
        const uint64_t t = ((uint64_t)0u - rng_excl) % rng_excl;
        while (m_lo < t) {
            x = c4a_pcg64_engine_next_uint64(state->rng);
            c4a_aug_umul128(x, rng_excl, &m_hi, &m_lo);
        }
    }
    return (int64_t)m_hi + lo;
