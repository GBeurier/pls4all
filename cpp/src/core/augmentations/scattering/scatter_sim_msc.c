...
struct c4a_aug_scatter_sim_state_t {
    double a_low;
    double a_high;
    double b_low;
    double b_high;
...
c4a_aug_scatter_sim_state_t* c4a_aug_scatter_sim_state_new(
    double a_low, double a_high, double b_low, double b_high) {
    if (a_high < a_low || b_high < b_low) {
        return NULL;
    }
    c4a_aug_scatter_sim_state_t* s =
        (c4a_aug_scatter_sim_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->a_low  = a_low;
...
void c4a_aug_scatter_sim_state_free(c4a_aug_scatter_sim_state_t* state) {
    free(state);
}

c4a_status_t c4a_aug_scatter_sim_apply_impl(
    const c4a_aug_scatter_sim_state_t* state,
    c4a_rng_pcg64* rng,
    ...
    c4a_aug_rng_uniform_fill(rng, state->a_low, state->a_high, a, (size_t)rows);
    c4a_aug_rng_uniform_fill(rng, state->b_low, state->b_high, b, (size_t)rows);
