...
struct c4a_aug_emsc_distort_state_t {
    double  mult_low;
    double  mult_high;
    double  add_low;
    double  add_high;
...
c4a_aug_emsc_distort_state_t* c4a_aug_emsc_distort_state_new(
    ...
    c4a_aug_emsc_distort_state_t* s =
        (c4a_aug_emsc_distort_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->mult_low            = mult_low;
...
c4a_status_t c4a_aug_emsc_distort_apply_impl(
    ...
    for (int64_t i = 0; i < rows; ++i) {
        ...
        double b_raw = c4a_aug_rng_standard_normal(rng) * b_std + b_mean;
        ...
        double a_raw = c4a_aug_rng_standard_normal(rng) * (a_std * a_dev_scale)
                       + a_mean_correlated;
        ...
        for (int32_t k = 1; k <= state->polynomial_order; ++k) {
            const double coef_std = state->polynomial_strength
                                    / sqrt((double)k);
            coef[k - 1] = c4a_aug_rng_standard_normal(rng) * coef_std;
        }
