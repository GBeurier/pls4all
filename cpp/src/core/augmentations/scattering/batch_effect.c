...
struct c4a_aug_batch_effect_state_t {
    double  offset_std;
    double  slope_std;
    double  gain_std;
...
c4a_aug_batch_effect_state_t* c4a_aug_batch_effect_state_new(
    ...
    c4a_aug_batch_effect_state_t* s =
        (c4a_aug_batch_effect_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
...
c4a_status_t c4a_aug_batch_effect_apply_impl(
    ...
    if (state->variation_scope == 1) {
        ...
        const double offset = c4a_aug_rng_standard_normal(rng) * state->offset_std;
        const double slope  = c4a_aug_rng_standard_normal(rng) * state->slope_std;
        const double gain   = c4a_aug_rng_standard_normal(rng) * state->gain_std + 1.0;
        ...
        c4a_aug_rng_normal_fill(rng, 0.0, state->offset_std, offsets, (size_t)rows);
        c4a_aug_rng_normal_fill(rng, 0.0, state->slope_std,  slopes,  (size_t)rows);
        c4a_aug_rng_normal_fill(rng, 1.0, state->gain_std,   gains,   (size_t)rows);
        for (int64_t i = 0; i < rows; ++i) {
