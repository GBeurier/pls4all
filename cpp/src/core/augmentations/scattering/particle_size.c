 ...
 * the SciPy default). We reuse the existing c4a_pp_gaussian engine to get
...
struct c4a_aug_particle_size_state_t {
    double mean_size_um;
...
c4a_aug_particle_size_state_t* c4a_aug_particle_size_state_new(
    ...
    c4a_aug_particle_size_state_t* s =
        (c4a_aug_particle_size_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
...
c4a_status_t c4a_aug_particle_size_apply_impl(
     ...
     * per row. We use the existing c4a_pp_gaussian engine with default
     * scipy parameters (reflect mode, truncate=4.0). */
    c4a_pp_gaussian_state_t* gauss = c4a_pp_gaussian_state_new(
        3.0, 0, C4A_PP_GAUSSIAN_REFLECT, 0.0, 4.0);
    if (gauss == NULL) {
    ...
    if (noise_in == NULL || noise_out == NULL) {
        free(sizes); free(wl_factor);
        c4a_pp_gaussian_state_free(gauss);
    ...
    for (int64_t i = 0; i < rows; ++i) {
         ...
         * is the c4a contract. */
        c4a_aug_rng_normal_fill(rng, 0.0, noise_std,
                                noise_in, (size_t)cols);
        c4a_pp_gaussian_state_apply(gauss, noise_in, 1, cols, noise_out);
        for (int64_t j = 0; j < cols; ++j) {
