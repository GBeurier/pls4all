...
c4a_status_t c4a_aug_dead_band_apply_impl(
    ...
    const uint64_t width_count =
        (uint64_t)(state->width_high - state->width_low + 1);
    if (state->variation_scope == 1) {
        ...
        for (int32_t b = 0; b < state->n_bands; ++b) {
            const int32_t width = state->width_low
                + (int32_t)c4a_aug_rng_integers(rng, width_count);
            const int64_t start_max = (cols - width > 0) ? (cols - width) : 1;
            const int32_t start = (int32_t)c4a_aug_rng_integers(rng,
                                                    (uint64_t)start_max);
