...
struct c4a_aug_temperature_state_t {
...
c4a_aug_temperature_state_t* c4a_aug_temperature_state_new(
    ...
    c4a_aug_temperature_state_t* s =
        (c4a_aug_temperature_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
...
static c4a_status_t apply_broaden(const double* row, int64_t cols,
    ...
    if (g == NULL) return C4A_ERR_INVALID_ARGUMENT;
    double* tmp = (double*)malloc((size_t)cols * sizeof(double));
    if (tmp == NULL) {
        c4a_pp_gaussian_state_free(g);
        return C4A_ERR_OUT_OF_MEMORY;
    }
    c4a_pp_gaussian_state_apply(g, row, 1, cols, tmp);
    for (int64_t j = 0; j < cols; ++j) {
...
c4a_status_t c4a_aug_temperature_apply_impl(
    ...
    if (deltas == NULL) return C4A_ERR_OUT_OF_MEMORY;
    if (state->use_temp_range) {
        c4a_aug_rng_uniform_fill(rng, state->temp_low, state->temp_high,
                                 deltas, (size_t)rows);
    } else {
    ...
    for (int64_t i = 0; i < rows; ++i) {
        ...
        if (state->region_specific) {
            ...
            if (state->enable_shift) {
                ...
                c4a_aug_np_interp(wavelengths, cols, shifted_wl, row, cols, tmp);
                free(shifted_wl);
                memcpy(row, tmp, (size_t)cols * sizeof(double));
            }
            if (state->enable_intensity) {
