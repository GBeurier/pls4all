...
struct n4m_aug_local_clip_state_t {
    n4m_rng_pcg64* rng;
    int32_t        n_regions;
    int32_t        width_lo;
    int32_t        width_hi;
...
n4m_aug_local_clip_state_t* n4m_aug_local_clip_state_new(
    ...
    n4m_aug_local_clip_state_t* s =
        (n4m_aug_local_clip_state_t*)malloc(sizeof(*s));
...
n4m_status_t n4m_aug_local_clip_state_apply(
    ...
    if (rows == 0 || cols == 0 || state->n_regions == 0) return N4M_OK;
    ...
    n4m_aug_randint_state_reset(&rs, state->rng);
    for (size_t k = 0; k < total; ++k) {
        widths[k] = n4m_aug_randint(&rs, state->width_lo, state->width_hi);
    }
