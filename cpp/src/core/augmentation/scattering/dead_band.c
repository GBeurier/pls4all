...
n4m_aug_dead_band_state_t* n4m_aug_dead_band_state_new(
    ...
    n4m_aug_dead_band_state_t* s =
        (n4m_aug_dead_band_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->n_bands         = n_bands;
...
n4m_status_t n4m_aug_dead_band_apply_impl(
    ...
    if (state->variation_scope == 1) {
        ...
        for (int32_t b = 0; b < state->n_bands; ++b) {
            ...
            n4m_aug_rng_normal_fill(rng, 0.0, state->noise_std, tmp,
