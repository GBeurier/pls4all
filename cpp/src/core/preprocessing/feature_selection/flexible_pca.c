...
struct c4a_pp_flex_pca_state_t {
...
c4a_pp_flex_pca_state_t* c4a_pp_flex_pca_state_new(double n_components_param) {
    ...
    c4a_pp_flex_pca_state_t* s = (c4a_pp_flex_pca_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
...
c4a_status_t c4a_pp_flex_pca_state_fit(c4a_pp_flex_pca_state_t* state,
    ...
    const c4a_status_t svd_st = c4a_svd_compact(Xc, rows, cols, U, S, Vt);
...
c4a_status_t c4a_pp_flex_pca_state_apply(const c4a_pp_flex_pca_state_t* state,
     ...
     * Iterate per row, per output component, accumulating along
     * features to match the NumPy `(X - mean) @ components_.T`
     * reduction order. */
    for (int64_t i = 0; i < rows; ++i) {
