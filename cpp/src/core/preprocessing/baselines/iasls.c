...
n4m_pp_iasls_state_t* n4m_pp_iasls_state_new(double lam, double p,
    ...
    return n4m_pp_iasls_state_new_ex(lam, p, 1e-4, polyorder, 2,
                                      max_iter, tol);
}
...
n4m_pp_iasls_state_t* n4m_pp_iasls_state_new_ex(double lam, double p,
    ...
    n4m_pp_iasls_state_t* s =
        (n4m_pp_iasls_state_t*)malloc(sizeof(*s));
...
n4m_status_t n4m_pp_iasls_state_apply(const n4m_pp_iasls_state_t* state,
    ...
    n4m_second_diff_penalty_pent5(n, lam, pen_main, pen_sup1, pen_sup2);
    ...
    const n4m_status_t qrst = n4m_householder_qr(V_qr, n, pcoefs, tau);
    if (qrst != N4M_OK) {
        free(pen_main); free(pen_sup1); free(pen_sup2);
        free(main_d); free(w); free(w_new); free(z); free(rhs);
        free(d1_y);
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        n4m_status_t st = n4m_apply_qt(V_qr, n, pcoefs, tau, qt_y);
        if (st != N4M_OK) { goto fail; }
        st = n4m_back_solve_R(V_qr, n, pcoefs, qt_y, coefs);
        if (st != N4M_OK) { goto fail; }
        ...
        for (int32_t it = 0; it <= max_iter; ++it) {
            ...
            st = n4m_banded5_factor_into(L_buf, D_buf, main_d,
                                          pen_sup1, pen_sup2, n);
            if (st != N4M_OK) { goto fail; }
            st = n4m_banded5_solve_into(L_buf, D_buf, n, rhs, z);
            if (st != N4M_OK) { goto fail; }
            for (int64_t i = 0; i < n; ++i) {
                w_new[i] = (y[i] > z[i]) ? p : (1.0 - p);
            }
            const double rdiff = n4m_relative_l2_diff(w, w_new, n);
            if (rdiff < tol) {
                break;
            }
