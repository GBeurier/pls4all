 ...
 *   out := y - z
 *
 * vs AsLS: the only difference is the initial weight vector (AsLS uses
 * ones; IAsLS uses the asymmetry rule applied to the polynomial pre-fit).
 * The penalty / solve / convergence test are identical.
 */
...
c4a_status_t c4a_pp_iasls_state_apply(const c4a_pp_iasls_state_t* state,
    ...
    if (!pen_main || !pen_sup1 || !pen_sup2 || !main_d || !w || !w_new ||
        !z || !rhs || !L_buf || !D_buf || !V_orig || !V_qr || !tau ||
        !qt_y || !coefs) {
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        /* AsLS-style banded reweighting loop. */
