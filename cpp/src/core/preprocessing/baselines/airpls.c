...
struct n4m_pp_airpls_state_t {
    double  lam;
    int32_t max_iter;
    double  tol;
};

n4m_pp_airpls_state_t* n4m_pp_airpls_state_new(double lam, int32_t max_iter,
                                                 double tol) {
    if (!(lam > 0.0) || max_iter < 0 || !(tol >= 0.0)) {
        return NULL;
    }
    n4m_pp_airpls_state_t* s = (n4m_pp_airpls_state_t*)malloc(sizeof(*s));
