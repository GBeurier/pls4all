 ...
 * dependency, fitting the chemometrics4all "pure C, no deps" constraint.
 *
 * Pure C, no dependencies. INTERNAL: never exposed by the public ABI.
 */
#ifndef CHEMOMETRICS4ALL_CORE_COMMON_SVD_H
#define CHEMOMETRICS4ALL_CORE_COMMON_SVD_H
 ...
 * Returns:
 *   - C4A_OK on success.
 *   - C4A_ERR_NULL_POINTER if any of A, U, S, Vt is NULL with m > 0 and
 *     n > 0.
 *   - C4A_ERR_INVALID_ARGUMENT for m < 1 or n < 1.
 *   - C4A_ERR_OUT_OF_MEMORY when an internal scratch allocation fails.
 *   - C4A_ERR_CONVERGENCE_FAILED if the Jacobi sweep limit is reached
...
#endif /* CHEMOMETRICS4ALL_CORE_COMMON_SVD_H */
