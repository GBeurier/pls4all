...
n4m_split_splt_state_t* n4m_split_splt_state_new(double test_size,
    ...
    n4m_split_splt_state_t* s =
        (n4m_split_splt_state_t*)malloc(sizeof(*s));
...
n4m_status_t n4m_split_splt_apply(const n4m_split_splt_state_t* state,
    ...
    if (keep_col == NULL) return N4M_ERR_OUT_OF_MEMORY;
    int64_t kept = 0;
    ...
    if (Xz == NULL) { free(keep_col); return N4M_ERR_OUT_OF_MEMORY; }
    {
        int64_t k = 0;
        for (int64_t c = 0; c < cols; ++c) {
    ...
    const double u_start = n4m_split_rng_uniform01(&rng);
    ...
    n4m_status_t st = n4m_split_result_alloc(out, n_train_actual, n_test_actual);
    if (st != N4M_OK) {
