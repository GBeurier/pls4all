...
c4a_split_spxy_fold_state_t* c4a_split_spxy_fold_state_new(int32_t n_splits,
                                                            int32_t use_y) {
    if (n_splits < 2) return NULL;
    c4a_split_spxy_fold_state_t* s =
        (c4a_split_spxy_fold_state_t*)malloc(sizeof(*s));
...
void c4a_split_spxy_fold_state_free(c4a_split_spxy_fold_state_t* state) {
    free(state);
}
...
static c4a_status_t compute_distance_and_centroid(
    const c4a_split_spxy_fold_state_t* s,
    const double* X, int64_t rows, int64_t cols_x,
    const double* Y, int64_t cols_y,
    double* D, double* centroid_distances) {

    c4a_status_t st = c4a_split_euclidean_dist(X, rows, cols_x, D);
    if (st != C4A_OK) return st;
    c4a_split_normalise_distance_matrix(D, rows);

    if (s->use_y == 1) {
        if (Y == NULL) return C4A_ERR_NULL_POINTER;
        double* DY = (double*)malloc((size_t)rows * (size_t)rows * sizeof(double));
        if (DY == NULL) return C4A_ERR_OUT_OF_MEMORY;
        st = c4a_split_euclidean_dist(Y, rows, cols_y, DY);
        if (st != C4A_OK) { free(DY); return st; }
...
c4a_status_t c4a_split_spxy_fold_assign(const c4a_split_spxy_fold_state_t* s,
    ...
    c4a_status_t st = compute_distance_and_centroid(s, X, rows, cols_x,
                                                     Y, cols_y, D, centroid);
    if (st != C4A_OK) {
        free(D); free(centroid);
        return st;
...
c4a_status_t c4a_split_spxy_fold_extract(const int32_t* fold_assignment,
