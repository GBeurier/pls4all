...
c4a_split_bsgk_state_t* c4a_split_bsgk_state_new(int32_t n_splits,
    ...
    c4a_split_bsgk_state_t* s =
        (c4a_split_bsgk_state_t*)malloc(sizeof(*s));
...
static void fisher_yates(int64_t* arr, int64_t n, c4a_rng_pcg64* rng) {
...
c4a_status_t c4a_split_bsgk_extract(const int32_t* fold_assignment,
    ...
    if (rows < 1 || fold_idx < 0) return C4A_ERR_INVALID_ARGUMENT;
    int64_t n_test = 0;
    for (int64_t i = 0; i < rows; ++i) {
        if (fold_assignment[i] == fold_idx) ++n_test;
    }
    int64_t n_train = rows - n_test;
    if (n_train < 1 || n_test < 1) return C4A_ERR_INVALID_ARGUMENT;
