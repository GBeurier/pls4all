...
[[nodiscard]] n4m_status_t update_weights(::n4m::core::Context& ctx,
    ...
    for (std::size_t feature = 0; feature < weights.size(); ++feature) {
        const double next =
            damping * weights[feature] + (1.0 - damping) * scores[feature];
        weights[feature] = std::max(weight_floor, next);
        max_weight = std::max(max_weight, weights[feature]);
    }
...
[[nodiscard]] std::vector<std::int64_t> top_k_subset(const std::vector<std::int64_t>& ranking,
                                                     std::int32_t top_k) {
    std::vector<std::int64_t> selected;
    selected.reserve(static_cast<std::size_t>(top_k));
    for (std::int32_t i = 0; i < top_k; ++i) {
...
n4m_status_t select_by_ipw(Context& ctx,
    ...
    try {
        ...
        out.score_path.assign(static_cast<std::size_t>(n_iterations) * p, 0.0);
        out.weight_path.assign(static_cast<std::size_t>(n_iterations) * p, 0.0);
        out.rmse_by_iteration.reserve(static_cast<std::size_t>(n_iterations));
        out.selected_by_iteration.reserve(static_cast<std::size_t>(n_iterations) *
        ...
        for (std::int32_t iteration = 0; iteration < n_iterations; ++iteration) {
            ...
            for (std::size_t feature = 0; feature < p; ++feature) {
                out.score_path[static_cast<std::size_t>(iteration) * p + feature] =
                    scores[feature];
            }
            ...
            for (std::size_t feature = 0; feature < p; ++feature) {
                out.weight_path[static_cast<std::size_t>(iteration) * p + feature] =
                    weights[feature];
            }

            std::vector<std::int64_t> ranking = rank_indices(weights);
            std::vector<std::int64_t> selected = top_k_subset(ranking, top_k);
            out.selected_by_iteration.insert(out.selected_by_iteration.end(),
                                             selected.begin(),
                                             selected.end());
