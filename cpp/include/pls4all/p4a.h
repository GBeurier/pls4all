 ...
 *     SIMPLS, kernel, wide-kernel, SVD and power-iteration PLS
 ...
 *     P4A_SOLVER_WIDE_KERNEL, P4A_SOLVER_SVD or P4A_SOLVER_POWER +
...
typedef enum p4a_algorithm_t {
     ...
     * Orthogonal-scores, SIMPLS, kernel-algorithm, wide-kernel and power are
     * SOLVERS (see below), not algorithms. POP-PLS is a GATING STRATEGY
     * (see below). */
} p4a_algorithm_t;
 ...
 * P4A_SOLVER_KERNEL_ALGORITHM, P4A_SOLVER_WIDE_KERNEL, P4A_SOLVER_SVD or
 * P4A_SOLVER_POWER, and P4A_ALGO_PCR with P4A_SOLVER_SVD. Both use P4A_DEFLATION_REGRESSION. Unsupported algorithms /
...
P4A_API const char*  p4a_get_version_string(void);    /* e.g. "0.9.0+abi.1.0.0" */
