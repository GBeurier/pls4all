...
verbatim — the n4m engine reproduces the same arithmetic order and
threshold semantics. Because every check reduces to a single comparison,
the n4m output is bit-exact (mask equality, not float tolerance).
"""
...
def spectral_quality_mask(X: np.ndarray,
    ...
    if n_features == 0:
        ...
        # behaviour of the n4m engine when cols == 0).
