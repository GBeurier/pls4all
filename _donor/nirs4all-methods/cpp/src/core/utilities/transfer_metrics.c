...
#ifndef N4M_TRANSFER_JACOBI_MAX_SWEEPS
#  define N4M_TRANSFER_JACOBI_MAX_SWEEPS 100
#endif

/* Fixed subsample size for the spread-distance pairwise step, matching the
 * Python reference. */
#define N4M_TRANSFER_SPREAD_SUBSAMPLE 100
 ...
 *
 * Returns N4M_OK on convergence within N4M_TRANSFER_JACOBI_MAX_SWEEPS sweeps,
 * N4M_ERR_NUMERICAL_FAILURE otherwise. p == 0 returns OK with no work. */
static n4m_status_t jacobi_eig_sym(double* A, int64_t p, double* V) {
    if (p <= 0) {
        return N4M_OK;
    }
    ...
    for (int sweep = 0; sweep < N4M_TRANSFER_JACOBI_MAX_SWEEPS; ++sweep) {
 ...
 * frees them via `pca_result_free`. Returns N4M_OK on success.
 *
...
static n4m_status_t pca_fit(const double* Xc, int64_t n, int64_t p,
    ...
    gram_xtx(Xc, n, p, G);
    const n4m_status_t st = jacobi_eig_sym(G, p, V);
    if (st != N4M_OK) {
        free(G); free(V); free(eigs_full);
        return st;
    }
...
static double procrustes_disparity(const double* Zs, int64_t ns,
    ...
    const n4m_status_t st = jacobi_eig_sym(MtM, d, Vd);
    if (st != N4M_OK) {
        free(A); free(B); free(M); free(MtM); free(Vd);
        return nan_value();
    }
 ...
 * computed on subsamples of size up to N4M_TRANSFER_SPREAD_SUBSAMPLE.
 */
static double spread_distance(const double* Zs, int64_t ns,
    ...
    int32_t n1 = (int32_t)((ns < N4M_TRANSFER_SPREAD_SUBSAMPLE) ? ns : N4M_TRANSFER_SPREAD_SUBSAMPLE);
    int32_t n2 = (int32_t)((nt < N4M_TRANSFER_SPREAD_SUBSAMPLE) ? nt : N4M_TRANSFER_SPREAD_SUBSAMPLE);
     ...
     * reference for n4m is the C-engine-friendly variant that uses the same
...
static double grassmann_distance(const double* Us, const double* Ut,
    ...
    const n4m_status_t st = jacobi_eig_sym(MtM, r, Vd);
    if (st != N4M_OK) {
...
n4m_status_t n4m_transfer_metrics_compute_impl(
    ...
    n4m_status_t st = pca_fit(Xc_s, n_src, p_src, n_components, &pca_s);
    if (st != N4M_OK) {
        free(Xc_s); free(Xc_t);
        return st;
    }
