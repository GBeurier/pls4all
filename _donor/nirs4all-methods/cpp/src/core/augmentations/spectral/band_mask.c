...
struct n4m_aug_band_mask_state_t {
...
n4m_aug_band_mask_state_t* n4m_aug_band_mask_state_new(
    ...
    if (mode != N4M_AUG_BAND_MASK_ZERO && mode != N4M_AUG_BAND_MASK_INTERP) {
        return NULL;
    }
    n4m_aug_band_mask_state_t* s =
        (n4m_aug_band_mask_state_t*)malloc(sizeof(*s));
...
n4m_status_t n4m_aug_band_mask_state_apply(
    ...
    for (int64_t i = 0; i < rows; ++i) {
        n_per_sample[i] = n4m_aug_randint(&rs,
    ...
    for (int64_t i = 0; i < rows; ++i) {
        ...
        for (int64_t b = 0; b < nb_i; ++b) {
            ...
            if (state->mode == N4M_AUG_BAND_MASK_ZERO) {
