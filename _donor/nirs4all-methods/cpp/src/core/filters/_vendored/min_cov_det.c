...
#define N4M_MCD_MAX_ITERS 30
#define N4M_MCD_TOL       1e-12
...
static n4m_status_t factor_cov(const double* cov, int64_t cols,
                                double* M, double* tau,
                                double* out_log_det) {
    double trace = 0.0;
    ...
    const n4m_status_t qst = n4m_householder_qr(M, cols, cols, tau);
    if (qst != N4M_OK) return qst;
    double ld = 0.0;
    for (int64_t i = 0; i < cols; ++i) {
...
static n4m_status_t solve(const double* M, const double* tau, int64_t cols,
                           const double* b, double* x) {
    double* tmp = (double*)malloc((size_t)cols * sizeof(double));
    if (tmp == NULL) return N4M_ERR_OUT_OF_MEMORY;
    memcpy(tmp, b, (size_t)cols * sizeof(double));
    const n4m_status_t qst = n4m_apply_qt(M, cols, cols, tau, tmp);
    if (qst != N4M_OK) { free(tmp); return qst; }
    const n4m_status_t bst = n4m_back_solve_R(M, cols, cols, tmp, x);
    free(tmp);
    return bst;
}
 ...
 * factor of cov_reg. */
static n4m_status_t mahal_sq_row(const double* row, const double* mean,
    ...
    const n4m_status_t st = solve(M, tau, cols, diff, sol);
    if (st != N4M_OK) { free(diff); free(sol); return st; }
    double s = 0.0;
...
n4m_mcd_t* n4m_mcd_new(void) {
    return (n4m_mcd_t*)calloc(1, sizeof(n4m_mcd_t));
}
...
n4m_status_t n4m_mcd_fit(n4m_mcd_t* mcd,
                          ...
                          uint64_t seed) {
    if (mcd == NULL) return N4M_ERR_NULL_POINTER;
    ...
    if (h < 1 || h > rows) return N4M_ERR_INVALID_ARGUMENT;
    ...
    for (int64_t i = 0; i < h; ++i) {
        const uint64_t u = n4m_pcg64_engine_next_uint64(&rng);
    ...
    for (int iter = 0; iter < N4M_MCD_MAX_ITERS; ++iter) {
        double log_det = 0.0;
        st = factor_cov(cov, cols, M, tau, &log_det);
        if (st != N4M_OK) goto cleanup;
        /* Compute Mahalanobis for every row and pick the h smallest. */
        for (int64_t r = 0; r < rows; ++r) {
            double v = 0.0;
            const double* row = X + (size_t)r * (size_t)cols;
            const n4m_status_t mst = mahal_sq_row(row, mean, M, tau, cols, &v);
            if (mst != N4M_OK) { st = mst; goto cleanup; }
            pairs[r].d = v;
            pairs[r].i = r;
        }
        ...
        compute_mean(X, idx, h, cols, mean);
        compute_cov(X, idx, h, cols, mean, cov);
        if (iter > 0 && fabs(prev_log_det - log_det) < N4M_MCD_TOL) {
            break;
        }
...
n4m_status_t n4m_mcd_mahalanobis_sq(const n4m_mcd_t* mcd,
    ...
    if (mcd == NULL || mahal_sq == NULL) return N4M_ERR_NULL_POINTER;
    if (X_rows < 0 || cols < 0) return N4M_ERR_INVALID_ARGUMENT;
    if (X_rows == 0) return N4M_OK;
    if (X == NULL) return N4M_ERR_NULL_POINTER;
    if (!mcd->fitted) return N4M_ERR_NOT_FITTED;
    if (cols != mcd->cols) return N4M_ERR_SHAPE_MISMATCH;
    for (int64_t r = 0; r < X_rows; ++r) {
        const double* row = X + (size_t)r * (size_t)cols;
        const n4m_status_t st = mahal_sq_row(row, mcd->location,
                                                mcd->precision_qr,
                                                mcd->precision_tau,
                                                cols, &mahal_sq[r]);
