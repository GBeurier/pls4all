 ...
 *      Returns N4M_ERR_INVALID_STATE when called on an unfitted state.
 *
...
struct n4m_filter_y_outlier_state_t {
    int32_t method;
    double  threshold;
    double  lower_pct;
    double  upper_pct;
...
n4m_filter_y_outlier_state_t* n4m_filter_y_outlier_state_new(
    ...
    if (method != N4M_CORE_Y_OUTLIER_IQR    &&
        method != N4M_CORE_Y_OUTLIER_ZSCORE &&
        method != N4M_CORE_Y_OUTLIER_PERCENTILE &&
        method != N4M_CORE_Y_OUTLIER_MAD) {
        return NULL;
    }
    if (threshold <= 0.0) {
        return NULL;
    ...
    n4m_filter_y_outlier_state_t* s =
        (n4m_filter_y_outlier_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->method      = method;
