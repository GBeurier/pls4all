...
struct n4m_pp_wavelet_denoise_state_t {
...
n4m_pp_wavelet_denoise_state_t* n4m_pp_wavelet_denoise_state_new(
    ...
    n4m_pp_wavelet_denoise_state_t* s =
        (n4m_pp_wavelet_denoise_state_t*)malloc(sizeof(*s));
...
static void apply_threshold(double* buf, int64_t n, double thr,
                             n4m_pp_wavelet_threshold_mode_t mode) {
    if (mode == N4M_PP_WD_THRESHOLD_SOFT) {
...
n4m_status_t n4m_pp_wavelet_denoise_state_apply(
    ...
    const int32_t max_level =
        n4m_wavelet_dwt_max_level(cols, state->family);
    ...
    st = n4m_wavelet_wavedec_workspace_length(
        cols, state->family, state->mode, level, &dec_work_len);
    if (st == N4M_OK) {
        st = n4m_wavelet_waverec_workspace_length(cols, level, &rec_work_len);
    }
    if (st != N4M_OK) {
    ...
    for (int64_t i = 0; i < rows; ++i) {
        ...
        double sigma;
        if (state->noise_estimator == N4M_PP_WD_NOISE_MEDIAN) {
