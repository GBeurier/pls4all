/* SPDX-License-Identifier: CECILL-2.1 */

#include "wavelet_pca.h"

#include <math.h>
#include <stdlib.h>
#include <string.h>

#include "core/common/svd.h"

struct n4m_pp_wavelet_pca_state_t {
    /* Constructor parameters. */
    n4m_wavelet_family_t family;
    n4m_wavelet_mode_t   mode;
    int32_t              max_level;
    double               n_components_param;

    /* Fitted state. */
    int      fitted;
    int32_t  actual_level;
    int64_t  flat_dim;      /* features after DWT flatten */
    int64_t  n_components;  /* k' actually kept */
    int64_t  n_features_in; /* cols of the fit X */
    double*  mean;          /* length flat_dim */
    double*  components;    /* k' x flat_dim, row-major */
};

n4m_pp_wavelet_pca_state_t* n4m_pp_wavelet_pca_state_new(
    n4m_wavelet_family_t family,
    n4m_wavelet_mode_t   mode,
    int32_t              max_level,
    double               n_components_param) {
    if (max_level < 0) return NULL;
    if (!(n_components_param > 0.0)) return NULL;
    n4m_pp_wavelet_pca_state_t* s =
        (n4m_pp_wavelet_pca_state_t*)malloc(sizeof(*s));
    if (s == NULL) return NULL;
    s->family             = family;
    s->mode               = mode;
    s->max_level          = max_level;
    s->n_components_param = n_components_param;
    s->fitted             = 0;
    s->actual_level       = 0;
    s->flat_dim           = 0;
    s->n_components       = 0;
    s->n_features_in      = 0;
    s->mean               = NULL;
    s->components         = NULL;
    return s;
}

void n4m_pp_wavelet_pca_state_free(n4m_pp_wavelet_pca_state_t* state) {
    if (state == NULL) return;
    free(state->mean);
    free(state->components);
    free(state);
}

int n4m_pp_wavelet_pca_state_is_fitted(
    const n4m_pp_wavelet_pca_state_t* state) {
    return (state != NULL && state->fitted) ? 1 : 0;
}

int64_t n4m_pp_wavelet_pca_state_n_components(
    const n4m_pp_wavelet_pca_state_t* state) {
    return (state != NULL && state->fitted) ? state->n_components : 0;
}

/* Select k' the same way FlexiblePCA does. */
static int64_t pca_select_k(double param, int64_t k_full,
                             const double* var_ratio) {
    if (param >= 1.0) {
        int64_t k = (int64_t)param;
        if (k < 1) k = 1;
        if (k > k_full) k = k_full;
        return k;
    }
    double cum = 0.0;
    for (int64_t i = 0; i < k_full; ++i) {
        cum += var_ratio[i];
        if (cum >= param) return i + 1;
    }
    return k_full;
}

/* Helper: run multi-level DWT on a single row into ``flat_out`` (length
 * ``flat_dim``).  ``coef_lengths`` and ``offsets`` of length level+1 are
 * supplied by the caller.  ``flat_out`` stores the packed coefficients
 * verbatim — flattening is implicit because the wavedec lays them out
 * in [cA, cD_top, ..., cD_1] order. */
static n4m_status_t dwt_flatten_row(const double* row, int64_t cols,
                                      n4m_wavelet_family_t family,
                                      n4m_wavelet_mode_t mode,
                                      int32_t level,
                                      int64_t total_flat,
                                      int64_t* coef_lengths,
                                      int64_t* offsets,
                                      double* flat_out) {
    return n4m_wavelet_wavedec(row, cols, family, mode, level,
                                flat_out, coef_lengths, offsets,
                                total_flat);
}

n4m_status_t n4m_pp_wavelet_pca_state_fit(
    n4m_pp_wavelet_pca_state_t* state,
    const double* X, int64_t rows, int64_t cols) {
    if (state == NULL || X == NULL) {
        return N4M_ERR_NULL_POINTER;
    }
    if (rows < 2 || cols < 1) {
        return N4M_ERR_INVALID_ARGUMENT;
    }
    const int32_t max_lvl = n4m_wavelet_dwt_max_level(cols, state->family);
    const int32_t level   = (state->max_level < max_lvl) ? state->max_level
                                                          : max_lvl;
    /* Determine flat dimension. */
    int64_t flat_dim = 0;
    int64_t* coef_lengths = (int64_t*)malloc((size_t)(level + 1) * sizeof(int64_t));
    int64_t* offsets      = (int64_t*)malloc((size_t)(level + 1) * sizeof(int64_t));
    if (coef_lengths == NULL || offsets == NULL) {
        free(coef_lengths); free(offsets);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    n4m_status_t st = n4m_wavelet_wavedec_lengths(
        cols, state->family, state->mode, level, &flat_dim, coef_lengths);
    if (st != N4M_OK) {
        free(coef_lengths); free(offsets);
        return st;
    }
    offsets[0] = 0;
    {
        int64_t acc = coef_lengths[0];
        for (int32_t k = 1; k <= level; ++k) {
            offsets[k] = acc;
            acc += coef_lengths[k];
        }
    }
    if (flat_dim < 1) {
        free(coef_lengths); free(offsets);
        return N4M_ERR_INVALID_ARGUMENT;
    }

    /* Build the per-row coefficient matrix (rows x flat_dim). */
    double* F = (double*)malloc((size_t)rows * (size_t)flat_dim * sizeof(double));
    if (F == NULL) {
        free(coef_lengths); free(offsets);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    for (int64_t i = 0; i < rows; ++i) {
        const double* row = X + (size_t)i * (size_t)cols;
        st = dwt_flatten_row(row, cols, state->family, state->mode, level,
                              flat_dim, coef_lengths, offsets,
                              F + (size_t)i * (size_t)flat_dim);
        if (st != N4M_OK) {
            free(F); free(coef_lengths); free(offsets);
            return st;
        }
    }
    free(coef_lengths);
    free(offsets);

    /* PCA on F.  Same path as FlexiblePCA. */
    const int64_t m = rows;
    const int64_t n = flat_dim;
    const int64_t k_full = (m < n) ? m : n;
    double* mean = (double*)malloc((size_t)n * sizeof(double));
    if (mean == NULL) {
        free(F);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    for (int64_t j = 0; j < n; ++j) {
        double acc = 0.0;
        for (int64_t i = 0; i < m; ++i) {
            acc += F[(size_t)i * (size_t)n + (size_t)j];
        }
        mean[j] = acc / (double)m;
    }
    for (int64_t i = 0; i < m; ++i) {
        for (int64_t j = 0; j < n; ++j) {
            F[(size_t)i * (size_t)n + (size_t)j] -= mean[j];
        }
    }
    double* U  = (double*)malloc((size_t)m * (size_t)k_full * sizeof(double));
    double* S  = (double*)malloc((size_t)k_full * sizeof(double));
    double* Vt = (double*)malloc((size_t)k_full * (size_t)n * sizeof(double));
    if (U == NULL || S == NULL || Vt == NULL) {
        free(U); free(S); free(Vt); free(mean); free(F);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    st = n4m_svd_compact(F, m, n, U, S, Vt);
    free(F);
    if (st != N4M_OK) {
        free(U); free(S); free(Vt); free(mean);
        return st;
    }
    double* evar      = (double*)malloc((size_t)k_full * sizeof(double));
    double* var_ratio = (double*)malloc((size_t)k_full * sizeof(double));
    if (evar == NULL || var_ratio == NULL) {
        free(evar); free(var_ratio);
        free(U); free(S); free(Vt); free(mean);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    const double inv_dof = 1.0 / (double)(m - 1);
    double total = 0.0;
    for (int64_t i = 0; i < k_full; ++i) {
        evar[i] = S[i] * S[i] * inv_dof;
        total  += evar[i];
    }
    if (total > 0.0) {
        const double inv_total = 1.0 / total;
        for (int64_t i = 0; i < k_full; ++i) var_ratio[i] = evar[i] * inv_total;
    } else {
        for (int64_t i = 0; i < k_full; ++i) var_ratio[i] = 0.0;
    }
    const int64_t k_kept =
        pca_select_k(state->n_components_param, k_full, var_ratio);
    double* components =
        (double*)malloc((size_t)k_kept * (size_t)n * sizeof(double));
    if (components == NULL) {
        free(evar); free(var_ratio);
        free(U); free(S); free(Vt); free(mean);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    memcpy(components, Vt, (size_t)k_kept * (size_t)n * sizeof(double));
    free(evar); free(var_ratio);
    free(U); free(S); free(Vt);

    /* Commit. */
    free(state->mean);
    free(state->components);
    state->actual_level   = level;
    state->flat_dim       = n;
    state->n_components   = k_kept;
    state->n_features_in  = cols;
    state->mean           = mean;
    state->components     = components;
    state->fitted         = 1;
    return N4M_OK;
}

n4m_status_t n4m_pp_wavelet_pca_state_apply(
    const n4m_pp_wavelet_pca_state_t* state,
    const double* X, int64_t rows, int64_t cols,
    int64_t out_cols, double* out) {
    if (state == NULL || X == NULL || out == NULL) {
        return N4M_ERR_NULL_POINTER;
    }
    if (!state->fitted) return N4M_ERR_NOT_FITTED;
    if (rows < 0 || cols < 1 || out_cols < 0) {
        return N4M_ERR_INVALID_ARGUMENT;
    }
    if (cols != state->n_features_in) return N4M_ERR_SHAPE_MISMATCH;
    if (out_cols != state->n_components) return N4M_ERR_SHAPE_MISMATCH;
    if (rows == 0) return N4M_OK;

    /* Re-build coef_lengths / offsets — they depend on (family, mode,
     * level, cols), all fixed by the fit. */
    const int32_t level = state->actual_level;
    int64_t* coef_lengths = (int64_t*)malloc((size_t)(level + 1) * sizeof(int64_t));
    int64_t* offsets      = (int64_t*)malloc((size_t)(level + 1) * sizeof(int64_t));
    if (coef_lengths == NULL || offsets == NULL) {
        free(coef_lengths); free(offsets);
        return N4M_ERR_OUT_OF_MEMORY;
    }
    int64_t total_flat = 0;
    n4m_status_t st = n4m_wavelet_wavedec_lengths(
        cols, state->family, state->mode, level, &total_flat, coef_lengths);
    if (st != N4M_OK || total_flat != state->flat_dim) {
        free(coef_lengths); free(offsets);
        return (st != N4M_OK) ? st : N4M_ERR_SHAPE_MISMATCH;
    }
    offsets[0] = 0;
    {
        int64_t acc = coef_lengths[0];
        for (int32_t k = 1; k <= level; ++k) {
            offsets[k] = acc;
            acc += coef_lengths[k];
        }
    }
    double* flat = (double*)malloc((size_t)state->flat_dim * sizeof(double));
    if (flat == NULL) {
        free(coef_lengths); free(offsets);
        return N4M_ERR_OUT_OF_MEMORY;
    }

    for (int64_t i = 0; i < rows; ++i) {
        const double* row = X + (size_t)i * (size_t)cols;
        st = dwt_flatten_row(row, cols, state->family, state->mode, level,
                              state->flat_dim, coef_lengths, offsets, flat);
        if (st != N4M_OK) {
            free(flat); free(coef_lengths); free(offsets);
            return st;
        }
        /* out[i, k] = sum_j (flat[j] - mean[j]) * components[k, j]. */
        double* orow = out + (size_t)i * (size_t)out_cols;
        for (int64_t k = 0; k < state->n_components; ++k) {
            const double* comp_k =
                state->components + (size_t)k * (size_t)state->flat_dim;
            double acc = 0.0;
            for (int64_t j = 0; j < state->flat_dim; ++j) {
                acc += (flat[j] - state->mean[j]) * comp_k[j];
            }
            orow[k] = acc;
        }
    }
    free(flat); free(coef_lengths); free(offsets);
    return N4M_OK;
}
