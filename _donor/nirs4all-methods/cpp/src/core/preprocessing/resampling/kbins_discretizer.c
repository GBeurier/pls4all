...
#define N4M_KBINS_WIDTH_FLOOR 1e-8

struct n4m_pp_kbins_disc_state_t {
    int32_t  n_bins;
...
n4m_pp_kbins_disc_state_t* n4m_pp_kbins_disc_state_new(int32_t n_bins,
    ...
    n4m_pp_kbins_disc_state_t* s =
        (n4m_pp_kbins_disc_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->n_bins   = n_bins;
...
static void free_edges(n4m_pp_kbins_disc_state_t* s) {
    if (s->edges != NULL) {
...
static int32_t dedup_edges(double* edges, int32_t n) {
    ...
    for (int32_t r = 1; r < n; ++r) {
        if ((edges[r] - edges[w - 1]) > N4M_KBINS_WIDTH_FLOOR) {
            edges[w++] = edges[r];
        }
    }
    return w;
