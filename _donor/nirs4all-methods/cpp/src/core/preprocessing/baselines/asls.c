 ...
 *
 * Internal parity fixture: parity/python_generator/src/n4m_parity_pybaselines_ref/asls.py
 *
...
#define N4M_ASLS_RESTRICT __restrict__
#else
#define N4M_ASLS_RESTRICT
#endif

struct n4m_pp_asls_state_t {
    double  lam;
    double  p;
    int32_t max_iter;
    double  tol;
...
n4m_pp_asls_state_t* n4m_pp_asls_state_new(double lam, double p,
    ...
    n4m_pp_asls_state_t* s = (n4m_pp_asls_state_t*)malloc(sizeof(*s));
...
static n4m_status_t asls_factor_into(double* N4M_ASLS_RESTRICT L,
                                      double* N4M_ASLS_RESTRICT D,
                                      const double* N4M_ASLS_RESTRICT w,
...
static n4m_status_t asls_factor_weighted_into(double* N4M_ASLS_RESTRICT L,
                                               double* N4M_ASLS_RESTRICT D,
                                               const double* N4M_ASLS_RESTRICT w,
                                               const double* N4M_ASLS_RESTRICT y,
                                               double* N4M_ASLS_RESTRICT z,
                                               int64_t n, double lam) {
...
static void asls_solve_y_invdiag(const double* N4M_ASLS_RESTRICT L,
                                 const double* N4M_ASLS_RESTRICT invD,
                                 int64_t n,
                                 const double* N4M_ASLS_RESTRICT y,
                                 double* N4M_ASLS_RESTRICT z) {
    z[0] = y[0];
...
static void asls_backsolve_weighted_y(const double* N4M_ASLS_RESTRICT L,
                                      const double* N4M_ASLS_RESTRICT D,
                                      int64_t n,
                                      double* N4M_ASLS_RESTRICT z) {
    z[n - 1] = z[n - 1] / D[n - 1];
    z[n - 2] = z[n - 2] / D[n - 2]
...
static double asls_update_initial_weights(double* N4M_ASLS_RESTRICT w,
                                          const double* N4M_ASLS_RESTRICT y,
                                          const double* N4M_ASLS_RESTRICT z,
                                          int64_t n,
                                          double p, double q) {
    double num_sq = 0.0;
    for (int64_t i = 0; i < n; ++i) {
...
static double asls_update_weights(double* N4M_ASLS_RESTRICT w,
                                  const double* N4M_ASLS_RESTRICT y,
                                  const double* N4M_ASLS_RESTRICT z,
...
n4m_status_t n4m_pp_asls_state_apply(const n4m_pp_asls_state_t* state,
    ...
    n4m_status_t st = asls_factor_into(L_init, D_init, NULL, n, lam);
    if (st != N4M_OK) {
        free(scratch);
        return st;
    }
