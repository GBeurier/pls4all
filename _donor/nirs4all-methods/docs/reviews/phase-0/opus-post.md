...
**Scope**: Phase 0 bootstrap of nirs4all-methods (clone pls4all template, rename, strip PLS code, smoke build)
...
| 2 | `bindings/python/src/n4m/__init__.py` imports deleted `_aom`, `_config`, `_methods`, `_model` modules | Entire `bindings/` directory stripped (rebuilt in Phases 22-25) |
| 3 | 5+ downstream manifests still pin `0.97.0` while `n4m_version.h` declares `0.1.0` | `version-sync.yml` workflow moved to `.disabled/`; manifests live with the strip of `bindings/`, `parity/python_generator/`, `benchmarks/`; `docs/conf.py` now parses version from `n4m_version.h` dynamically |
| 4 | `n4m.h` still declares the full pls4all surface (~194 `N4M_API` decls) for a Phase 0 with 29 implementations | Phase 0 note added to header documenting the inheritance; subsequent phases trim the legacy declarations as they add canonical n4m operators. The declarations are NOT exported (`expected_symbols_*.txt` only lists what's actually linked). |
| 5 | `expected_symbols_*.txt` are empty but libn4m exports 29 symbols | Populated from `nm -D --defined-only libn4m.so.1.0.0` (29 symbols) for Linux, macOS, Windows |
| 6 | `CITATION.cff` is entirely pls4all metadata | Rewritten with nirs4all-methods scope, keywords (NIRS, SNV, MSC, Kennard-Stone, Hotelling T², transfer-learning, …), version 0.1.0, references pls4all + nirs4all |
| 7 | `parity-gate.yml` hard-codes `libn4m.so.0.52.0` and `0.52.0` assertion | Moved to `.github/workflows.disabled/parity-gate.yml`; re-enabled in Phase 2 with proper fixture content |
...
| 9 | `CONTRIBUTING.md` references deleted `Direction_Technique.md` + PLS workflow steps | Rewritten with nirs4all-methods workflow: pick a phase from ROADMAP.md, Codex pre-review of brief, atomic green-CI commits, Codex post-review, Opus independent post-review, push, user update |
...
| 12 | `docs/conf.py:32` hardcoded `0.97.0` | Fixed: now parses from `n4m_version.h` dynamically |
| 13 | `parity/python_generator/cli.py` generates 100+ PLS-only fixtures | Entire `parity/python_generator/` stripped; rebuilt in Phase 1+ with n4m adapters |
| 14 | `bindings/js/CMakeLists.txt` references missing PLS WASM symbols | `bindings/` entirely stripped (rebuilt in Phase 25 for JS) |
| 15 | `n4m.h:3` header doc says "v1.1.0" while `n4m_version.h` says ABI 1.0.0 | Deferred — Phase 1 will rewrite the n4m.h prose alongside the canonical surface trim |
...
- **(#18)** `n4m_linux.map` symbol-versioning script absent — fine for Phase 0; `-fvisibility=hidden` + `expected_symbols_*.txt` suffice. Reintroduced in Phase 22 alongside Python wheel build.
...
./build/dev-debug/cpp/cli/n4m_cli --version   → nirs4all-methods 0.1.0+abi.1.0.0 (ABI 1.0.0)
./build/dev-debug/cpp/cli/n4m_cli --selfcheck → selfcheck OK
ctest --preset dev-debug                                   → 100% tests passed (1/1: 7 sub-tests)
nm -D --defined-only build/dev-debug/cpp/src/libn4m.so.1.0.0 | awk '$2=="T" {print $3}' | wc -l → 29
diff <(nm -D --defined-only build/dev-debug/cpp/src/libn4m.so.1.0.0 | awk '$2=="T" {print $3}' | sort -u) cpp/abi/expected_symbols_linux.txt → no drift
```

## Things done well (preserved from review)

- Core code is consistently namespaced `n4m::core`, no stray `pls4all::` references.
- Build system rename is consistent across `CMakeLists.txt`, `cmake/N4M{Config,Options}.cmake`, `CMakePresets.json`, `N4MConfig.cmake.in`.
