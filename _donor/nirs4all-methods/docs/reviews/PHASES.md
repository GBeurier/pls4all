...
| 3 | Stateful scatter preprocessings + n4m.h cleanup | ACCEPT (3 follow-ups applied) | — | `98c9e25` | 51 / 51 |
...
| 6 | Wavelets & denoising (6 ops, `n4m_pp_wavelet_*`) | — (parallel batch) | — | (batch commit) | 12 / 12 (subset) |
...
| 13 | Filters: X-based outliers (6 methods, `n4m_filter_x_outlier_*`) | — (parallel batch) | — | (batch commit) | 12 / 12 (subset) |
| 14 | Filters: leverage, quality, composite | — (parallel batch) | — | (batch commit) | 6 / 6 (subset) |
| 15 | Augmentations: noise + drift (7 ops, `n4m_aug_*`) | — (parallel batch) | — | (batch commit) | 14 / 14 (subset) |
| 16 | Augmentations: wavelength + spectral (10 ops, `n4m_aug_*`) | — (parallel batch) | — | (batch commit) | 20 / 20 (subset) |
| 17 | Augmentations: mixup + physical (10 ops, `n4m_aug_*`) | — (parallel batch) | — | (batch commit) | 25 / 25 (subset) |
| 18 | Augmentations: edge + splines + random (12 ops, `n4m_aug_*`) | — (parallel batch) | — | (batch commit) | 12 / 12 (subset) |
...
| 1.3.0 | 3 | 79 | + 22 stateful preprocessing symbols; n4m.h trimmed from 2208 to 680 lines. |
...
| 1.8.0 | 6, 13, 15-18 (parallel batch) | 400 | + 149 symbols across wavelets (27 + 4 enums), X-outlier filter (5 + 1 enum), and the new `n4m_aug_*` ABI category (117 augmenter symbols + helper opcodes). |
