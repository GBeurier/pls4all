...
n4m_split_kmeans_state_t* n4m_split_kmeans_state_new(double test_size,
    ...
    n4m_split_kmeans_state_t* s =
        (n4m_split_kmeans_state_t*)malloc(sizeof(*s));
 ...
 * already-chosen centroid. Returns N4M_OK or an error code. */
static n4m_status_t kmeans_plus_plus_init(const double* X, int64_t rows,
                                           int64_t cols, int64_t k,
                                           n4m_rng_pcg64* rng,
                                           double* centroids,
                                           double* closest_sq) {
    /* Centroid 0: uniform random sample. */
    const int64_t first = (int64_t)floor(n4m_split_rng_uniform01(rng) * (double)rows);
    ...
    for (int64_t c = 1; c < k; ++c) {
        ...
        const double u = n4m_split_rng_uniform01(rng) * total;
        double acc = 0.0;
        int64_t pick = rows - 1;
        for (int64_t i = 0; i < rows; ++i) {
            acc += closest_sq[i];
...
n4m_status_t n4m_split_kmeans_apply(const n4m_split_kmeans_state_t* state,
    ...
    st = n4m_split_result_alloc(out, actual_n_train, actual_n_test);
    if (st != N4M_OK) {
        free(centroids); free(closest_sq); free(assignments);
