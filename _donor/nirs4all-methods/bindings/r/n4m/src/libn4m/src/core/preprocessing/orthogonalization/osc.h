 ...
 * N4M_ERR_UNSUPPORTED from inverse_transform.
 *
 ...
 */
#ifndef N4M_CORE_PP_ORTHOG_OSC_H
#define N4M_CORE_PP_ORTHOG_OSC_H
...
typedef struct n4m_pp_osc_state_t n4m_pp_osc_state_t;
 ...
 * scaling. */
n4m_pp_osc_state_t* n4m_pp_osc_state_new(int32_t n_components, int scale);

/* Release a state allocated by n4m_pp_osc_state_new. NULL-safe. */
void n4m_pp_osc_state_free(n4m_pp_osc_state_t* state);

/* Returns 1 if `_fit` has been called successfully on this state, 0
 * otherwise. NULL-safe (returns 0 on NULL). */
int n4m_pp_osc_state_is_fitted(const n4m_pp_osc_state_t* state);
 ...
 *   - Returns N4M_ERR_NULL_POINTER for NULL state / X / y.
 *   - Returns N4M_ERR_INVALID_ARGUMENT for invalid shape.
 *   - Returns N4M_ERR_NUMERICAL_FAILURE when X has no Y-correlation
 *     (||X^T y|| < 1e-10), matching the reference.
 *   - Returns N4M_ERR_OUT_OF_MEMORY on allocation failure.
 *   - Returns N4M_ERR_CONVERGENCE_FAILED when extracting orthogonal
...
n4m_status_t n4m_pp_osc_state_fit(n4m_pp_osc_state_t* state,
                                   const double* X,
                                   const double* y,
 ...
 * state has not yet been fitted, N4M_ERR_SHAPE_MISMATCH if `cols` differs
 * from the column count seen at fit time. */
n4m_status_t n4m_pp_osc_state_apply(const n4m_pp_osc_state_t* state,
                                     const double* X,
                                     int64_t rows, int64_t cols,
                                     double* out);
...
#endif /* N4M_CORE_PP_ORTHOG_OSC_H */
