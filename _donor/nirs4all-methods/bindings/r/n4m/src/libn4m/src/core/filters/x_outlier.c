...
#define N4M_X_OUTLIER_PCA_DEFAULT_K        10
#define N4M_X_OUTLIER_MAHAL_DEFAULT_K      20
#define N4M_X_OUTLIER_ROBUST_MAHAL_DEFAULT_K 20  /* MinCovDet O(n^3) cap */
#define N4M_X_OUTLIER_DEFAULT_N_ESTIMATORS 100
#define N4M_X_OUTLIER_DEFAULT_MAX_SAMPLES  256

struct n4m_filter_x_outlier_state_t {
    /* Constructor params. */
    int32_t  method;
    int      use_threshold;
    double   threshold;
    ...
    n4m_pca_fit_t* maha_pca;       /* optional PCA reduction */
    ...
    n4m_mcd_t* maha_mcd;           /* robust path only */

    /* PCA methods. */
    n4m_pca_fit_t* pca_fit;

    /* Isolation Forest. */
    n4m_iforest_t* iforest;
...
n4m_filter_x_outlier_state_t* n4m_filter_x_outlier_state_new(
    ...
    if (n_estimators <= 0) n_estimators = N4M_X_OUTLIER_DEFAULT_N_ESTIMATORS;
    if (max_samples  <= 0) max_samples  = N4M_X_OUTLIER_DEFAULT_MAX_SAMPLES;
    n4m_filter_x_outlier_state_t* s =
        (n4m_filter_x_outlier_state_t*)calloc(1, sizeof(*s));
...
static void clear_fit(n4m_filter_x_outlier_state_t* s) {
    if (s->maha_pca) { n4m_pca_fit_free(s->maha_pca); free(s->maha_pca); s->maha_pca = NULL; }
    ...
    s->maha_eff_dim = 0;
    if (s->maha_mcd) { n4m_mcd_free(s->maha_mcd); s->maha_mcd = NULL; }
    if (s->pca_fit)  { n4m_pca_fit_free(s->pca_fit); free(s->pca_fit); s->pca_fit = NULL; }
    if (s->iforest)  { n4m_iforest_free(s->iforest); s->iforest = NULL; }
    if (s->lof)      { n4m_lof_free(s->lof); s->lof = NULL; }
    s->fitted = 0;
    s->fit_cols = 0;
    s->threshold_value = (double)NAN;
}
 ...
 * ========================================================================= */
static n4m_status_t mahal_distances(const double* X, int64_t rows, int64_t cols,
                                     const double* mean,
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        const n4m_status_t qst = n4m_apply_qt(M, cols, cols, tau, tmp);
        if (qst != N4M_OK) { free(diff); free(tmp); free(sol); return qst; }
        const n4m_status_t bst = n4m_back_solve_R(M, cols, cols, tmp, sol);
        if (bst != N4M_OK) { free(diff); free(tmp); free(sol); return bst; }
        double s = 0.0;
...
static n4m_status_t covariance_precision_from_qr(const double* M,
                                                  const double* tau,
                                                  int64_t cols,
                                                  double* precision) {
    double* tmp = (double*)malloc((size_t)cols * sizeof(double));
    ...
    for (int64_t rhs = 0; rhs < cols; ++rhs) {
        ...
        tmp[rhs] = 1.0;
        n4m_status_t st = n4m_apply_qt(M, cols, cols, tau, tmp);
        if (st != N4M_OK) { free(tmp); free(sol); return st; }
        st = n4m_back_solve_R(M, cols, cols, tmp, sol);
        if (st != N4M_OK) { free(tmp); free(sol); return st; }
...
static n4m_status_t fit_mahalanobis(n4m_filter_x_outlier_state_t* s,
    ...
    int64_t max_components = s->n_components > 0
        ...
        : (robust ? N4M_X_OUTLIER_ROBUST_MAHAL_DEFAULT_K
                  : N4M_X_OUTLIER_MAHAL_DEFAULT_K);
    ...
    double* scores = NULL;
    if (cols > eff_max) {
        s->maha_pca = (n4m_pca_fit_t*)calloc(1, sizeof(n4m_pca_fit_t));
        if (s->maha_pca == NULL) return N4M_ERR_OUT_OF_MEMORY;
        n4m_status_t ps = n4m_pca_fit(X, rows, cols, eff_max, 1, s->maha_pca);
        if (ps != N4M_OK) return ps;
        ...
        if (s->maha_pca->scores != NULL) {
            ...
            if (scores == NULL) return N4M_ERR_OUT_OF_MEMORY;
            const n4m_status_t ts =
                n4m_pca_transform(s->maha_pca, X, rows, scores);
            if (ts != N4M_OK) { free(scores); return ts; }
            X_used = scores;
        }
        cols_used = eff_dim;
    }
    ...
    if (robust) {
        s->maha_mcd = n4m_mcd_new();
        if (s->maha_mcd == NULL) {
            free(scores);
            return N4M_ERR_OUT_OF_MEMORY;
        }
        st = n4m_mcd_fit(s->maha_mcd, X_used, rows, cols_used, s->seed);
        if (st != N4M_OK) { free(scores); return st; }
    } else {
        ...
        st = n4m_householder_qr(s->maha_cov_qr, cols_used, cols_used,
                                  s->maha_cov_tau);
        if (st != N4M_OK) { free(scores); return st; }
...
static n4m_status_t fit_pca_residual(n4m_filter_x_outlier_state_t* s,
                                       const double* X, int64_t rows,
                                       int64_t cols) {
    int64_t want_k = (s->n_components > 0)
        ? s->n_components
        : N4M_X_OUTLIER_PCA_DEFAULT_K;
    if (want_k > rows) want_k = rows;
    if (want_k > cols) want_k = cols;
    if (want_k < 1) want_k = 1;
    s->pca_fit = (n4m_pca_fit_t*)calloc(1, sizeof(n4m_pca_fit_t));
    if (s->pca_fit == NULL) return N4M_ERR_OUT_OF_MEMORY;
    n4m_status_t st = n4m_pca_fit(X, rows, cols, want_k, 1, s->pca_fit);
    ...
    st = n4m_pca_transform(s->pca_fit, X, rows, scores);
    if (st != N4M_OK) { free(scores); free(recon); free(q); return st; }
    st = n4m_pca_inverse_transform(s->pca_fit, scores, rows, recon);
    if (st != N4M_OK) { free(scores); free(recon); free(q); return st; }
    for (int64_t r = 0; r < rows; ++r) {
        double sum = 0.0;
...
static n4m_status_t fit_pca_leverage(n4m_filter_x_outlier_state_t* s,
                                       const double* X, int64_t rows,
                                       int64_t cols) {
    int64_t want_k = (s->n_components > 0)
        ? s->n_components
    ...
    st = n4m_pca_transform(s->pca_fit, X, rows, scores);
    if (st != N4M_OK) { free(scores); free(t2); return st; }
...
static n4m_status_t fit_iforest(n4m_filter_x_outlier_state_t* s,
                                  const double* X, int64_t rows,
                                  int64_t cols) {
    s->iforest = n4m_iforest_new();
    if (s->iforest == NULL) return N4M_ERR_OUT_OF_MEMORY;
    int64_t sub_n = s->max_samples;
    if (sub_n > rows) sub_n = rows;
    n4m_status_t st = n4m_iforest_fit(s->iforest, X, rows, cols,
                                          s->n_estimators, sub_n, s->seed);
    if (st != N4M_OK) return st;
    double* scores = (double*)malloc((size_t)rows * sizeof(double));
    if (scores == NULL) return N4M_ERR_OUT_OF_MEMORY;
    st = n4m_iforest_score(s->iforest, X, rows, cols, scores);
    if (st != N4M_OK) { free(scores); return st; }
...
static n4m_status_t fit_lof(n4m_filter_x_outlier_state_t* s,
                              const double* X, int64_t rows, int64_t cols) {
    s->lof = n4m_lof_new();
    if (s->lof == NULL) return N4M_ERR_OUT_OF_MEMORY;
    int k = 20;
    if ((int64_t)k > rows - 1) k = (int)(rows - 1);
    if (k < 1) k = 1;
    n4m_status_t st = n4m_lof_fit(s->lof, X, rows, cols, k);
    if (st != N4M_OK) return st;
    double* scores = (double*)malloc((size_t)rows * sizeof(double));
    if (scores == NULL) return N4M_ERR_OUT_OF_MEMORY;
    st = n4m_lof_score(s->lof, X, rows, cols, scores);
    if (st != N4M_OK) { free(scores); return st; }
...
n4m_status_t n4m_filter_x_outlier_state_fit(
    ...
    n4m_status_t st;
    switch (state->method) {
        case N4M_CORE_X_OUTLIER_MAHALANOBIS:
            st = fit_mahalanobis(state, X, rows, cols, 0);
            break;
        case N4M_CORE_X_OUTLIER_ROBUST_MAHALANOBIS:
            st = fit_mahalanobis(state, X, rows, cols, 1);
            break;
        case N4M_CORE_X_OUTLIER_PCA_RESIDUAL:
            st = fit_pca_residual(state, X, rows, cols);
            break;
        case N4M_CORE_X_OUTLIER_PCA_LEVERAGE:
            st = fit_pca_leverage(state, X, rows, cols);
            break;
        case N4M_CORE_X_OUTLIER_ISOLATION_FOREST:
            st = fit_iforest(state, X, rows, cols);
            break;
        case N4M_CORE_X_OUTLIER_LOF:
...
static n4m_status_t apply_mahalanobis(const n4m_filter_x_outlier_state_t* s,
                                        const double* X, int64_t rows,
    ...
    if (s->maha_pca != NULL) {
        ...
        if (scores == NULL) return N4M_ERR_OUT_OF_MEMORY;
        const n4m_status_t st = n4m_pca_transform(s->maha_pca, X, rows, scores);
        if (st != N4M_OK) { free(scores); return st; }
        X_used = scores;
    ...
    if (s->maha_mcd != NULL) {
        ...
        if (mahsq == NULL) { free(scores); return N4M_ERR_OUT_OF_MEMORY; }
        st = n4m_mcd_mahalanobis_sq(s->maha_mcd, X_used, rows, cols_used, mahsq);
        if (st != N4M_OK) { free(mahsq); free(scores); return st; }
        for (int64_t i = 0; i < rows; ++i) {
...
static n4m_status_t apply_pca_residual(const n4m_filter_x_outlier_state_t* s,
                                         const double* X, int64_t rows,
    ...
    n4m_status_t st = n4m_pca_transform(s->pca_fit, X, rows, scores);
    if (st != N4M_OK) { free(scores); free(recon); return st; }
    st = n4m_pca_inverse_transform(s->pca_fit, scores, rows, recon);
    if (st != N4M_OK) { free(scores); free(recon); return st; }
    for (int64_t r = 0; r < rows; ++r) {
        double sum = 0.0;
...
static n4m_status_t apply_pca_leverage(const n4m_filter_x_outlier_state_t* s,
                                         const double* X, int64_t rows,
                                         int64_t cols, double* t2_out) {
    (void)cols;
    double* scores = (double*)malloc((size_t)rows * (size_t)s->pca_fit->k * sizeof(double));
    if (scores == NULL) return N4M_ERR_OUT_OF_MEMORY;
    const n4m_status_t st = n4m_pca_transform(s->pca_fit, X, rows, scores);
    if (st != N4M_OK) { free(scores); return st; }
    const int64_t k = s->pca_fit->k;
    for (int64_t r = 0; r < rows; ++r) {
        double sum = 0.0;
...
n4m_status_t n4m_filter_x_outlier_state_apply(
    ...
    if (state == NULL || mask_out == NULL) return N4M_ERR_NULL_POINTER;
    if (!state->fitted) return N4M_ERR_NOT_FITTED;
    if (rows < 0 || cols < 0) return N4M_ERR_INVALID_ARGUMENT;
    if (cols != state->fit_cols) return N4M_ERR_SHAPE_MISMATCH;
    if (rows == 0) {
        if (stats_out) {
    ...
    switch (state->method) {
        ...
        case N4M_CORE_X_OUTLIER_PCA_LEVERAGE:
            ...
            break;
        case N4M_CORE_X_OUTLIER_ISOLATION_FOREST:
            st = n4m_iforest_score(state->iforest, X, rows, cols, scores);
            break;
        case N4M_CORE_X_OUTLIER_LOF:
            st = n4m_lof_score(state->lof, X, rows, cols, scores);
            break;
        default:
