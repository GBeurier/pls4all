...
class EPO(StatefulOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_pp_epo_create(ctypes.byref(h), ctypes.c_int(1 if self.scale else 0)),
            "n4m_pp_epo_create",
        )
        return h
