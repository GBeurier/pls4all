...
nirs4all-methods/
├─ cpp/
│  ├─ include/n4m/   # public C ABI: n4m.h, n4m_version.h, n4m_export.h.in
│  ├─ src/
...
│  ├─ cli/n4m_cli.cpp             # tiny CLI: --version, --abi-info, --selfcheck, --bench
...
├─ parity/
│  ├─ schema/n4m_fixture_schema_v1.json
│  ├─ tolerances.md
...
├─ scripts/bump_version.sh        # single source of truth = n4m_version.h
...
1. **C++ core + stable C ABI public.** Internal C++ stays in `cpp/src/`; only `cpp/include/n4m/n4m.h` is exported. ABI symbols are `n4m_*`, gated by `N4M_API` visibility, tracked in `expected_symbols_{linux,macos,windows}.txt`.
...
3. **Opaque-handle C ABI.** Every stateful operator is a `n4m_*_state_t` opaque handle with `_create / _fit / _transform / _inverse_transform / _destroy` symbols. Stateless operators expose only `_transform` (or a static `_apply`). Error messages are stored in a per-context 4 KiB buffer.
...
6. **Dual versioning.** Project semver and ABI semver are independent. ABI major bumps only on signature change / symbol removal; minor on add; patch on bugfix-only. Single source of truth: `cpp/include/n4m/n4m_version.h`. `scripts/bump_version.sh` propagates to all manifests; `version-sync.yml` workflow gates any drift.
...
- **Per-context** error buffer + RNG state — `n4m_context_t` is **not** thread-safe; use one context per thread or guard externally.
