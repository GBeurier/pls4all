...
**Status — Phase 1 shipped, PLSCanonical, PLS-DA, OPLS, orthogonal-scores, SIMPLS, kernel, wide-kernel, SVD, power, randomized-SVD and PCR core live · API unstable until v1.0 · Not yet on PyPI / CRAN / npm.**
...
The core currently implements NIPALS, orthogonal-scores, SIMPLS, kernel, wide-kernel, SVD, power-iteration and randomized-SVD PLS regression, PLSCanonical, PLS-DA, OPLS and PCR
...
`P4A_DEFLATION_REGRESSION`, for `P4A_ALGO_OPLS` with one predictive target,
`P4A_SOLVER_NIPALS` and `P4A_DEFLATION_ORTHOGONAL`, and for `P4A_ALGO_PCR`
with `P4A_SOLVER_SVD`.
