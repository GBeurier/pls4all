...
osc <- function(X, y, n_components = 1L, scale = TRUE) {
  c4a_pp_osc_fit_transform(X, y = y, n_components = n_components,
                           scale = scale)
}
...
epo <- function(X, d, scale = TRUE) {
  c4a_pp_epo_fit_transform(X, d = d, scale = scale)
}
...
epo_inverse_transform <- function(X, d, scale = TRUE) {
  c4a_pp_epo_fit_inverse_transform(X, d = d, scale = scale)
}
...
flexible_pca <- function(X, n_components = 5.0) {
  c4a_pp_flex_pca_fit_transform(X, n_components = n_components)
}
...
flexible_svd <- function(X, n_components = 5.0) {
  c4a_pp_flex_svd_fit_transform(X, n_components = n_components)
}
...
fck_static <- function(X, kernel_size = 5L,
  ...
  c4a_pp_fck_static_transform(X, kernel_size = kernel_size,
