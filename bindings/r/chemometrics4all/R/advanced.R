...
direct_standardization <- function(source, target, X = source,
  ...
  c4a_pp_direct_standardization_fit_transform(source, target, X,
                                              fit_intercept, ridge)
}
...
robust_direct_standardization <- function(source, target, X = source,
                                          ...
                                          trim_quantile = 0.1,
                                          max_iter = 5L) {
  c4a_pp_robust_direct_standardization_fit_transform(
...
piecewise_direct_standardization <- function(source, target, X = source,
                                             ...
                                             fit_intercept = TRUE,
                                             ridge = 0.0) {
  c4a_pp_piecewise_direct_standardization_fit_transform(
...
score_augmented_projection_standardization <- function(source, target, X = source,
                                                       ...
                                                       fit_intercept = TRUE,
                                                       ridge = 1e-8) {
  c4a_pp_saps_fit_transform(source, target, X, n_components, score_weight,
                            fit_intercept, ridge)
}
...
cross_correlation_alignment <- function(X, reference = NULL,
                                        interval_size = 16L,
                                        max_shift = 3L) {
  c4a_pp_xcorr_align_fit_transform(X, reference, interval_size, max_shift)
}
...
icoshift_alignment <- function(X, reference = NULL,
  ...
  c4a_pp_icoshift_align_fit_transform(X, reference, interval_size, max_shift)
}
...
dynamic_time_warping_alignment <- function(X, reference = NULL,
                                           interval_size = 16L,
                                           max_shift = 3L) {
  c4a_pp_dtw_align_fit_transform(X, reference, interval_size, max_shift)
}
...
correlation_optimized_warping <- function(X, reference = NULL,
                                          interval_size = 16L,
                                          max_shift = 3L) {
  c4a_pp_cow_align_fit_transform(X, reference, interval_size, max_shift)
}

#' Variance feature filter.
#' @export
variance_filter <- function(X, threshold = 0.0, top_k = 0L) {
  c4a_filter_variance_fit_transform(X, threshold, top_k)
}

#' Correlation feature filter.
#' @export
correlation_filter <- function(X, y, threshold = 0.0, top_k = 0L) {
  c4a_filter_correlation_fit_transform(X, y, threshold, top_k)
}
...
interval_generator <- function(X, interval_size = 16L, step = interval_size) {
  ...
  out <- c4a_interval_generator_fit_transform(X, interval_size, step)
