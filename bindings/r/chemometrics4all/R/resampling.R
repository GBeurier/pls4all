...
.c4a_resampling_method <- function(method, caller) {
...
.c4a_kbins_strategy <- function(strategy, caller) {
...
crop_transform <- function(X, start, end) {
  c4a_pp_crop_transform(X, start = start, end = end)
}
...
resample_transform <- function(X, num_samples) {
  c4a_pp_resample_transform(X, num_samples = num_samples)
}
...
resampler <- function(X, source_wavelengths, target_wavelengths,
                      ...
                      bounds_error = FALSE,
                      extrapolate = FALSE) {
  c4a_pp_resampler_fit_transform(
    X,
    source_wavelengths = source_wavelengths,
    target_wavelengths = target_wavelengths,
    method = .c4a_resampling_method(method, "resampler"),
    crop_min = crop_min,
    crop_max = crop_max,
    use_crop = use_crop,
...
range_discretizer <- function(X, edges) {
  c4a_pp_range_disc_transform(X, edges = edges)
}
...
kbins_discretizer <- function(X, n_bins = 5L, strategy = "uniform") {
  c4a_pp_kbins_disc_fit_transform(
    X,
    n_bins = n_bins,
    strategy = .c4a_kbins_strategy(strategy, "kbins_discretizer"))
}
