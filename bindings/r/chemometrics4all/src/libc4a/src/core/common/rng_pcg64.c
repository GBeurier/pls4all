...
#if C4A_PCG64_HAS_NATIVE_128

static c4a_pcg128_t c4a_pcg128_make(uint64_t high, uint64_t low) {
    return (((c4a_pcg128_t)high) << 64) | (c4a_pcg128_t)low;
}

static c4a_pcg128_t c4a_pcg128_zero(void) { return (c4a_pcg128_t)0; }
static c4a_pcg128_t c4a_pcg128_one(void)  { return (c4a_pcg128_t)1; }

static c4a_pcg128_t c4a_pcg128_add(c4a_pcg128_t a, c4a_pcg128_t b) {
    return a + b;
}

static c4a_pcg128_t c4a_pcg128_mul(c4a_pcg128_t a, c4a_pcg128_t b) {
    return a * b;
}

static int c4a_pcg128_nonzero(c4a_pcg128_t v) { return v != 0; }
static int c4a_pcg128_low_bit_set(c4a_pcg128_t v) { return (int)(v & 1u); }
static c4a_pcg128_t c4a_pcg128_shr1(c4a_pcg128_t v) { return v >> 1; }

static uint64_t c4a_pcg128_high(c4a_pcg128_t v) {
    return (uint64_t)(v >> 64);
}

static uint64_t c4a_pcg128_low(c4a_pcg128_t v) {
    return (uint64_t)(v & 0xFFFFFFFFFFFFFFFFULL);
}

#else  /* !C4A_PCG64_HAS_NATIVE_128 */
...
static c4a_pcg128_t c4a_pcg128_zero(void) { return c4a_pcg128_make(0, 0); }
static c4a_pcg128_t c4a_pcg128_one(void)  { return c4a_pcg128_make(0, 1); }
...
static void c4a_pcg_mul64(uint64_t x, uint64_t y,
...
static c4a_pcg128_t c4a_pcg128_mul(c4a_pcg128_t a, c4a_pcg128_t b) {
    ...
    c4a_pcg128_t r;
    c4a_pcg_mul64(a.low, b.low, &r.high, &r.low);
    r.high += h1;
    return r;
}

static int c4a_pcg128_nonzero(c4a_pcg128_t v) {
    return (v.high != 0) || (v.low != 0);
}
static int c4a_pcg128_low_bit_set(c4a_pcg128_t v) { return (int)(v.low & 1u); }
static c4a_pcg128_t c4a_pcg128_shr1(c4a_pcg128_t v) {
    c4a_pcg128_t r;
    r.low  = (v.low >> 1) | (v.high << 63);
    r.high = v.high >> 1;
    return r;
}
static uint64_t c4a_pcg128_high(c4a_pcg128_t v) { return v.high; }
static uint64_t c4a_pcg128_low(c4a_pcg128_t v)  { return v.low; }

#endif  /* C4A_PCG64_HAS_NATIVE_128 */
...
#define C4A_PCG_MULT_HIGH 2549297995355413924ULL
#define C4A_PCG_MULT_LOW  4865540595714422341ULL

static c4a_pcg128_t c4a_pcg_default_multiplier(void) {
    return c4a_pcg128_make(C4A_PCG_MULT_HIGH, C4A_PCG_MULT_LOW);
}
...
static uint64_t c4a_rotr64(uint64_t value, unsigned int rot) {
...
#define C4A_SS_POOL_SIZE   4u
#define C4A_SS_INIT_A      0x43b0d7e5u
#define C4A_SS_MULT_A      0x931e8875u
#define C4A_SS_INIT_B      0x8b51f9ddu
#define C4A_SS_MULT_B      0x58f38dedu
#define C4A_SS_MIX_MULT_L  0xca01f9ddu
#define C4A_SS_MIX_MULT_R  0x4973f715u
#define C4A_SS_XSHIFT      16u

static uint32_t c4a_ss_hashmix(uint32_t value, uint32_t* hash_const) {
    /* mirrors `bit_generator.pyx` :: hashmix */
    value         ^= hash_const[0];
    hash_const[0] *= C4A_SS_MULT_A;
    value         *= hash_const[0];
    value         ^= value >> C4A_SS_XSHIFT;
    return value;
}

static uint32_t c4a_ss_mix(uint32_t x, uint32_t y) {
    /* mirrors `bit_generator.pyx` :: mix — unsigned 32-bit wrap is intended. */
    uint32_t result = C4A_SS_MIX_MULT_L * x - C4A_SS_MIX_MULT_R * y;
    result ^= result >> C4A_SS_XSHIFT;
    return result;
}
...
static void c4a_ss_mix_entropy(uint32_t pool[C4A_SS_POOL_SIZE],
                                const uint32_t* entropy,
                                size_t entropy_len) {
    uint32_t hash_const = C4A_SS_INIT_A;
    size_t i;
    ...
    for (i = 0; i < C4A_SS_POOL_SIZE; ++i) {
        uint32_t v = (i < entropy_len) ? entropy[i] : 0u;
        pool[i] = c4a_ss_hashmix(v, &hash_const);
    }

    /* Stage 2 — cross-mix all pool words so later words can affect earlier
     * ones. O(pool_size^2) = 16 iterations for the default pool. */
    for (size_t src = 0; src < C4A_SS_POOL_SIZE; ++src) {
        for (size_t dst = 0; dst < C4A_SS_POOL_SIZE; ++dst) {
            if (src == dst) continue;
            pool[dst] = c4a_ss_mix(pool[dst],
                                    c4a_ss_hashmix(pool[src], &hash_const));
        }
    }
    ...
    for (size_t src = C4A_SS_POOL_SIZE; src < entropy_len; ++src) {
        for (size_t dst = 0; dst < C4A_SS_POOL_SIZE; ++dst) {
            pool[dst] = c4a_ss_mix(pool[dst],
                                    c4a_ss_hashmix(entropy[src], &hash_const));
        }
    }
}
...
static void c4a_ss_generate_state_u64(const uint32_t pool[C4A_SS_POOL_SIZE],
                                       size_t n_uint64,
                                       uint64_t* out_u64) {
    uint32_t hash_const = C4A_SS_INIT_B;
    size_t pool_idx = 0;
    ...
    for (size_t i = 0; i < n_words; ++i) {
        ...
        v          ^= hash_const;
        hash_const *= C4A_SS_MULT_B;
        v          *= hash_const;
        v          ^= v >> C4A_SS_XSHIFT;
        words[i]   = v;
        pool_idx = (pool_idx + 1u) % C4A_SS_POOL_SIZE;
    }
...
static void c4a_pcg_expand_seed(uint64_t seed, uint64_t out_u64_4[4]) {
    ...
    uint32_t pool[C4A_SS_POOL_SIZE] = {0u, 0u, 0u, 0u};
    c4a_ss_mix_entropy(pool, entropy, entropy_len);
    c4a_ss_generate_state_u64(pool, 4u, out_u64_4);
}
...
static void c4a_pcg_step(c4a_rng_pcg64* rng) {
    rng->state = c4a_pcg128_add(c4a_pcg128_mul(rng->state,
                                                c4a_pcg_default_multiplier()),
                                  rng->inc);
}

static uint64_t c4a_pcg_output_xsl_rr(c4a_pcg128_t state) {
    /* high ^ low, rotate right by (state >> 122) = high >> 58. */
    uint64_t hi = c4a_pcg128_high(state);
    uint64_t lo = c4a_pcg128_low(state);
    return c4a_rotr64(hi ^ lo, (unsigned int)(hi >> 58));
}

static c4a_pcg128_t c4a_pcg_advance_lcg(c4a_pcg128_t state,
                                          c4a_pcg128_t delta,
                                          c4a_pcg128_t cur_mult,
                                          c4a_pcg128_t cur_plus) {
    /* Brown 1994 "Random Number Generation with Arbitrary Stride" —
     * O(log delta) fast-exponentiation walk. */
    c4a_pcg128_t acc_mult = c4a_pcg128_one();
    c4a_pcg128_t acc_plus = c4a_pcg128_zero();
    while (c4a_pcg128_nonzero(delta)) {
        if (c4a_pcg128_low_bit_set(delta)) {
            acc_mult = c4a_pcg128_mul(acc_mult, cur_mult);
            acc_plus = c4a_pcg128_add(c4a_pcg128_mul(acc_plus, cur_mult),
                                       cur_plus);
        }
        cur_plus = c4a_pcg128_mul(c4a_pcg128_add(cur_mult, c4a_pcg128_one()),
                                    cur_plus);
        cur_mult = c4a_pcg128_mul(cur_mult, cur_mult);
        delta    = c4a_pcg128_shr1(delta);
    }
    return c4a_pcg128_add(c4a_pcg128_mul(acc_mult, state), acc_plus);
}
...
void c4a_pcg64_engine_seed(c4a_rng_pcg64* rng, uint64_t seed) {
    uint64_t expanded[4];
    c4a_pcg_expand_seed(seed, expanded);
    ...
    c4a_pcg128_t initstate = c4a_pcg128_make(expanded[0], expanded[1]);
    c4a_pcg128_t initseq   = c4a_pcg128_make(expanded[2], expanded[3]);

    /* pcg_setseq_128_srandom_r: state=0; inc=(initseq<<1)|1; step;
     *                          state+=initstate; step. */
    rng->state = c4a_pcg128_zero();
    /* inc = (initseq << 1) | 1: shift high/low by 1 with carry, then OR 1. */
    {
        uint64_t seq_hi = c4a_pcg128_high(initseq);
        uint64_t seq_lo = c4a_pcg128_low(initseq);
        uint64_t inc_hi = (seq_hi << 1) | (seq_lo >> 63);
        uint64_t inc_lo = (seq_lo << 1) | 1u;
        rng->inc = c4a_pcg128_make(inc_hi, inc_lo);
    }
    c4a_pcg_step(rng);
    rng->state = c4a_pcg128_add(rng->state, initstate);
    c4a_pcg_step(rng);
}
...
uint64_t c4a_pcg64_engine_next_uint64(c4a_rng_pcg64* rng) {
    ...
    c4a_pcg_step(rng);
    return c4a_pcg_output_xsl_rr(rng->state);
}
...
void c4a_pcg64_engine_advance(c4a_rng_pcg64* rng,
                            uint64_t delta_high, uint64_t delta_low) {
    c4a_pcg128_t delta = c4a_pcg128_make(delta_high, delta_low);
    rng->state = c4a_pcg_advance_lcg(rng->state, delta,
                                       c4a_pcg_default_multiplier(),
                                       rng->inc);
}
