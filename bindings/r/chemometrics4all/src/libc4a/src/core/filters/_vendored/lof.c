...
c4a_lof_t* c4a_lof_new(void) {
    c4a_lof_t* l = (c4a_lof_t*)calloc(1, sizeof(c4a_lof_t));
    return l;
}
...
c4a_status_t c4a_lof_score(const c4a_lof_t* lof,
                            ...
                            int64_t cols,
                            double* scores) {
    if (lof == NULL || scores == NULL) return C4A_ERR_NULL_POINTER;
    if (rows_new < 0 || cols < 0) return C4A_ERR_INVALID_ARGUMENT;
    if (rows_new == 0) return C4A_OK;
    if (X_new == NULL) return C4A_ERR_NULL_POINTER;
    if (cols != lof->cols) return C4A_ERR_SHAPE_MISMATCH;
    if (lof->lrd == NULL) return C4A_ERR_NOT_FITTED;
    const int k = lof->k;
