 ...
 * thresholds are exact (no float tolerance) so the c4a output matches the
...
struct c4a_filter_quality_state_t {
...
c4a_filter_quality_state_t* c4a_filter_quality_state_new(
    ...
    c4a_filter_quality_state_t* s =
        (c4a_filter_quality_state_t*)malloc(sizeof(*s));
...
static int row_passes(const c4a_filter_quality_state_t* state,
