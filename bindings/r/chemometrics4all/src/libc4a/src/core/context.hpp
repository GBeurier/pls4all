...
//
// Internal Context — the C++ object pointed to by the opaque c4a_context_s
...
//     ABI level (see c4a.h §6). Internally that means we DO NOT need locks
...
#  define C4A_PRINTF_LIKE(fmt_index, first_arg) __attribute__((format(printf, fmt_index, first_arg)))
#else
#  define C4A_PRINTF_LIKE(fmt_index, first_arg)
#endif
...
class Context {
  public:
    ...
    // Returns C4A_OK on success, C4A_ERR_INVALID_ARGUMENT for an
    // out-of-range backend value, or C4A_ERR_BACKEND_UNAVAILABLE if the
    // backend is not compiled into this build.
    c4a_status_t set_backend(c4a_backend_t backend) noexcept;
    [[nodiscard]] c4a_backend_t backend() const noexcept { return backend_; }
    ...
    void set_errorf(const char* fmt, ...) noexcept C4A_PRINTF_LIKE(2, 3);

  private:
    std::uint64_t seed_;
    c4a_backend_t backend_;
    std::int32_t  num_threads_;
    void*         user_data_;
    char          error_buf_[C4A_ERROR_BUFFER_BYTES];
};

}  // namespace chemometrics4all::core

// Opaque struct alias. c4a.h declares `struct c4a_context_s; typedef … c4a_context_t;`;
// we attach the C++ Context implementation directly to that tag.
struct c4a_context_s : public ::chemometrics4all::core::Context {};

#undef C4A_PRINTF_LIKE
