...
static c4a_status_t check_inputs(const double* y_true, const double* y_pred,
...
c4a_status_t c4a_metric_rmse_impl(const double* y_true, const double* y_pred,
    ...
    const c4a_status_t s = check_inputs(y_true, y_pred, n, out);
...
c4a_status_t c4a_metric_rpiq_impl(const double* y_true, const double* y_pred,
    ...
    if (sorted == NULL) return C4A_ERR_OUT_OF_MEMORY;
