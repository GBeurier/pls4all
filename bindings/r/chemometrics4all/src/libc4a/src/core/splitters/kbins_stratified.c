 ...
 *     RandomState(MT19937), not c4a's PCG64 engine.
 */
...
#define C4A_MT19937_N 624
#define C4A_MT19937_M 397

typedef struct c4a_kbins_mt19937_t {
    uint32_t state[C4A_MT19937_N];
    int32_t pos;
} c4a_kbins_mt19937_t;

static void mt19937_seed(c4a_kbins_mt19937_t* rng, uint32_t seed) {
    rng->state[0] = seed;
    for (int32_t i = 1; i < C4A_MT19937_N; ++i) {
        const uint32_t prev = rng->state[i - 1];
        rng->state[i] = (uint32_t)(1812433253UL * (prev ^ (prev >> 30)) +
                                   (uint32_t)i);
    }
    rng->pos = C4A_MT19937_N;
}

static void mt19937_twist(c4a_kbins_mt19937_t* rng) {
    static const uint32_t mag01[2] = {0x0U, 0x9908b0dfU};
    int32_t kk;
    for (kk = 0; kk < C4A_MT19937_N - C4A_MT19937_M; ++kk) {
        const uint32_t y = (rng->state[kk] & 0x80000000U) |
                           (rng->state[kk + 1] & 0x7fffffffU);
        rng->state[kk] = rng->state[kk + C4A_MT19937_M] ^
                         (y >> 1) ^ mag01[y & 0x1U];
    }
    for (; kk < C4A_MT19937_N - 1; ++kk) {
        const uint32_t y = (rng->state[kk] & 0x80000000U) |
                           (rng->state[kk + 1] & 0x7fffffffU);
        rng->state[kk] = rng->state[kk + (C4A_MT19937_M - C4A_MT19937_N)] ^
                         (y >> 1) ^ mag01[y & 0x1U];
    }
    {
        const uint32_t y = (rng->state[C4A_MT19937_N - 1] & 0x80000000U) |
                           (rng->state[0] & 0x7fffffffU);
        rng->state[C4A_MT19937_N - 1] = rng->state[C4A_MT19937_M - 1] ^
                                        (y >> 1) ^ mag01[y & 0x1U];
    }
    rng->pos = 0;
}

static uint32_t mt19937_next_uint32(c4a_kbins_mt19937_t* rng) {
    if (rng->pos >= C4A_MT19937_N) {
        mt19937_twist(rng);
    }
...
static uint32_t mt19937_interval(c4a_kbins_mt19937_t* rng, uint32_t max) {
    uint32_t mask = max;
    mask |= mask >> 1;
    mask |= mask >> 2;
    mask |= mask >> 4;
...
static void mt19937_shuffle_i64(c4a_kbins_mt19937_t* rng,
...
static void mt19937_permutation_i64(c4a_kbins_mt19937_t* rng,
...
static c4a_status_t approximate_mode(const int64_t* class_counts,
                                      int32_t n_classes,
                                      int64_t n_draws,
                                      c4a_kbins_mt19937_t* rng,
                                      int64_t* out) {
    int64_t total = 0;
