...
size_t dtype_size(c4a_dtype_t dtype) noexcept {
    switch (dtype) {
...
bool dtype_is_valid(c4a_dtype_t dtype) noexcept {
    return dtype == C4A_DTYPE_F64 || dtype == C4A_DTYPE_F32
        || dtype == C4A_DTYPE_I32 || dtype == C4A_DTYPE_I64;
}

c4a_status_t validate_nonnull_view(const c4a_matrix_view_t& v) noexcept {
