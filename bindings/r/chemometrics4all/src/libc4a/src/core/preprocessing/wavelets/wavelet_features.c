...
c4a_pp_wavelet_features_state_t* c4a_pp_wavelet_features_state_new(
    ...
    c4a_wavelet_mode_t   mode,
    int32_t              max_level) {
    return c4a_pp_wavelet_features_state_new_ex(
        family, mode, max_level, C4A_PP_WAVELET_FEATURES_ENTROPY_ENERGY);
}
...
c4a_pp_wavelet_features_state_t* c4a_pp_wavelet_features_state_new_ex(
    ...
    c4a_pp_wavelet_features_entropy_t entropy_mode) {
    if (max_level < 0 ||
        (entropy_mode != C4A_PP_WAVELET_FEATURES_ENTROPY_ENERGY &&
         entropy_mode != C4A_PP_WAVELET_FEATURES_ENTROPY_HISTOGRAM)) {
        return NULL;
    }
