...
struct c4a_aug_local_warp_state_t {
...
c4a_aug_local_warp_state_t* c4a_aug_local_warp_state_new(
    ...
    c4a_aug_local_warp_state_t* s =
        (c4a_aug_local_warp_state_t*)malloc(sizeof(*s));
    ...
    for (int64_t i = 0; i < rows; ++i) {
        for (int32_t c = 0; c < k; ++c) {
            ctrl_y[(size_t)i * (size_t)k + (size_t)c] =
                c4a_aug_uniform(state->rng, lo, hi);
        }
    }
    for (int64_t i = 0; i < rows; ++i) {
        const double* row_ctrl_y = ctrl_y + (size_t)i * (size_t)k;
        /* shift[j] = np.interp(lambdas[j], ctrl_x, ctrl_y_i). */
        c4a_aug_interp_linear(lambdas, cols, ctrl_x, row_ctrl_y, k, shift);
        for (int64_t j = 0; j < cols; ++j) {
