...
struct c4a_aug_unsharp_mask_state_t {
    c4a_rng_pcg64* rng;
    double         amount_lo;
    double         amount_hi;
    double         sigma;
...
c4a_aug_unsharp_mask_state_t* c4a_aug_unsharp_mask_state_new(
    ...
    c4a_aug_unsharp_mask_state_t* s =
        (c4a_aug_unsharp_mask_state_t*)malloc(sizeof(*s));
    ...
    c4a_aug_gauss_kernel_build(sigma, w, s->kernel);
    return s;
}
...
c4a_status_t c4a_aug_unsharp_mask_state_apply(
    ...
    if (smoothed == NULL) return C4A_ERR_OUT_OF_MEMORY;
    ...
    for (int64_t i = 0; i < rows; ++i) {
        amounts[i] = c4a_aug_uniform(state->rng,
