...
struct c4a_aug_band_perturb_state_t {
...
c4a_aug_band_perturb_state_t* c4a_aug_band_perturb_state_new(
    ...
    c4a_aug_band_perturb_state_t* s =
        (c4a_aug_band_perturb_state_t*)malloc(sizeof(*s));
...
void c4a_aug_band_perturb_state_free(c4a_aug_band_perturb_state_t* state) {
    free(state);
}
...
c4a_status_t c4a_aug_band_perturb_state_apply(
    ...
    if (rows == 0 || cols == 0 || state->n_bands == 0) return C4A_OK;
    ...
    /* Block 3: gains. */
    for (size_t k = 0; k < total; ++k) {
        gains[k] = c4a_aug_uniform(state->rng, state->gain_lo, state->gain_hi);
    }
    /* Block 4: offsets. */
    for (size_t k = 0; k < total; ++k) {
        offsets[k] = c4a_aug_uniform(state->rng,
