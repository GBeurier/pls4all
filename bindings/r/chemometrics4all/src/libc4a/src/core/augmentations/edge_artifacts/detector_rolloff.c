...
struct c4a_aug_detector_rolloff_state_t {
...
c4a_aug_detector_rolloff_state_t* c4a_aug_detector_rolloff_state_new(
    ...
    c4a_aug_detector_rolloff_state_t* s = (c4a_aug_detector_rolloff_state_t*)
        calloc(1, sizeof(*s));
...
c4a_status_t c4a_aug_detector_rolloff_state_apply(
    ...
    if (sens == NULL) return C4A_ERR_OUT_OF_MEMORY;
    ...
    if (state->include_baseline_distortion) {
        ...
        if (baseline == NULL) { free(sens); return C4A_ERR_OUT_OF_MEMORY; }
        for (int64_t j = 0; j < cols; ++j) {
    ...
    for (int64_t i = 0; i < rows; ++i) {
        ...
        if (state->noise_amplification > 0.0) {
            c4a_pcg64_engine_standard_normal_fill(rng, noise_buf,
                                                    (size_t)cols);
            for (int64_t j = 0; j < cols; ++j) {
