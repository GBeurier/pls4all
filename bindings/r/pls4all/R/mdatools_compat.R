...
#' @return A `n4m_mdatools_pls` object.
#' @export
pls_mdatools <- function(x, y,
    ...
    if (!is.null(x.test)) {
        ...
        if (!is.null(y.test)) {
            Ytest <- .n4m_as_response_matrix(y.test)
            residuals <- Ytest - .n4m_as_response_matrix(ypred)
            obj$testres$residuals <- .n4m_drop_response_dim(residuals)
...
#' @export
predict.n4m_mdatools_pls <- function(object, newdata = NULL,
...
#' @export
print.n4m_mdatools_pls <- function(x, ...) {
    cat("n4m mdatools-compatible PLS model\n")
    cat("Method:      ", x$method, "\n", sep = "")
