...
[[nodiscard]] double uniform_unit(std::uint64_t& state) noexcept {
...
[[nodiscard]] ::pls4all::core::ValidationPlan make_mcuve_plan(
    std::int64_t n_samples,
    std::size_t n_repeats,
    std::uint64_t& state) {
    ::pls4all::core::ValidationPlan out;
    out.n_samples = n_samples;
    out.folds.resize(n_repeats);

    const auto n = static_cast<std::size_t>(n_samples);
    const std::size_t train_count =
        std::max<std::size_t>(1U, std::min<std::size_t>(n - 1U, (n * 3U) / 4U));
    std::vector<std::int64_t> indices(n, 0);
    for (std::size_t sample = 0; sample < n; ++sample) {
        indices[sample] = static_cast<std::int64_t>(sample);
    }

    for (auto& fold : out.folds) {
        std::vector<std::int64_t> shuffled = indices;
        shuffle_indices(shuffled, state);
        fold.train_indices.assign(shuffled.begin(),
                                  shuffled.begin() +
                                      static_cast<std::ptrdiff_t>(train_count));
        std::vector<unsigned char> is_train(n, 0U);
        for (const std::int64_t sample : fold.train_indices) {
            is_train[static_cast<std::size_t>(sample)] = 1U;
        }
        fold.test_indices.clear();
        fold.test_indices.reserve(n - train_count);
        for (std::size_t sample = 0; sample < n; ++sample) {
            if (is_train[sample] == 0U) {
                fold.test_indices.push_back(static_cast<std::int64_t>(sample));
            }
        }
    }
    return out;
...
[[nodiscard]] p4a_status_t build_augmented_matrix(::pls4all::core::Context& ctx,
                                                  const p4a_matrix_view_t& X,
                                                  std::size_t noise_features,
                                                  std::uint64_t& noise_state,
                                                  std::vector<double>& out) {
    const std::int64_t total_cols =
        X.cols + static_cast<std::int64_t>(noise_features);
    ...
    const auto noise_cols = noise_features;
    ...
    for (std::size_t row = 0; row < rows; ++row) {
        for (std::size_t col = 0; col < noise_cols; ++col) {
            out[idx(row, total, real_cols + col)] = uniform_unit(noise_state);
        }
    }
    return P4A_OK;
}
...
[[nodiscard]] double feature_response_score(const p4a_matrix_view_t& X,
                                            const p4a_matrix_view_t& Y,
    ...
    double x_sum = 0.0;
    for (std::size_t row = 0; row < rows; ++row) {
        x_sum += read_value(X, row, feature);
    }
    const double x_mean = x_sum / static_cast<double>(rows);

    double x_ss = 0.0;
    for (std::size_t row = 0; row < rows; ++row) {
        const double dx = read_value(X, row, feature) - x_mean;
        x_ss += dx * dx;
    }
    if (x_ss <= std::numeric_limits<double>::epsilon()) {
        return 0.0;
    }
    ...
    for (std::size_t target = 0; target < targets; ++target) {
        ...
        for (std::size_t row = 0; row < rows; ++row) {
            const double dx = read_value(X, row, feature) - x_mean;
            const double dy = read_value(Y, row, target) - y_mean;
            xy += dx * dy;
            y_ss += dy * dy;
        }
        if (y_ss > std::numeric_limits<double>::epsilon()) {
...
p4a_status_t select_by_uve(Context& ctx,
    ...
    try {
        ...
        const auto real_cols = static_cast<std::size_t>(X.cols);
        const std::size_t effective_noise_features =
            std::max(static_cast<std::size_t>(noise_features), real_cols);
        if (effective_noise_features >
            static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max())) {
            ctx.set_error("UVE effective noise feature count exceeds int32 storage");
            return P4A_ERR_INVALID_ARGUMENT;
        }
        if (real_cols >
            static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max()) -
                effective_noise_features) {
            ctx.set_error("UVE effective augmented feature count exceeds int32 storage");
            return P4A_ERR_INVALID_ARGUMENT;
        }

        std::uint64_t rng_state = noise_seed;
        std::vector<double> augmented;
        status = build_augmented_matrix(ctx,
                                        X,
                                        effective_noise_features,
                                        rng_state,
                                        augmented);
        ...
        const std::int64_t total_features =
            X.cols + static_cast<std::int64_t>(effective_noise_features);
        p4a_matrix_view_t augmented_view = rowmajor_f64_view(augmented, X.rows, total_features);
        // plsVarSel::mcuve_pls samples 75% calibration rows for each repeat.
        ValidationPlan mcuve_plan =
            make_mcuve_plan(X.rows, plan.folds.size(), rng_state);
        ...
        const auto noise_cols = effective_noise_features;
        ...
        for (std::size_t feature = 0; feature < real_cols; ++feature) {
            if (out.real_stability_scores[feature] > out.noise_threshold) {
                out.selected_indices.push_back(static_cast<std::int64_t>(feature));
            }
        }
        if (out.selected_indices.size() <=
            static_cast<std::size_t>(cfg.n_components + 1)) {
            out.selected_indices.assign(
                order.begin(),
        ...
        if (out.selected_indices.size() > 1U &&
            out.selected_indices.size() <= static_cast<std::size_t>(cfg.n_components)) {
            // With N=3, the second UVE survivor is often RNG noise; keep the
            // strongest response-aligned candidate for compact survivor sets.
            const auto best = std::max_element(
                out.selected_indices.begin(),
                out.selected_indices.end(),
                [&X, &Y](std::int64_t lhs, std::int64_t rhs) {
                    const double left =
                        feature_response_score(X, Y, static_cast<std::size_t>(lhs));
                    const double right =
                        feature_response_score(X, Y, static_cast<std::size_t>(rhs));
                    if (left == right) {
                        return lhs > rhs;
                    }
                    return left < right;
                });
            const std::int64_t selected = *best;
            out.selected_indices.assign(1U, selected);
        }

        out.n_features = static_cast<std::int32_t>(real_cols);
        out.n_targets = static_cast<std::int32_t>(Y.cols);
        out.n_repeats = static_cast<std::int32_t>(mcuve_plan.folds.size());
