...
test_that("pls() vs n4m_fit produce identical predictions", {
    set.seed(1)
    n <- 60; p <- 8
    X <- matrix(rnorm(n * p), n, p)
    ...
    fit_t1 <- n4m_fit(X_t1, y, algo = "pls_nipals", n_components = 3L)
    preds_t1 <- as.vector(n4m_predict(fit_t1, X_t1))
...
test_that("predict() rejects newdata missing model columns", {
    ...
    # `predict.n4m_fit` delegates to `stats::model.frame` to apply
    ...
    # rather than a n4m-specific feature-count error. This is the
...
test_that("summary() and print() produce structured output", {
    ...
    fit <- pls(y ~ ., data = df, ncomp = 2L)
    out <- summary(fit)
    expect_s3_class(out, "summary.n4m_fit")
