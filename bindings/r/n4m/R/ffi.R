...
# performs minimal validation, and dispatches into the nirs4all-methods C
...
#     storage layout via `n4m_matrix_view_init_colmajor` so no transpose
#     is necessary.
#   - Outputs are returned as newly-allocated R matrices in column-major
#     layout matching libn4m's row-major outputs row-for-row (see
#     `r_methods.c` for the copy logic).
#   - On error the C side calls `Rf_error()` with the libn4m error string,
#     so callers can `tryCatch` if needed.

.n4m_check_matrix <- function(X, name = "X") {
...
.n4m_check_numeric_vector <- function(x, name) {
  if (!is.numeric(x)) {
...
#' @export
n4m_pp_snv_transform <- function(X, with_mean = TRUE, with_std = TRUE,
                                 ddof = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_snv_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_lsnv_transform <- function(X, window = 11L, pad_mode = 0L,
                                  constant_value = 0.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_lsnv_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_rnv_transform <- function(X, with_center = TRUE, with_scale = TRUE,
                                 k = 1.4826) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_rnv_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_area_transform <- function(X, method = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_area_transform, X, as.integer(method)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_normalize_transform <- function(X, feature_min = -1.0,
                                       feature_max = 1.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_normalize_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_simple_scale_transform <- function(X) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_simple_scale_transform, X)
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_msc_fit_transform <- function(X_fit, X) {
  X_fit <- .n4m_check_matrix(X_fit, "X_fit")
  X <- .n4m_check_matrix(X, "X")
  if (ncol(X_fit) != ncol(X)) {
    stop("X_fit and X must have the same number of columns", call. = FALSE)
  }
  .Call(C_n4m_pp_msc_fit_transform, X_fit, X)
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_msc_fit_inverse_transform <- function(X_fit, X) {
  X_fit <- .n4m_check_matrix(X_fit, "X_fit")
  X <- .n4m_check_matrix(X, "X")
  if (ncol(X_fit) != ncol(X)) {
    stop("X_fit and X must have the same number of columns", call. = FALSE)
  }
  stop("n4m_pp_msc_fit_inverse_transform cannot reconstruct conventional ",
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_emsc_fit_transform <- function(X_fit, X, degree = 1L) {
  X_fit <- .n4m_check_matrix(X_fit, "X_fit")
  X <- .n4m_check_matrix(X, "X")
  if (ncol(X_fit) != ncol(X)) {
    stop("X_fit and X must have the same number of columns", call. = FALSE)
  }
  .Call(C_n4m_pp_emsc_fit_transform, X_fit, X, as.integer(degree)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_baseline_fit_transform <- function(X_fit, X) {
  X_fit <- .n4m_check_matrix(X_fit, "X_fit")
  X <- .n4m_check_matrix(X, "X")
  if (ncol(X_fit) != ncol(X)) {
    stop("X_fit and X must have the same number of columns", call. = FALSE)
  }
  .Call(C_n4m_pp_baseline_fit_transform, X_fit, X)
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_baseline_fit_inverse_transform <- function(X_fit, X) {
  X_fit <- .n4m_check_matrix(X_fit, "X_fit")
  X <- .n4m_check_matrix(X, "X")
  if (ncol(X_fit) != ncol(X)) {
    stop("X_fit and X must have the same number of columns", call. = FALSE)
  }
  .Call(C_n4m_pp_baseline_fit_inverse_transform, X_fit, X)
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_savgol_transform <- function(X, window_length, polyorder, deriv = 0L,
                                    delta = 1.0, mode = 0L, cval = 0.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_savgol_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_first_derivative_transform <- function(X, delta = 1.0, edge_order = 2L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_first_derivative_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_second_derivative_transform <- function(X, delta = 1.0, edge_order = 2L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_second_derivative_transform, X,
        as.double(delta)[1L], as.integer(edge_order)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_norris_williams_transform <- function(X, segment = 5L, gap = 5L,
                                             derivative_order = 1L,
                                             delta = 1.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_norris_williams_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_gaussian_transform <- function(X, sigma = 1.0, order = 0L,
                                      mode = 0L, cval = 0.0,
                                      truncate = 4.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_gaussian_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_to_absorbance_transform <- function(X, is_percent = FALSE,
                                           epsilon = 1e-10,
                                           clip_negative = TRUE) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_to_absorbance_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_from_absorbance_transform <- function(X, is_percent = FALSE) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_from_absorbance_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_pct_to_frac_transform <- function(X) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_pct_to_frac_transform, X)
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_frac_to_pct_transform <- function(X) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_frac_to_pct_transform, X)
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_kubelka_munk_transform <- function(X, is_percent = FALSE,
                                          epsilon = 1e-10) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_kubelka_munk_transform, X,
        as.logical(is_percent)[1L], as.double(epsilon)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_detrend_transform <- function(X, polyorder = 1L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_detrend_transform, X, as.integer(polyorder)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_asls_transform <- function(X, lam = 1e6, p = 1e-2,
                                  max_iter = 50L, tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_asls_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_airpls_transform <- function(X, lam = 1e6,
                                    max_iter = 50L, tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_airpls_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_arpls_transform <- function(X, lam = 1e5,
                                   max_iter = 50L, tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_arpls_transform, X,
        as.double(lam)[1L], as.integer(max_iter)[1L], as.double(tol)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_modpoly_transform <- function(X, polyorder = 2L,
                                     max_iter = 250L, tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_modpoly_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_imodpoly_transform <- function(X, polyorder = 2L,
                                      max_iter = 250L, tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_imodpoly_transform, X,
        as.integer(polyorder)[1L], as.integer(max_iter)[1L],
        as.double(tol)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_snip_transform <- function(X, max_half_window = 20L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_snip_transform, X, as.integer(max_half_window)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_rolling_ball_transform <- function(X, half_window = 20L,
                                          smooth_half_window = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_rolling_ball_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_iasls_transform <- function(X, lam = 1e6, p = 1e-2,
                                   lam_1 = 1e-4, polyorder = 2L,
                                   diff_order = 2L, max_iter = 50L,
                                   tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_iasls_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_beads_transform <- function(X, lam_0 = 100.0, lam_1 = 0.5,
                                   lam_2 = 0.5, max_iter = 50L,
                                   tol = 1e-3) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_beads_transform, X,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_wavelet_denoise_transform <- function(X, family, boundary, level,
                                             threshold_mode, noise_estimator) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_wavelet_denoise_transform, X,
...
#' @export
n4m_pp_wavelet_transform <- function(X, family = 0L, boundary = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_wavelet_transform, X,
        as.integer(family)[1L], as.integer(boundary)[1L])
}
...
#' @return numeric matrix containing approximation/detail coefficients.
#' @export
n4m_pp_haar_transform <- function(X) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_haar_transform, X)
}
...
#' @export
n4m_pp_wavelet_features_transform <- function(X, family = 0L,
                                              boundary = 0L,
                                              max_level = 2L,
                                              entropy = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_wavelet_features_transform, X,
        as.integer(family)[1L], as.integer(boundary)[1L],
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_osc_fit_transform <- function(X, y, n_components = 1L,
                                     scale = TRUE) {
  X <- .n4m_check_matrix(X, "X")
  y <- .n4m_check_numeric_vector(y, "y")
  if (length(y) != nrow(X)) {
    stop("y length must match nrow(X)", call. = FALSE)
  }
  .Call(C_n4m_pp_osc_fit_transform, X, y,
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_epo_fit_transform <- function(X, d, scale = TRUE) {
  X <- .n4m_check_matrix(X, "X")
  d <- .n4m_check_numeric_vector(d, "d")
  if (length(d) != nrow(X)) {
    stop("d length must match nrow(X)", call. = FALSE)
  }
  .Call(C_n4m_pp_epo_fit_transform, X, d, as.logical(scale)[1L])
}
...
#' @return numeric matrix with the same shape as `X`.
#' @export
n4m_pp_epo_fit_inverse_transform <- function(X, d, scale = TRUE) {
  X <- .n4m_check_matrix(X, "X")
  d <- .n4m_check_numeric_vector(d, "d")
  if (length(d) != nrow(X)) {
    stop("d length must match nrow(X)", call. = FALSE)
  }
  .Call(C_n4m_pp_epo_fit_inverse_transform, X, d, as.logical(scale)[1L])
}
...
#' @export
n4m_pp_flex_pca_fit_transform <- function(X, n_components = 5.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_flex_pca_fit_transform, X,
...
#' @return numeric score matrix.
#' @export
n4m_pp_flex_svd_fit_transform <- function(X, n_components = 5.0) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_flex_svd_fit_transform, X,
        as.double(n_components)[1L])
}
...
#' @export
n4m_pp_fck_static_transform <- function(X, kernel_size = 5L,
                                        filter_orders = c(0.5, 1.0),
                                        filter_scales = c(1.0, 2.0)) {
  X <- .n4m_check_matrix(X, "X")
  filter_orders <- .n4m_check_numeric_vector(filter_orders, "filter_orders")
  filter_scales <- .n4m_check_numeric_vector(filter_scales, "filter_scales")
  .Call(C_n4m_pp_fck_static_transform, X, as.integer(kernel_size)[1L],
...
#' @export
n4m_pp_crop_transform <- function(X, start, end) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_crop_transform, X,
...
#' @export
n4m_pp_resample_transform <- function(X, num_samples) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_resample_transform, X, as.integer(num_samples)[1L])
}
...
#' @export
n4m_pp_resampler_fit_transform <- function(X, source_wavelengths,
                                           target_wavelengths,
                                           method = 0L,
                                           crop_min = 0.0,
                                           crop_max = 0.0,
                                           ...
                                           extrapolate = FALSE) {
  X <- .n4m_check_matrix(X, "X")
  source_wavelengths <- .n4m_check_numeric_vector(source_wavelengths,
                                                  "source_wavelengths")
  target_wavelengths <- .n4m_check_numeric_vector(target_wavelengths,
                                                  "target_wavelengths")
  .Call(C_n4m_pp_resampler_fit_transform, X,
...
#' @export
n4m_pp_kbins_disc_fit_transform <- function(X, n_bins = 5L,
                                            strategy = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_kbins_disc_fit_transform, X,
...
#' @return integer matrix.
#' @export
n4m_pp_range_disc_transform <- function(X, edges) {
  X <- .n4m_check_matrix(X, "X")
  edges <- .n4m_check_numeric_vector(edges, "edges")
  .Call(C_n4m_pp_range_disc_transform, X, edges)
}
...
#' @export
n4m_split_kennard_stone_split <- function(X, test_size) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_split_kennard_stone_split, X, as.double(test_size)[1L])
}
...
#' @return list with `train_idx` and `test_idx` integer vectors (1-based).
#' @export
n4m_split_spxy_split <- function(X, Y, test_size) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.matrix(Y)) {
    Y <- matrix(as.double(Y), ncol = 1L)
  }
  Y <- .n4m_check_matrix(Y, "Y")
  if (nrow(X) != nrow(Y)) {
    stop("X and Y must have the same number of rows", call. = FALSE)
  }
  .Call(C_n4m_split_spxy_split, X, Y, as.double(test_size)[1L])
}
...
#' @return list with `train_idx` and `test_idx` integer vectors (1-based).
#' @export
n4m_split_kbins_stratified_split <- function(Y, test_size = 0.25,
                                             seed = 0,
                                             n_bins = 5L,
                                             strategy = 0L) {
  if (!is.matrix(Y)) {
  ...
  Y <- .n4m_check_matrix(Y, "Y")
  .Call(C_n4m_split_kbins_stratified_split, Y,
...
#'   and `exclusion_rate`.
#' @export
n4m_filter_y_outlier_fit_apply <- function(y, method, threshold,
                                           lower_pct = 1.0,
                                           upper_pct = 99.0) {
  if (!is.numeric(y)) {
    stop("y must be a numeric vector", call. = FALSE)
  }
  .Call(C_n4m_filter_y_outlier_fit_apply, as.double(y),
...
#'   and `exclusion_rate`.
#' @export
n4m_filter_x_outlier_fit_apply <- function(X, method = 0L,
                                           use_threshold = FALSE,
                                           threshold = 0.0,
                                           ...
                                           max_samples = 256) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_filter_x_outlier_fit_apply, X,
...
n4m_pp_direct_standardization_fit_transform <- function(source, target, X = source,
                                                        fit_intercept = TRUE,
                                                        ridge = 0.0) {
  source <- .n4m_check_matrix(source, "source")
  target <- .n4m_check_matrix(target, "target")
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_direct_standardization_fit_transform, source, target, X,
        as.logical(fit_intercept)[1L], as.double(ridge)[1L])
}

n4m_pp_robust_direct_standardization_fit_transform <- function(source, target, X = source,
                                                               fit_intercept = TRUE,
                                                               ridge = 0.0,
                                                               trim_quantile = 0.1,
                                                               max_iter = 5L) {
  ...
  target <- .n4m_check_matrix(target, "target")
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_robust_direct_standardization_fit_transform, source, target, X,
        as.logical(fit_intercept)[1L], as.double(ridge)[1L],
        as.double(trim_quantile)[1L], as.integer(max_iter)[1L])
}

n4m_pp_piecewise_direct_standardization_fit_transform <- function(source, target, X = source,
                                                                  window_size = 5L,
                                                                  fit_intercept = TRUE,
                                                                  ridge = 0.0) {
  source <- .n4m_check_matrix(source, "source")
  target <- .n4m_check_matrix(target, "target")
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_piecewise_direct_standardization_fit_transform, source, target, X,
        as.integer(window_size)[1L], as.logical(fit_intercept)[1L],
        as.double(ridge)[1L])
}

n4m_pp_saps_fit_transform <- function(source, target, X = source,
                                      n_components = 2L, score_weight = 1.0,
                                      fit_intercept = TRUE, ridge = 1e-8) {
  source <- .n4m_check_matrix(source, "source")
  target <- .n4m_check_matrix(target, "target")
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_saps_fit_transform, source, target, X,
        as.integer(n_components)[1L], as.double(score_weight)[1L],
        as.logical(fit_intercept)[1L], as.double(ridge)[1L])
}

n4m_pp_slope_bias_fit_transform <- function(source, target, y = source) {
  .Call(C_n4m_pp_slope_bias_fit_transform, as.double(source),
        as.double(target), as.double(y))
}

n4m_pp_local_centering_fit_transform <- function(source, target, X = source) {
  source <- .n4m_check_matrix(source, "source")
  target <- .n4m_check_matrix(target, "target")
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_local_centering_fit_transform, source, target, X)
}

n4m_pp_weighted_snv_fit_transform <- function(X, weights = NULL,
                                              ddof = 0L, eps = 1e-12) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(weights)) weights <- as.double(weights)
  .Call(C_n4m_pp_weighted_snv_fit_transform, X, weights,
        as.integer(ddof)[1L], as.double(eps)[1L])
}

n4m_pp_vsn_fit_transform <- function(X, eps = 1e-12) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_vsn_fit_transform, X, as.double(eps)[1L])
}

n4m_pp_piecewise_snv_fit_transform <- function(X, window = 16L,
                                               ddof = 0L, eps = 1e-12) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_pp_piecewise_snv_fit_transform, X, as.integer(window)[1L],
        as.integer(ddof)[1L], as.double(eps)[1L])
}

n4m_pp_piecewise_msc_fit_transform <- function(X, reference = NULL,
                                               window = 16L, eps = 1e-12) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(reference)) reference <- as.double(reference)
  .Call(C_n4m_pp_piecewise_msc_fit_transform, X, reference,
        as.integer(window)[1L], as.double(eps)[1L])
}

n4m_pp_localized_msc_fit_transform <- function(X, reference = NULL,
                                               window = 16L, eps = 1e-12) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(reference)) reference <- as.double(reference)
  .Call(C_n4m_pp_localized_msc_fit_transform, X, reference,
        as.integer(window)[1L], as.double(eps)[1L])
}

n4m_pp_xcorr_align_fit_transform <- function(X, reference = NULL,
                                             interval_size = 16L, max_shift = 3L) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(reference)) reference <- as.double(reference)
  .Call(C_n4m_pp_xcorr_align_fit_transform, X, reference,
        as.integer(interval_size)[1L], as.integer(max_shift)[1L])
}

n4m_pp_icoshift_align_fit_transform <- function(X, reference = NULL,
                                                interval_size = 16L, max_shift = 3L) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(reference)) reference <- as.double(reference)
  .Call(C_n4m_pp_icoshift_align_fit_transform, X, reference,
        as.integer(interval_size)[1L], as.integer(max_shift)[1L])
}

n4m_pp_dtw_align_fit_transform <- function(X, reference = NULL,
                                           interval_size = 16L, max_shift = 3L) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(reference)) reference <- as.double(reference)
  .Call(C_n4m_pp_dtw_align_fit_transform, X, reference,
        as.integer(interval_size)[1L], as.integer(max_shift)[1L])
}

n4m_pp_cow_align_fit_transform <- function(X, reference = NULL,
                                           interval_size = 16L, max_shift = 3L) {
  X <- .n4m_check_matrix(X, "X")
  if (!is.null(reference)) reference <- as.double(reference)
  .Call(C_n4m_pp_cow_align_fit_transform, X, reference,
        as.integer(interval_size)[1L], as.integer(max_shift)[1L])
}

n4m_filter_variance_fit_transform <- function(X, threshold = 0.0, top_k = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_filter_variance_fit_transform, X,
        as.double(threshold)[1L], as.integer(top_k)[1L])
}

n4m_filter_correlation_fit_transform <- function(X, y, threshold = 0.0,
                                                 top_k = 0L) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_filter_correlation_fit_transform, X, as.double(y),
        as.double(threshold)[1L], as.integer(top_k)[1L])
}

n4m_interval_generator_fit_transform <- function(X, interval_size = 16L,
                                                 step = interval_size) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_interval_generator_fit_transform, X,
