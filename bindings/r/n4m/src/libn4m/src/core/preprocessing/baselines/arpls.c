...
struct n4m_pp_arpls_state_t {
    double  lam;
    int32_t max_iter;
    double  tol;
};

n4m_pp_arpls_state_t* n4m_pp_arpls_state_new(double lam, int32_t max_iter,
                                              double tol) {
    if (!(lam > 0.0) || max_iter < 0 || !(tol >= 0.0)) {
        return NULL;
    }
    n4m_pp_arpls_state_t* s = (n4m_pp_arpls_state_t*)malloc(sizeof(*s));
...
void n4m_pp_arpls_state_free(n4m_pp_arpls_state_t* state) {
    free(state);
}

n4m_status_t n4m_pp_arpls_state_apply(const n4m_pp_arpls_state_t* state,
                                       const double* X,
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        for (int32_t it = 0; it <= max_iter; ++it) {
            ...
            const n4m_status_t fst = n4m_banded5_factor_into(
            ...
            const n4m_status_t sst = n4m_banded5_solve_into(
                L_buf, D_buf, n, rhs, z);
            if (sst != N4M_OK) {
                free(pen_main); free(pen_sup1); free(pen_sup2);
                free(main_d); free(w); free(w_new); free(z); free(rhs);
