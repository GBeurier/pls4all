 ...
 *
 *   - We read one uint64 per inner iteration via `n4m_rng_pcg64_next_uint64`.
 ...
 *     `n4m_pcg64_engine_next_double` (which is `(uint64 >> 11) * 2^-53`).
            ...
            if (idx == 0) {
                ...
                for (;;) {
                    double xx = -ziggurat_nor_inv_r *
                                log1p(-n4m_pcg64_engine_next_double(rng));
                    double yy = -log1p(-n4m_pcg64_engine_next_double(rng));
                    if (yy + yy > xx * xx) {
                ...
                if (((fi_double[idx - 1] - fi_double[idx]) *
                         n4m_pcg64_engine_next_double(rng) +
