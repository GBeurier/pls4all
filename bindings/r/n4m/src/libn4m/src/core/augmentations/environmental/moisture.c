...
n4m_aug_moisture_state_t* n4m_aug_moisture_state_new(
    ...
    n4m_aug_moisture_state_t* s =
        (n4m_aug_moisture_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
...
n4m_status_t n4m_aug_moisture_apply_impl(
    ...
    if (aw_deltas == NULL) return N4M_ERR_OUT_OF_MEMORY;
    if (state->use_aw_range) {
        n4m_aug_rng_uniform_fill(rng, state->aw_low, state->aw_high,
