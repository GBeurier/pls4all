...
class CropTransformer(StatelessOperator):
    ...
    def transform(self, X) -> np.ndarray:
        ...
        out_cols = int(lib.n4m_pp_crop_output_cols(handle, ctypes.c_int64(X.shape[1])))
        if out_cols <= 0:
...
class ResampleTransformer(StatelessOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_pp_resample_create(ctypes.byref(h), ctypes.c_int64(self.num_samples)),
            "n4m_pp_resample_create",
        )
        return h
    ...
    def transform(self, X) -> np.ndarray:
        X = as_f64_2d(X)
        handle = self._ensure_handle()
        out_cols = int(lib.n4m_pp_resample_output_cols(
...
class Resampler(BaseEstimator, TransformerMixin):
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.n4m_pp_resampler_destroy(self._handle)
            self._handle = None
...
class IntegerKBinsDiscretizer(StatefulOperator):
    ...
    def transform(self, X) -> np.ndarray:
        ...
        check(
            lib.n4m_pp_kbins_disc_transform(
                self._ensure_handle(), numpy_to_view(X), numpy_to_view(out)
            ),
            "n4m_pp_kbins_disc_transform",
        )
        return out
...
class RangeDiscretizer(StatelessOperator):
    ...
    def transform(self, X) -> np.ndarray:
        ...
        out = empty_like_i32(X.shape)
        check(
            lib.n4m_pp_range_disc_transform(
                self._ensure_handle(), numpy_to_view(X), numpy_to_view(out)
            ),
            "n4m_pp_range_disc_transform",
        )
        return out
