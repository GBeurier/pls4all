...
class KennardStoneSplitter(_SplitHandle):
    ...
    def __init__(self, test_size: float = 0.25):
        ...
        self._C_PREFIX = "c4a_split_kennard_stone"
...
class SPXYFoldSplitter(_SplitHandle):
    ...
    def get_n_splits(self) -> int:
        out = ctypes.c_int32()
        check(
            lib.c4a_split_spxy_fold_n_splits(self._ensure_handle(), ctypes.byref(out)),
            "c4a_split_spxy_fold_n_splits",
        )
    ...
    def split_fold(self, X, y, fold_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.c4a_split_spxy_fold_split_fold(
                self._ensure_handle(),
                numpy_to_view(X),
                numpy_to_view(y),
            ...
            "c4a_split_spxy_fold_split_fold",
        )
        return _copy_split_result(result)
...
class SPXYGroupFoldSplitter(_SplitHandle):
    ...
    _C_PREFIX = "c4a_split_spxy_g_fold"
    ...
    def get_n_splits(self) -> int:
        out = ctypes.c_int32()
        check(
            lib.c4a_split_spxy_g_fold_n_splits(self._ensure_handle(), ctypes.byref(out)),
            "c4a_split_spxy_g_fold_n_splits",
        )
        return int(out.value)
    ...
    def split_fold(self, X, y, groups, fold_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.c4a_split_spxy_g_fold_split_fold(
                self._ensure_handle(),
                numpy_to_view(X),
                numpy_to_view(y),
            ...
            "c4a_split_spxy_g_fold_split_fold",
        )
        return _copy_split_result(result)
...
class KBinsStratifiedSplitter(_SplitHandle):
    ...
    def __init__(self, test_size: float = 0.25, seed: int = 0,
        ...
        self._C_PREFIX = "c4a_split_kbins_stratified"
...
class BinnedStratifiedGroupKFoldSplitter(_SplitHandle):
    ...
    _C_PREFIX = "c4a_split_binned_strat_group_kfold"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.c4a_split_binned_strat_group_kfold_create(
                ctypes.byref(h),
                ctypes.c_int32(self.n_splits_value),
                ctypes.c_int32(self.n_bins),
                ctypes.c_int32(self.strategy),
            ...
            "c4a_split_binned_strat_group_kfold_create",
        )
        return h
    ...
    def get_n_splits(self) -> int:
        out = ctypes.c_int32()
        check(
            lib.c4a_split_binned_strat_group_kfold_n_splits(
                self._ensure_handle(), ctypes.byref(out)
            ),
            "c4a_split_binned_strat_group_kfold_n_splits",
        )
        return int(out.value)
    ...
    def split_fold(self, y, groups, fold_idx: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.c4a_split_binned_strat_group_kfold_split_fold(
                self._ensure_handle(),
                numpy_to_view(y),
                groups_arr.ctypes.data_as(ctypes.POINTER(ctypes.c_int64)),
                ctypes.c_int64(groups_arr.size),
            ...
            "c4a_split_binned_strat_group_kfold_split_fold",
        )
        return _copy_split_result(result)
...
class SystematicCircularSplitter(_SplitHandle):
    ...
    _C_PREFIX = "c4a_split_systematic_circular"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.c4a_split_systematic_circular_create(
                ctypes.byref(h), ctypes.c_double(self.test_size),
                ctypes.c_uint64(self.seed)
            ),
            "c4a_split_systematic_circular_create",
        )
        return h
    ...
    def split(self, y) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.c4a_split_systematic_circular_split(
                self._ensure_handle(), numpy_to_view(y), ctypes.byref(result)
            ),
            "c4a_split_systematic_circular_split",
        )
        return _copy_split_result(result)
...
class SPlitSplitter(_SplitHandle):
    ...
    _C_PREFIX = "c4a_split_split_splitter"
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.c4a_split_split_splitter_create(
                ctypes.byref(h), ctypes.c_double(self.test_size),
                ctypes.c_uint64(self.seed)
            ),
            "c4a_split_split_splitter_create",
        )
        return h
    ...
    def split(self, X) -> Tuple[np.ndarray, np.ndarray]:
        ...
        result = SplitResult()
        check(
            lib.c4a_split_split_splitter_split(
                self._ensure_handle(), numpy_to_view(X), ctypes.byref(result)
            ),
            "c4a_split_split_splitter_split",
        )
        return _copy_split_result(result)
