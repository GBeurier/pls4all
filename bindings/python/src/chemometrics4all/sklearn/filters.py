...
class HighLeverageFilter(BaseEstimator):
    ...
    def _create_handle(self):
        ...
        check(
            lib.c4a_filter_leverage_create(
                ctypes.byref(h),
                ctypes.c_int32(self._method_int),
    ...
    def fit(self, X):
        X = as_f64_2d(X)
        check(
            lib.c4a_filter_leverage_fit(self._ensure_handle(), numpy_to_view(X)),
    ...
    def threshold_(self) -> float:
        return float(lib.c4a_filter_leverage_threshold(self._ensure_handle()))
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.c4a_filter_leverage_destroy(self._handle)
            self._handle = None
...
class SpectralQualityFilter(BaseEstimator):
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.c4a_filter_quality_destroy(self._handle)
            self._handle = None
...
class CompositeFilter(BaseEstimator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.c4a_filter_composite_create(
    ...
    def _add_to_c_handle(self, filt):
        if isinstance(filt, HighLeverageFilter):
            check(
                lib.c4a_filter_composite_add_leverage(
            ...
            check(
                lib.c4a_filter_composite_add_quality(
                    self._ensure_handle(), filt._ensure_handle()
                ),
    ...
    def __del__(self):
        if self._handle is not None and self._handle.value:
            lib.c4a_filter_composite_destroy(self._handle)
            self._handle = None
