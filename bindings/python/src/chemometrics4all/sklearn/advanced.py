...
"""Advanced sklearn-style wrappers backed by the libc4a C ABI."""
...
class DirectStandardization(_PairedMatrixOperator):
    ...
    _C_PREFIX = "c4a_pp_direct_standardization"
...
class RobustDirectStandardization(_PairedMatrixOperator):
    ...
    _C_PREFIX = "c4a_pp_robust_direct_standardization"
...
class PiecewiseDirectStandardization(_PairedMatrixOperator):
    ...
    _C_PREFIX = "c4a_pp_piecewise_direct_standardization"
...
class LocalCentering(_PairedMatrixOperator):
    ...
    _C_PREFIX = "c4a_pp_local_centering"
...
class WeightedSNV(_HandleEstimator):
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.c4a_pp_weighted_snv_fit(self._ensure_handle(), numpy_to_view(X)),
            "c4a_pp_weighted_snv_fit",
        )
...
class VariableSortingNormalization(WeightedSNV):
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.c4a_pp_vsn_fit(self._ensure_handle(), numpy_to_view(X)),
            "c4a_pp_vsn_fit",
        )
        self.n_features_in_ = X.shape[1]
        self._fitted = True
...
class PiecewiseSNV(_HandleEstimator):
    ...
    _C_PREFIX = "c4a_pp_piecewise_snv"
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.c4a_pp_piecewise_snv_fit(self._ensure_handle(), numpy_to_view(X)),
            "c4a_pp_piecewise_snv_fit",
        )
        self.n_features_in_ = X.shape[1]
        self._fitted = True
        return self
...
class IcoshiftAlignment(_AlignmentBase):
    ...
    _C_PREFIX = "c4a_pp_icoshift_align"
...
class VarianceFilter(_HandleEstimator):
    ...
    def fit(self, X, y=None):
        X = as_f64_2d(X)
        check(
            lib.c4a_filter_variance_fit(self._ensure_handle(), numpy_to_view(X)),
            "c4a_filter_variance_fit",
        )
        self.n_features_in_ = X.shape[1]
        self._fitted = True
        return self
...
class CorrelationFilter(VarianceFilter):
    ...
    _C_PREFIX = "c4a_filter_correlation"
...
class IntervalGenerator(_HandleEstimator):
    ...
    _C_PREFIX = "c4a_interval_generator"
    ...
    def fit(self, X, y=None):
        ...
        check(
            lib.c4a_interval_generator_fit(
                self._ensure_handle(), numpy_to_view(X)
            ),
            "c4a_interval_generator_fit",
        )
        self.n_features_in_ = X.shape[1]
        self._fitted = True
        return self
    ...
    def _output_cols(self) -> int:
        out_cols = ctypes.c_int64()
        check(
            lib.c4a_interval_generator_output_cols(
                self._ensure_handle(), ctypes.byref(out_cols)
            ),
            "c4a_interval_generator_output_cols",
        )
        return int(out_cols.value)
    ...
    def transform(self, X):
        ...
        check(
            lib.c4a_interval_generator_transform(
                self._ensure_handle(), numpy_to_view(X), numpy_to_view(out)
            ),
            "c4a_interval_generator_transform",
        )
        blocks = []
        offset = 0
