...
"""ABI-close Python functions over ``libc4a``.
...
def snv(X, with_mean: bool = True, with_std: bool = True, ddof: int = 0):
    return transform("c4a_pp_snv", X, ctypes.c_int(with_mean), ctypes.c_int(with_std), ctypes.c_int(ddof))


def lsnv(X, window: int = 11, pad_mode: str | int = "reflect", constant_value: float = 0.0):
    return transform("c4a_pp_lsnv", X, ctypes.c_int32(window), ctypes.c_int32(_enum(_LSNV_PAD_MODES, pad_mode, "pad_mode")), ctypes.c_double(constant_value))


def rnv(X, with_center: bool = True, with_scale: bool = True, k: float = 1.4826):
    return transform("c4a_pp_rnv", X, ctypes.c_int(with_center), ctypes.c_int(with_scale), ctypes.c_double(k))


def area_norm(X, method: str | int = "sum"):
    return transform("c4a_pp_area", X, ctypes.c_int32(_enum(_AREA_METHODS, method, "method")))
...
def normalize(X, feature_min: float = -1.0, feature_max: float = 1.0):
    return transform("c4a_pp_normalize", X, ctypes.c_double(feature_min), ctypes.c_double(feature_max))


def simple_scale(X):
    return transform("c4a_pp_simple_scale", X)
...
def msc(X):
    return transform("c4a_pp_msc", X, fit_X=X)


def emsc(X, degree: int = 2):
    return transform("c4a_pp_emsc", X, ctypes.c_int32(degree), fit_X=X)


def baseline_center(X):
    return transform("c4a_pp_baseline", X, fit_X=X)
...
def first_derivative(X, delta: float = 1.0, edge_order: int = 2):
    return transform("c4a_pp_first_derivative", X, ctypes.c_double(delta), ctypes.c_int32(edge_order))


def second_derivative(X, delta: float = 1.0, edge_order: int = 2):
    return transform("c4a_pp_second_derivative", X, ctypes.c_double(delta), ctypes.c_int32(edge_order))


def norris_williams(X, segment: int = 5, gap: int = 5, derivative_order: int = 1, delta: float = 1.0):
    return transform("c4a_pp_norris_williams", X, ctypes.c_int32(segment), ctypes.c_int32(gap), ctypes.c_int32(derivative_order), ctypes.c_double(delta))


def gaussian(X, sigma: float = 1.0, order: int = 0, mode: str | int = "reflect", cval: float = 0.0, truncate: float = 4.0):
    return transform("c4a_pp_gaussian", X, ctypes.c_double(sigma), ctypes.c_int32(order), ctypes.c_int(_enum(_GAUSSIAN_MODES, mode, "mode")), ctypes.c_double(cval), ctypes.c_double(truncate))


def to_absorbance(X, source_type: int = 0, epsilon: float = 1e-10, clip_negative: bool = True):
    return transform("c4a_pp_to_absorbance", X, ctypes.c_int(source_type), ctypes.c_double(epsilon), ctypes.c_int(clip_negative))


def from_absorbance(X, target_type: int = 0):
    return transform("c4a_pp_from_absorbance", X, ctypes.c_int(target_type))


def pct_to_frac(X):
    return transform("c4a_pp_pct_to_frac", X)
...
def frac_to_pct(X):
    return transform("c4a_pp_frac_to_pct", X)
...
def kubelka_munk(X, source_type: int = 0, epsilon: float = 1e-10):
    return transform("c4a_pp_kubelka_munk", X, ctypes.c_int(source_type), ctypes.c_double(epsilon))


def detrend(X, polyorder: int = 1):
    return transform("c4a_pp_detrend", X, ctypes.c_int32(polyorder))


def asls(X, lam: float = 1e6, p: float = 1e-2, max_iter: int = 50, tol: float = 1e-3):
    return transform("c4a_pp_asls", X, ctypes.c_double(lam), ctypes.c_double(p), ctypes.c_int32(max_iter), ctypes.c_double(tol))


def airpls(X, lam: float = 1e6, max_iter: int = 50, tol: float = 1e-3):
    return transform("c4a_pp_airpls", X, ctypes.c_double(lam), ctypes.c_int32(max_iter), ctypes.c_double(tol))


def arpls(X, lam: float = 1e5, max_iter: int = 50, tol: float = 1e-3):
    return transform("c4a_pp_arpls", X, ctypes.c_double(lam), ctypes.c_int32(max_iter), ctypes.c_double(tol))


def modpoly(X, polyorder: int = 2, max_iter: int = 250, tol: float = 1e-3):
    return transform("c4a_pp_modpoly", X, ctypes.c_int32(polyorder), ctypes.c_int32(max_iter), ctypes.c_double(tol))


def imodpoly(X, polyorder: int = 2, max_iter: int = 250, tol: float = 1e-3):
    return transform("c4a_pp_imodpoly", X, ctypes.c_int32(polyorder), ctypes.c_int32(max_iter), ctypes.c_double(tol))


def snip(X, max_half_window: int = 20):
    return transform("c4a_pp_snip", X, ctypes.c_int32(max_half_window))


def rolling_ball(X, half_window: int = 20, smooth_half_window: int = 0):
    return transform("c4a_pp_rolling_ball", X, ctypes.c_int32(half_window), ctypes.c_int32(smooth_half_window))


def iasls(X, lam: float = 1e6, p: float = 1e-2, lam_1: float = 1e-4, polyorder: int = 2, diff_order: int = 2, max_iter: int = 50, tol: float = 1e-3):
    X_arr = as_f64_2d(X)
    handle = _create_ex("c4a_pp_iasls", ctypes.c_double(lam), ctypes.c_double(p), ctypes.c_double(lam_1), ctypes.c_int32(polyorder), ctypes.c_int32(diff_order), ctypes.c_int32(max_iter), ctypes.c_double(tol))
    try:
        out = empty_like_f64(X_arr.shape)
        check(lib.c4a_pp_iasls_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_iasls_transform")
        return out
    finally:
        _destroy("c4a_pp_iasls", handle)


def beads(X, lam_0: float = 100.0, lam_1: float = 0.5, lam_2: float = 0.5, max_iter: int = 50, tol: float = 1e-3):
    return transform("c4a_pp_beads", X, ctypes.c_double(lam_0), ctypes.c_double(lam_1), ctypes.c_double(lam_2), ctypes.c_int32(max_iter), ctypes.c_double(tol))


def wavelet(X, family: str | int = "haar", mode: str | int = "periodization"):
    X_arr = as_f64_2d(X)
    handle = _create("c4a_pp_wavelet", ctypes.c_int(_enum(_WAVELET_FAMILIES, family, "family")), ctypes.c_int(_enum(_WAVELET_BOUNDARIES, mode, "mode")))
    try:
        out_cols = ctypes.c_int64()
        check(lib.c4a_pp_wavelet_output_cols(handle, ctypes.c_int64(X_arr.shape[1]), ctypes.byref(out_cols)), "c4a_pp_wavelet_output_cols")
        out = empty_like_f64((X_arr.shape[0], int(out_cols.value)))
        check(lib.c4a_pp_wavelet_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_wavelet_transform")
        return out
    finally:
        _destroy("c4a_pp_wavelet", handle)


def haar(X):
    X_arr = as_f64_2d(X)
    handle = _create("c4a_pp_haar")
    try:
        out_cols = ctypes.c_int64()
        check(lib.c4a_pp_haar_output_cols(ctypes.c_int64(X_arr.shape[1]), ctypes.byref(out_cols)), "c4a_pp_haar_output_cols")
        out = empty_like_f64((X_arr.shape[0], int(out_cols.value)))
        check(lib.c4a_pp_haar_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_haar_transform")
        return out
    finally:
        _destroy("c4a_pp_haar", handle)
...
def wavelet_denoise(X, family: str | int = "db4", mode: str | int = "periodization", level: int = 3, threshold_mode: str | int = "soft", noise_estimator: str | int = "median"):
    return transform("c4a_pp_wavelet_denoise", X, ctypes.c_int(_enum(_WAVELET_FAMILIES, family, "family")), ctypes.c_int(_enum(_WAVELET_BOUNDARIES, mode, "mode")), ctypes.c_int32(level), ctypes.c_int(_enum(_WAVELET_THRESHOLDS, threshold_mode, "threshold_mode")), ctypes.c_int(_enum(_WAVELET_NOISE, noise_estimator, "noise_estimator")))


def wavelet_features(X, family: str | int = "haar", mode: str | int = "periodization", max_level: int = 3, entropy: str | int = "energy"):
    X_arr = as_f64_2d(X)
    handle = _create_ex("c4a_pp_wavelet_features", ctypes.c_int(_enum(_WAVELET_FAMILIES, family, "family")), ctypes.c_int(_enum(_WAVELET_BOUNDARIES, mode, "mode")), ctypes.c_int32(max_level), ctypes.c_int(_enum(_WAVELET_FEATURE_ENTROPY, entropy, "entropy")))
    try:
        out_cols = ctypes.c_int64()
        check(lib.c4a_pp_wavelet_features_output_cols(handle, ctypes.c_int64(X_arr.shape[1]), ctypes.byref(out_cols)), "c4a_pp_wavelet_features_output_cols")
        out = empty_like_f64((X_arr.shape[0], int(out_cols.value)))
        check(lib.c4a_pp_wavelet_features_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_wavelet_features_transform")
        return out
    finally:
        _destroy("c4a_pp_wavelet_features", handle)
...
def wavelet_pca(
    ...
    return _wavelet_projection(
        "c4a_pp_wavelet_pca", X, family, mode, max_level, n_components
    )
...
def wavelet_svd(
    ...
    return _wavelet_projection(
        "c4a_pp_wavelet_svd", X, family, mode, max_level, n_components
    )


def osc(X, y, n_components: int = 1, scale: bool = True):
    return _fit_y_transform("c4a_pp_osc", X, y, ctypes.c_int32(n_components), ctypes.c_int(scale))


def epo(X, d, scale: bool = True):
    X_arr = as_f64_2d(X)
    d_arr = _as_f64_1d(d, "d")
    handle = _create("c4a_pp_epo", ctypes.c_int(scale))
    try:
        check(lib.c4a_pp_epo_fit(handle, numpy_to_view(X_arr), _f64_ptr(d_arr), ctypes.c_int64(d_arr.size)), "c4a_pp_epo_fit")
        out = empty_like_f64(X_arr.shape)
        check(lib.c4a_pp_epo_transform_with_d(handle, numpy_to_view(X_arr), _f64_ptr(d_arr), ctypes.c_int64(d_arr.size), numpy_to_view(out)), "c4a_pp_epo_transform_with_d")
        return out
    finally:
        _destroy("c4a_pp_epo", handle)


def flexible_pca(X, n_components: float = 5.0):
    return _flexible("c4a_pp_flex_pca", X, n_components)


def flexible_svd(X, n_components: float = 5.0):
    return _flexible("c4a_pp_flex_svd", X, n_components)
...
def fck_static(X, kernel_size: int, alphas: Sequence[float], sigmas: Sequence[float]):
    ...
    handle = _create("c4a_pp_fck_static", ctypes.c_int32(kernel_size), _f64_ptr(alpha_arr), ctypes.c_int32(alpha_arr.size), _f64_ptr(sigma_arr), ctypes.c_int32(sigma_arr.size))
    try:
        out_cols = ctypes.c_int32()
        check(lib.c4a_pp_fck_static_output_cols(ctypes.c_int32(alpha_arr.size * sigma_arr.size), ctypes.c_int32(X_arr.shape[1]), ctypes.byref(out_cols)), "c4a_pp_fck_static_output_cols")
        out = empty_like_f64((X_arr.shape[0], int(out_cols.value)))
        check(lib.c4a_pp_fck_static_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_fck_static_transform")
        return out
    finally:
        _destroy("c4a_pp_fck_static", handle)


def direct_standardization(X_source, X_target, X=None, fit_intercept: bool = True, ridge: float = 0.0):
    return _paired_transfer("c4a_pp_direct_standardization", X_source, X_target, X_source if X is None else X, ctypes.c_int(fit_intercept), ctypes.c_double(ridge))
...
def robust_direct_standardization(X_source, X_target, X=None, fit_intercept: bool = True, ridge: float = 0.0, trim_quantile: float = 0.9, max_iter: int = 3):
    return _paired_transfer("c4a_pp_robust_direct_standardization", X_source, X_target, X_source if X is None else X, ctypes.c_int(fit_intercept), ctypes.c_double(ridge), ctypes.c_double(trim_quantile), ctypes.c_int32(max_iter))


def piecewise_direct_standardization(X_source, X_target, X=None, window_size: int = 5, fit_intercept: bool = True, ridge: float = 0.0):
    return _paired_transfer("c4a_pp_piecewise_direct_standardization", X_source, X_target, X_source if X is None else X, ctypes.c_int32(window_size), ctypes.c_int(fit_intercept), ctypes.c_double(ridge))
...
def score_augmented_projection_standardization(X_source, X_target, X=None, n_components: int = 5, score_weight: float = 1.0, fit_intercept: bool = True, ridge: float = 0.0):
    return _paired_transfer("c4a_pp_saps", X_source, X_target, X_source if X is None else X, ctypes.c_int32(n_components), ctypes.c_double(score_weight), ctypes.c_int(fit_intercept), ctypes.c_double(ridge))
...
def slope_bias_correction(y_source, y_target, y=None):
    ...
    try:
        check(lib.c4a_pp_slope_bias_fit(handle, _f64_ptr(source), _f64_ptr(target), ctypes.c_int64(source.size)), "c4a_pp_slope_bias_fit")
        check(lib.c4a_pp_slope_bias_transform(handle, _f64_ptr(values), ctypes.c_int64(values.size), _f64_ptr(out)), "c4a_pp_slope_bias_transform")
        return out
    finally:
        _destroy("c4a_pp_slope_bias", handle)


def local_centering(X_source, X_target, X=None):
    return _paired_transfer("c4a_pp_local_centering", X_source, X_target, X_source if X is None else X)
...
def weighted_snv(X, weights=None, ddof: int = 0, eps: float = 1e-12):
    ...
    return transform("c4a_pp_weighted_snv", X, ptr, ctypes.c_int64(n_weights), ctypes.c_int32(ddof), ctypes.c_double(eps), fit_X=X)


def variable_sorting_normalization(X, eps: float = 1e-12):
    return transform("c4a_pp_vsn", X, ctypes.c_double(eps), fit_X=X)


def piecewise_snv(X, window_size: int = 32, ddof: int = 0, eps: float = 1e-12):
    return transform("c4a_pp_piecewise_snv", X, ctypes.c_int32(window_size), ctypes.c_int32(ddof), ctypes.c_double(eps), fit_X=X)
...
def piecewise_msc(X, reference=None, window_size: int = 32, eps: float = 1e-12):
    ...
    return transform("c4a_pp_piecewise_msc", X, ptr, ctypes.c_int64(0 if ref is None else ref.size), ctypes.c_int32(window_size), ctypes.c_double(eps), fit_X=X)


def localized_msc(X, reference=None, window_size: int = 32, eps: float = 1e-12):
    ref = None if reference is None else _as_f64_1d(reference, "reference")
    ptr = _null_f64_ptr() if ref is None else _f64_ptr(ref)
    return transform("c4a_pp_localized_msc", X, ptr, ctypes.c_int64(0 if ref is None else ref.size), ctypes.c_int32(window_size), ctypes.c_double(eps), fit_X=X)
...
def cross_correlation_alignment(X, reference=None, max_shift: int = 5):
    return _align("c4a_pp_xcorr_align", X, reference, 0, max_shift)


def icoshift_alignment(X, reference=None, interval_size: int = 32, max_shift: int = 5):
    return _align("c4a_pp_icoshift_align", X, reference, interval_size, max_shift)


def dynamic_time_warping_alignment(X, reference=None):
    return _align("c4a_pp_dtw_align", X, reference, 0, 0)


def correlation_optimized_warping(X, reference=None, interval_size: int = 32, max_shift: int = 5):
    return _align("c4a_pp_cow_align", X, reference, interval_size, max_shift)


def variance_filter(X, threshold: float = 0.0, top_k: int | None = None):
    return _selector("c4a_filter_variance", X, None, threshold=threshold, top_k=top_k)


def correlation_filter(X, y, threshold: float = 0.0, top_k: int | None = None):
    return _selector("c4a_filter_correlation", X, y, threshold=threshold, top_k=top_k)
...
def _selector(prefix: str, X, y=None, *, threshold: float = 0.0, top_k: int | None = None):
    ...
    try:
        if prefix == "c4a_filter_correlation":
            y_arr = _as_f64_1d(y, "y")
            check(lib.c4a_filter_correlation_fit(handle, numpy_to_view(X_arr), _f64_ptr(y_arr), ctypes.c_int64(y_arr.size)), "c4a_filter_correlation_fit")
        else:
            check(lib.c4a_filter_variance_fit(handle, numpy_to_view(X_arr)), "c4a_filter_variance_fit")
        out_cols = ctypes.c_int64()
        check(getattr(lib, f"{prefix}_output_cols")(handle, ctypes.byref(out_cols)), f"{prefix}_output_cols")
        out = empty_like_f64((X_arr.shape[0], int(out_cols.value)))
        check(getattr(lib, f"{prefix}_transform")(handle, numpy_to_view(X_arr), numpy_to_view(out)), f"{prefix}_transform")
...
def interval_generator(X, interval_size: int = 32, step: int | None = None):
    ...
    handle = _create("c4a_interval_generator", ctypes.c_int32(width), ctypes.c_int32(0 if step is None else stride))
    try:
        check(lib.c4a_interval_generator_fit(handle, numpy_to_view(X_arr)), "c4a_interval_generator_fit")
        out_cols = ctypes.c_int64()
        check(lib.c4a_interval_generator_output_cols(handle, ctypes.byref(out_cols)), "c4a_interval_generator_output_cols")
        out = empty_like_f64((X_arr.shape[0], int(out_cols.value)))
        check(lib.c4a_interval_generator_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_interval_generator_transform")
        blocks = []
        offset = 0
        ...
        _destroy("c4a_interval_generator", handle)
...
def crop(X, start: int, end: int):
    ...
    try:
        out_cols = int(lib.c4a_pp_crop_output_cols(handle, ctypes.c_int64(X_arr.shape[1])))
        out = empty_like_f64((X_arr.shape[0], out_cols))
        check(lib.c4a_pp_crop_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_crop_transform")
        return out
    finally:
        _destroy("c4a_pp_crop", handle)


def resample(X, num_samples: int):
    X_arr = as_f64_2d(X)
    handle = _create("c4a_pp_resample", ctypes.c_int64(num_samples))
    try:
        out_cols = int(lib.c4a_pp_resample_output_cols(handle, ctypes.c_int64(X_arr.shape[1])))
        out = empty_like_f64((X_arr.shape[0], out_cols))
        check(lib.c4a_pp_resample_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_resample_transform")
        return out
    finally:
        _destroy("c4a_pp_resample", handle)
...
def resampler(X, source_wavelengths, target_wavelengths, method: int = 0, crop_min: float = 0.0, crop_max: float = 0.0, use_crop: bool = False, fill_value: float = 0.0, bounds_error: bool = False, extrapolate: bool = False):
    ...
    handle = _create("c4a_pp_resampler", _f64_ptr(target), ctypes.c_int64(target.size), ctypes.c_int32(method), ctypes.c_double(crop_min), ctypes.c_double(crop_max), ctypes.c_int(use_crop), ctypes.c_double(fill_value), ctypes.c_int(bounds_error), ctypes.c_int(extrapolate))
    try:
        check(lib.c4a_pp_resampler_fit(handle, _f64_ptr(source), ctypes.c_int64(source.size)), "c4a_pp_resampler_fit")
        out_cols = int(lib.c4a_pp_resampler_output_cols(handle))
        out = empty_like_f64((X_arr.shape[0], out_cols))
        check(lib.c4a_pp_resampler_transform(handle, numpy_to_view(X_arr), numpy_to_view(out)), "c4a_pp_resampler_transform")
        return out
    finally:
        _destroy("c4a_pp_resampler", handle)


def range_discretizer(X, edges: Sequence[float]):
    edge_arr = _as_f64_1d(edges, "edges")
    return transform("c4a_pp_range_disc", X, _f64_ptr(edge_arr), ctypes.c_int64(edge_arr.size), out_dtype="i32")


def kbins_discretizer(X, n_bins: int = 5, strategy: str | int = "uniform"):
    if isinstance(strategy, str):
        strategy = {"uniform": 0, "quantile": 1}[strategy]
    return transform("c4a_pp_kbins_disc", X, ctypes.c_int32(n_bins), ctypes.c_int32(strategy), fit_X=X, out_dtype="i32")


def kennard_stone(X, test_size: float = 0.25):
    X_arr = as_f64_2d(X)
    handle = _create("c4a_split_kennard_stone", ctypes.c_double(test_size))
    try:
        result = SplitResult()
        check(lib.c4a_split_kennard_stone_split(handle, numpy_to_view(X_arr), ctypes.byref(result)), "c4a_split_kennard_stone_split")
        return _split_result_to_arrays(result)
    finally:
        _destroy("c4a_split_kennard_stone", handle)


def spxy(X, y, test_size: float = 0.25):
    X_arr = as_f64_2d(X)
    y_arr = as_f64_2d(y)
    handle = _create("c4a_split_spxy", ctypes.c_double(test_size))
    try:
        result = SplitResult()
        check(lib.c4a_split_spxy_split(handle, numpy_to_view(X_arr), numpy_to_view(y_arr), ctypes.byref(result)), "c4a_split_spxy_split")
        return _split_result_to_arrays(result)
    finally:
        _destroy("c4a_split_spxy", handle)
...
def kbins_stratified(y, test_size: float = 0.25, seed: int = 17, n_bins: int = 5, strategy: str | int = "uniform"):
    if isinstance(strategy, str):
        strategy = {"uniform": 0, "quantile": 1}[strategy]
    y_arr = as_f64_2d(y)
    handle = _create("c4a_split_kbins_stratified", ctypes.c_double(test_size), ctypes.c_uint64(seed), ctypes.c_int32(n_bins), ctypes.c_int32(strategy))
    try:
        result = SplitResult()
        check(lib.c4a_split_kbins_stratified_split(handle, numpy_to_view(y_arr), ctypes.byref(result)), "c4a_split_kbins_stratified_split")
        return _split_result_to_arrays(result)
    finally:
        _destroy("c4a_split_kbins_stratified", handle)
...
def y_outlier_filter(y, method: str | int = "iqr", threshold: float = 1.5, lower_percentile: float = 1.0, upper_percentile: float = 99.0):
    y_arr = _as_f64_1d(y, "y")
    handle = _create("c4a_filter_y_outlier", ctypes.c_int(_enum(_Y_METHODS, method, "method")), ctypes.c_double(threshold), ctypes.c_double(lower_percentile), ctypes.c_double(upper_percentile))
    try:
        check(lib.c4a_filter_y_outlier_fit(handle, _f64_ptr(y_arr), ctypes.c_int64(y_arr.size)), "c4a_filter_y_outlier_fit")
        mask = np.empty(y_arr.size, dtype=np.uint8)
        stats = FilterStats()
        check(lib.c4a_filter_y_outlier_apply(handle, _f64_ptr(y_arr), ctypes.c_int64(y_arr.size), mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)), ctypes.byref(stats)), "c4a_filter_y_outlier_apply")
        return mask, stats
    finally:
        _destroy("c4a_filter_y_outlier", handle)
...
def x_outlier_filter(X, method: str | int = "mahalanobis", use_threshold: bool = False, threshold: float = 0.0, n_components: int = 0, contamination: float = 0.1, seed: int = 0, n_estimators: int = 100, max_samples: int = 256):
    X_arr = as_f64_2d(X)
    handle = _create("c4a_filter_x_outlier", ctypes.c_int32(_enum(_X_METHODS, method, "method")), ctypes.c_int(use_threshold), ctypes.c_double(threshold), ctypes.c_int32(n_components), ctypes.c_double(contamination), ctypes.c_uint64(seed), ctypes.c_int32(n_estimators), ctypes.c_int64(max_samples))
    try:
        check(lib.c4a_filter_x_outlier_fit(handle, numpy_to_view(X_arr)), "c4a_filter_x_outlier_fit")
        mask = np.empty(X_arr.shape[0], dtype=np.uint8)
        stats = FilterStats()
        check(lib.c4a_filter_x_outlier_apply(handle, numpy_to_view(X_arr), mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)), ctypes.byref(stats)), "c4a_filter_x_outlier_apply")
        return mask, stats
    finally:
        _destroy("c4a_filter_x_outlier", handle)
...
def high_leverage_filter(
    ...
    try:
        check(lib.c4a_filter_leverage_fit(handle, numpy_to_view(X_arr)),
              "c4a_filter_leverage_fit")
        mask = np.empty(X_arr.shape[0], dtype=np.uint8)
        stats = FilterStats()
        check(
            lib.c4a_filter_leverage_apply(
                handle,
                numpy_to_view(X_arr),
        ...
        _destroy("c4a_filter_leverage", handle)
...
def spectral_quality_filter(
    ...
    try:
        ...
        _destroy("c4a_filter_quality", handle)
...
def composite_filter(X, mode: str | int = "any"):
    ...
    composite = _create(
        "c4a_filter_composite",
        ctypes.c_int(_enum(_COMPOSITE_MODES, mode, "mode")),
    )
    try:
        check(lib.c4a_filter_leverage_fit(leverage, numpy_to_view(X_arr)),
              "c4a_filter_leverage_fit")
        check(lib.c4a_filter_composite_add_leverage(composite, leverage),
              "c4a_filter_composite_add_leverage")
        check(lib.c4a_filter_composite_add_quality(composite, quality),
              "c4a_filter_composite_add_quality")
        mask = np.empty(X_arr.shape[0], dtype=np.uint8)
        stats = FilterStats()
        check(
            lib.c4a_filter_composite_apply(
                composite,
                numpy_to_view(X_arr),
                mask.ctypes.data_as(ctypes.POINTER(ctypes.c_uint8)),
                ctypes.byref(stats),
        ...
        _destroy("c4a_filter_composite", composite)
        _destroy("c4a_filter_quality", quality)
        _destroy("c4a_filter_leverage", leverage)
...
def _metric_scalar(name: str, y_true, y_pred) -> float:
    ...
    out = ctypes.c_double()
    check(
        getattr(lib, f"c4a_metric_{name}")(
...
def transfer_metrics(
    ...
    out = TransferMetrics()
    check(
        lib.c4a_transfer_metrics_compute(
...
def rng_pcg64(seed: int = 0, n: int = 1024):
    ...
    try:
        check(
            lib.c4a_rng_pcg64_standard_normal_fill(
                rng.handle,
                _f64_ptr(out),
                ctypes.c_size_t(out.size),
            ),
            "c4a_rng_pcg64_standard_normal_fill",
        )
        return out
    finally:
        rng.close()
...
def aug_gaussian_noise(X, sigma: float = 0.01, seed: int = 0):
    return _aug_apply("c4a_aug_gaussian_noise", X, ctypes.c_double(sigma), seed=seed)


def aug_multiplicative_noise(X, sigma_gain: float = 0.01, seed: int = 0):
    return _aug_apply(
        "c4a_aug_multiplicative_noise", X, ctypes.c_double(sigma_gain), seed=seed
    )
...
def aug_mixup(X, alpha: float = 0.2, seed: int = 0):
    return _aug_apply("c4a_aug_mixup", X, ctypes.c_double(alpha), seed=seed)
...
def aug_spline_smooth(X, seed: int = 0):
    return _aug_apply("c4a_aug_spline_smooth", X, seed=seed)


def aug_spline_x_perturb(X, seed: int = 0):
    return _aug_apply(
        "c4a_aug_spline_x_perturb",
        X,
        ctypes.c_int32(3),
        ctypes.c_double(0.05),
        ctypes.c_double(-0.1),
