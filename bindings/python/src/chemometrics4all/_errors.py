...
"""Exception type raised when libc4a returns a non-``OK`` status code."""
...
class Chemometrics4allError(RuntimeError):
    """libc4a returned a non-OK status code."""
...
def check(status: int, message: str = "") -> None:
    """Raise :class:`Chemometrics4allError` when ``status != Status.OK``."""
    if status != Status.OK:
        raise Chemometrics4allError(status, message)


__all__ = ["Chemometrics4allError", "check"]
