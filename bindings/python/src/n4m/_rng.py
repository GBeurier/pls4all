...
class PCG64:
    """Owned :c:type:`n4m_rng_pcg64_state_t` handle.

    Seeding from a single ``seed`` reproduces ``np.random.default_rng(seed)``
    bit-for-bit; see ``n4m.h §7`` for the parity guarantees.
    """

    def __init__(self, seed: int) -> None:
        handle = ctypes.c_void_p()
        status = lib.n4m_rng_pcg64_create(ctypes.c_uint64(seed), ctypes.byref(handle))
        check(status, f"n4m_rng_pcg64_create(seed={seed})")
        self._handle = handle

    def set_seed(self, seed: int) -> None:
        check(lib.n4m_rng_pcg64_set_seed(self._handle, ctypes.c_uint64(seed)),
              "n4m_rng_pcg64_set_seed")

    def close(self) -> None:
        if getattr(self, "_handle", None) is not None and self._handle.value:
            lib.n4m_rng_pcg64_destroy(self._handle)
