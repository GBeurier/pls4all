...
export class Config {
    ...
    static create(): Config {
        ...
        try {
            const status = m.ccall(
                "n4m_config_create", "number", ["number"], [out]) as number;
            checkStatus(status);
    ...
    destroy(): void {
        ...
        getModule().ccall(
            "n4m_config_destroy", null, ["number"], [this._ptr]);
        this._ptr = 0;
    }

    setNComponents(k: number): void {
        const s = getModule().ccall(
            "n4m_config_set_n_components", "number",
    ...
    setAlgorithm(a: Algorithm): void {
        const s = getModule().ccall(
            "n4m_config_set_algorithm", "number",
    ...
    setDeflation(d: Deflation): void {
        const s = getModule().ccall(
            "n4m_config_set_deflation", "number",
    ...
    setCenterX(on: boolean): void {
        const s = getModule().ccall(
            "n4m_config_set_center_x", "number",
    ...
    setCenterY(on: boolean): void {
        const s = getModule().ccall(
            "n4m_config_set_center_y", "number",
            ["number", "number"], [this._ptr, on ? 1 : 0]) as number;
        checkStatus(s);
    }
    ...
    setStoreScores(on: boolean): void {
        const s = getModule().ccall(
            "n4m_config_set_store_scores", "number",
            ["number", "number"], [this._ptr, on ? 1 : 0]) as number;
        checkStatus(s);
    }
}
