...
#
# Gate 1 (binding_parity) tests for the nirs4all-methods R binding.
#
...
# R-binding output matches the libn4m-reference output bit-for-bit within
# the binding-parity tolerance (`1e-6` by default).
#
# Tests can be skipped at session level by setting N4M_SKIP_PARITY=1.

skip_if_no_lib <- function() {
  if (identical(Sys.getenv("N4M_SKIP_PARITY"), "1")) {
    testthat::skip("N4M_SKIP_PARITY=1 set")
  }
  abi <- tryCatch(n4m_abi_version(), error = function(e) NULL)
  if (is.null(abi)) {
    testthat::skip("libn4m is not loaded")
  }
}
...
test_that("n4m_abi_version returns 1.x.y", {
  skip_if_no_lib()
  abi <- n4m_abi_version()
...
test_that("SNV: every case matches libn4m", {
  skip_if_no_lib()
  fx <- load_fixture("snv")
  X <- fixture_input_matrix(fx)
  for (case in fx$cases) {
...
test_that("LSNV: every case matches libn4m", {
  run_matrix_parity_cases("lsnv", lsnv, "lsnv")
})

test_that("RNV: every case matches libn4m", {
  run_matrix_parity_cases("rnv", rnv, "rnv")
})

test_that("AreaNormalization: every case matches libn4m", {
  run_matrix_parity_cases("area_norm", area_normalization, "area_norm")
})

test_that("Normalize: every case matches libn4m", {
  run_matrix_parity_cases("normalize", normalize, "normalize")
})

test_that("SimpleScale: every case matches libn4m", {
  run_matrix_parity_cases("simple_scale", simple_scale, "simple_scale")
})

test_that("MSC: every case matches libn4m", {
  skip_if_no_lib()
  fx <- load_fixture("msc")
...
test_that("EMSC: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("BaselineCenter: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("SavitzkyGolay: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("FirstDerivative: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("SecondDerivative: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("NorrisWilliams: every case matches libn4m", {
  run_matrix_parity_cases("norris_williams", norris_williams,
                          "norris_williams")
})

test_that("Gaussian: every case matches libn4m", {
  run_matrix_parity_cases("gaussian", gaussian, "gaussian")
})

test_that("ToAbsorbance: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("FromAbsorbance: every case matches libn4m", {
  run_matrix_parity_cases("from_absorbance", from_absorbance,
                          "from_absorbance")
})

test_that("PercentToFraction: every case matches libn4m", {
  run_matrix_parity_cases("pct_to_frac", percent_to_fraction, "pct_to_frac")
})

test_that("FractionToPercent: every case matches libn4m", {
  run_matrix_parity_cases("frac_to_pct", fraction_to_percent, "frac_to_pct")
})

test_that("KubelkaMunk: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("Detrend: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("AsLS: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("AirPLS: every case matches libn4m", {
  skip_if_no_lib()
...
test_that("ArPLS: every case matches libn4m", {
  run_matrix_parity_cases("arpls", arpls, "arpls")
})

test_that("ModPoly: every case matches libn4m", {
  run_matrix_parity_cases("modpoly", modpoly, "modpoly")
})

test_that("IModPoly: every case matches libn4m", {
  run_matrix_parity_cases("imodpoly", imodpoly, "imodpoly")
})

test_that("SNIP: every case matches libn4m", {
  run_matrix_parity_cases("snip", snip, "snip")
})

test_that("RollingBall: every case matches libn4m", {
  run_matrix_parity_cases("rolling_ball", rolling_ball, "rolling_ball")
})

test_that("IAsLS: every case matches libn4m", {
  run_matrix_parity_cases("iasls", iasls, "iasls", tolerance = 5e-6)
})

test_that("BEADS: every case matches libn4m", {
  run_matrix_parity_cases("beads", beads, "beads")
})

# ---- Phase 11 splitters ----

test_that("KennardStone: every case matches libn4m indices", {
  skip_if_no_lib()
...
test_that("SPXY: every case matches libn4m indices", {
  skip_if_no_lib()
...
test_that("YOutlierFilter (IQR): every case matches libn4m mask", {
  skip_if_no_lib()
...
test_that("WaveletDenoise: every case matches libn4m", {
  skip_if_no_lib()
