...
#define N4M_BEADS_FREQ_CUTOFF 0.005
#define N4M_BEADS_ASYMMETRY 6.0
#define N4M_BEADS_EPS0 1.0e-6
#define N4M_BEADS_EPS1 1.0e-6
#define N4M_BEADS_PI 3.141592653589793238462643383279502884

struct n4m_pp_beads_state_t {
    double  lam_0;
    double  lam_1;
    double  lam_2;
    int32_t max_iter;
...
n4m_pp_beads_state_t* n4m_pp_beads_state_new(double lam_0, double lam_1,
    ...
    n4m_pp_beads_state_t* s =
        (n4m_pp_beads_state_t*)malloc(sizeof(*s));
...
static n4m_status_t solve_banded_no_pivot(double* ab, int32_t lower,
...
static n4m_status_t solve_tridiagonal_constant(double off, double diag,
...
static void build_high_pass(int64_t n, double* A, double* B,
    ...
    const double cos_freq = cos(2.0 * N4M_BEADS_PI * N4M_BEADS_FREQ_CUTOFF);
...
static void build_difference_diags(const double* d1, const double* d2,
    ...
    for (int64_t j = 1; j < n; ++j) {
        const double w = -1.0 / (d1[j - 1] + N4M_BEADS_EPS1);
    ...
    for (int64_t j = 2; j < n; ++j) {
        d2_diag[j] = 1.0 / (d2[j - 2] + N4M_BEADS_EPS1);
    }
    for (int64_t j = 0; j < n - 2; ++j) {
        d2_diag[(size_t)4 * (size_t)n + (size_t)j] =
            1.0 / (d2[j] + N4M_BEADS_EPS1);
    }
    for (int64_t j = 0; j < n; ++j) {
    ...
    const double gamma_factor = lam_0 * (1.0 + N4M_BEADS_ASYMMETRY) / 2.0;
    for (int64_t j = 0; j < n; ++j) {
        const double ax = fabs(x[j]);
        out[(size_t)2 * (size_t)n + (size_t)j] +=
            (ax > N4M_BEADS_EPS0)
                ? gamma_factor / ax
                : gamma_factor / N4M_BEADS_EPS0;
    }
}

static double beads_theta(const double* x, int64_t n) {
    const double asym = N4M_BEADS_ASYMMETRY;
    double theta = 0.0;
    for (int64_t i = 0; i < n; ++i) {
        const double xi = x[i];
        if (xi > N4M_BEADS_EPS0) {
            theta += xi;
        } else if (xi < -N4M_BEADS_EPS0) {
            theta -= asym * xi;
        } else {
            theta += ((1.0 + asym) / (4.0 * N4M_BEADS_EPS0)) * xi * xi
                + ((1.0 - asym) / 2.0) * xi
                + N4M_BEADS_EPS0 * (1.0 + asym) / 4.0;
        }
    }
    return theta;
}
...
static double beads_loss_sum(const double* values, int64_t n) {
    ...
    for (int64_t i = 0; i < n; ++i) {
        ...
        total += x - N4M_BEADS_EPS1 * log(x + N4M_BEADS_EPS1);
    }
    return total;
}
...
n4m_status_t n4m_pp_beads_state_apply(const n4m_pp_beads_state_t* state,
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        n4m_status_t st = solve_tridiagonal_constant(
        ...
        for (int64_t i = 0; i < n; ++i) {
            tmp[i] = lam_0 * (1.0 - N4M_BEADS_ASYMMETRY) / 2.0;
        }
