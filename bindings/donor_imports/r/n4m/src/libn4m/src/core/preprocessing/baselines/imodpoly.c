...
struct n4m_pp_imodpoly_state_t {
    int32_t polyorder;
    int32_t max_iter;
    double  tol;
};
...
n4m_pp_imodpoly_state_t* n4m_pp_imodpoly_state_new(int32_t polyorder,
    ...
    n4m_pp_imodpoly_state_t* s =
        (n4m_pp_imodpoly_state_t*)malloc(sizeof(*s));
...
n4m_status_t n4m_pp_imodpoly_state_apply(const n4m_pp_imodpoly_state_t* state,
    ...
    const n4m_status_t qrst = n4m_householder_qr(V_qr, cols, p, tau);
    if (qrst != N4M_OK) {
        free(V_orig); free(V_qr); free(tau); free(qt_y); free(coefs);
        free(y_buf); free(z);
        return qrst;
    ...
    for (int64_t r = 0; r < rows; ++r) {
        ...
        n4m_status_t st = n4m_apply_qt(V_qr, cols, p, tau, qt_y);
        if (st != N4M_OK) { goto fail; }
        st = n4m_back_solve_R(V_qr, cols, p, qt_y, coefs);
        if (st != N4M_OK) { goto fail; }
        for (int64_t i = 0; i < cols; ++i) {
            double bval = 0.0;
        ...
        for (int32_t it = 0; it < state->max_iter; ++it) {
            ...
            memcpy(qt_y, y_buf, (size_t)cols * sizeof(double));
            st = n4m_apply_qt(V_qr, cols, p, tau, qt_y);
            if (st != N4M_OK) { goto fail; }
            st = n4m_back_solve_R(V_qr, cols, p, qt_y, coefs);
            if (st != N4M_OK) { goto fail; }
            for (int64_t i = 0; i < cols; ++i) {
