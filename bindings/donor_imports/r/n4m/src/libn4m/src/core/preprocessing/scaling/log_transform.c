 ...
 * captured by ``n4m_pp_log_state_fit`` and stored in the state struct.
 * ``n4m_pp_log_apply`` then reads the cached value instead of recomputing
...
n4m_pp_log_state_t* n4m_pp_log_state_new(double base, double offset,
    ...
    n4m_pp_log_state_t* s = (n4m_pp_log_state_t*)malloc(sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->base          = base;
