 ...
 *
 * Bounds handling:
 *   - `bounds_error == 1` returns N4M_ERR_NUMERICAL_FAILURE on any
 *     out-of-range query.
...
struct n4m_pp_resampler_state_t {
    /* Configuration */
...
n4m_pp_resampler_state_t* n4m_pp_resampler_state_new(
    ...
    n4m_pp_resampler_state_t* s =
        (n4m_pp_resampler_state_t*)calloc(1, sizeof(*s));
    if (s == NULL) {
        return NULL;
    }
    s->method        = method;
...
void n4m_pp_resampler_state_free(n4m_pp_resampler_state_t* state) {
...
int n4m_pp_resampler_state_is_fitted(const n4m_pp_resampler_state_t* state) {
...
n4m_status_t n4m_pp_resampler_state_fit(n4m_pp_resampler_state_t* state,
    ...
    if (state->use_crop) {
        ...
        if (idx == NULL) return N4M_ERR_OUT_OF_MEMORY;
    ...
    if (state->source_wl == NULL) return N4M_ERR_OUT_OF_MEMORY;
    if (state->use_crop) {
    ...
    if (state->method == 2) {
        ...
        if (state->h == NULL) return N4M_ERR_OUT_OF_MEMORY;
...
int64_t n4m_pp_resampler_state_input_cols(const n4m_pp_resampler_state_t* s) {
    if (s == NULL || !s->fitted) return 0;
    return s->n_raw;
}

int64_t n4m_pp_resampler_state_output_cols(const n4m_pp_resampler_state_t* s) {
    if (s == NULL || !s->fitted) return 0;
    if (s->n_target == -1) {
        return s->n_source;
    }
...
static void linear_evaluate_row(const n4m_pp_resampler_state_t* s,
                                 const double* y,
                                 double* out) {
    const int64_t n_target = s->n_target;
    for (int64_t k = 0; k < n_target; ++k) {
...
static void nearest_evaluate_row(const n4m_pp_resampler_state_t* s,
                                  const double* y,
                                  double* out) {
...
n4m_status_t n4m_pp_resampler_state_apply(
    const n4m_pp_resampler_state_t* state,
    const double* X,
