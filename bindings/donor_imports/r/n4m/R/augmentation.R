...
.n4m_wavelet_family <- function(family, caller) {
...
.n4m_wavelet_boundary <- function(boundary, caller) {
...
.n4m_wavelet_threshold <- function(threshold_mode, caller) {
...
.n4m_wavelet_noise <- function(noise_estimator, caller) {
...
.n4m_wavelet_entropy <- function(entropy, caller) {
...
wavelet <- function(X, family = "haar", boundary = "periodization") {
  n4m_pp_wavelet_transform(
    X,
    family = .n4m_wavelet_family(family, "wavelet"),
    boundary = .n4m_wavelet_boundary(boundary, "wavelet"))
}
...
wavelet_denoise <- function(X,
  ...
  n4m_pp_wavelet_denoise_transform(X,
                                   family = .n4m_wavelet_family(family, "wavelet_denoise"),
                                   boundary = .n4m_wavelet_boundary(boundary, "wavelet_denoise"),
                                   level = level,
                                   threshold_mode = .n4m_wavelet_threshold(threshold_mode, "wavelet_denoise"),
                                   noise_estimator = .n4m_wavelet_noise(noise_estimator, "wavelet_denoise"))
}
...
wavelet_features <- function(X, family = "haar",
  ...
  n4m_pp_wavelet_features_transform(
    X,
    family = .n4m_wavelet_family(family, "wavelet_features"),
    boundary = .n4m_wavelet_boundary(boundary, "wavelet_features"),
    max_level = max_level,
    entropy = .n4m_wavelet_entropy(entropy, "wavelet_features"))
}

.n4m_aug_wavelengths <- function(X, wavelengths) {
  if (is.null(wavelengths)) {
    return(seq(900.0, 1700.0, length.out = ncol(X)))
  }
  wavelengths <- .n4m_check_numeric_vector(wavelengths, "wavelengths")
...
.n4m_aug_apply <- function(name, X, params = numeric(), seed = 17,
                           wavelengths = NULL) {
  X <- .n4m_check_matrix(X, "X")
  wl <- if (is.null(wavelengths)) NULL else .n4m_aug_wavelengths(X, wavelengths)
  .Call(C_n4m_aug_apply, as.character(name)[1L], X, wl,
...
aug_gaussian_noise <- function(X, sigma = 0.01, seed = 17) {
  .n4m_aug_apply("gaussian_noise", X, c(sigma), seed)
}

#' Multiplicative noise augmenter.
#' @export
aug_multiplicative_noise <- function(X, sigma_gain = 0.01, seed = 17) {
  .n4m_aug_apply("multiplicative_noise", X, c(sigma_gain), seed)
}
...
aug_hetero_noise <- function(X, noise_base = 0.001,
  ...
  .n4m_aug_apply("hetero_noise", X, c(noise_base, noise_signal_dep), seed)
}
...
aug_poly_drift <- function(X, degree = 2L, coeff_min = -0.01,
  ...
  .n4m_aug_apply("poly_drift", X, c(degree, coeff_min, coeff_max), seed)
}
...
aug_path_length <- function(X, path_length_std = 0.05,
  ...
  .n4m_aug_apply("path_length", X, c(path_length_std, min_path_length), seed)
}
...
aug_wavelength_shift <- function(X, wavelengths = NULL, shift_lo = -1.0,
  ...
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("wavelength_shift", X, c(shift_lo, shift_hi), seed,
                 .n4m_aug_wavelengths(X, wavelengths))
}
...
aug_wavelength_stretch <- function(X, wavelengths = NULL, stretch_lo = 0.99,
  ...
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("wavelength_stretch", X, c(stretch_lo, stretch_hi), seed,
                 .n4m_aug_wavelengths(X, wavelengths))
}
...
aug_local_warp <- function(X, wavelengths = NULL, n_control_points = 3L,
  ...
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("local_warp", X, c(n_control_points, max_shift), seed,
                 .n4m_aug_wavelengths(X, wavelengths))
}
...
aug_channel_dropout <- function(X, dropout_prob = 0.05, mode = 0L,
                                seed = 17) {
  .n4m_aug_apply("channel_dropout", X, c(dropout_prob, mode), seed)
}
...
aug_magnitude_warp <- function(X, wavelengths = NULL,
                               ...
                               seed = 17) {
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("magnitude_warp", X,
...
aug_local_clip <- function(X, n_regions = 1L, width_lo = 5L,
  ...
  .n4m_aug_apply("local_clip", X, c(n_regions, width_lo, width_hi), seed)
}
...
aug_mixup <- function(X, alpha = 1.0, seed = 17) {
  .n4m_aug_apply("mixup", X, c(alpha), seed)
}

#' Local mixup augmenter.
#' @export
aug_local_mixup <- function(X, alpha = 1.0, k_neighbors = 5L, seed = 17) {
  .n4m_aug_apply("local_mixup", X, c(alpha, k_neighbors), seed)
}
...
aug_scatter_sim <- function(X, a_low = -0.05, a_high = 0.05,
  ...
  .n4m_aug_apply("scatter_sim", X, c(a_low, a_high, b_low, b_high), seed)
}
...
aug_particle_size <- function(X, wavelengths = NULL, mean_size_um = 50.0,
  ...
  .n4m_aug_apply("particle_size", X,
                 ...
                 seed, .n4m_aug_wavelengths(X, wavelengths))
}
...
aug_instrument_broaden <- function(X, wavelengths = NULL, fwhm = 3.0,
                                   ...
                                   variation_scope = 0L, seed = 17) {
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("instrument_broaden", X,
...
aug_detector_rolloff <- function(X, wavelengths = NULL,
                                 ...
                                 seed = 17) {
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("detector_rolloff", X,
...
aug_truncated_peak <- function(X, wavelengths = NULL,
                               ...
                               seed = 17) {
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("truncated_peak", X,
...
aug_edge_artifacts <- function(X, wavelengths = NULL, enabled_flags = 15L,
  ...
  X <- .n4m_check_matrix(X, "X")
  .n4m_aug_apply("edge_artifacts", X,
...
aug_spline_smooth <- function(X, seed = 17) {
  .n4m_aug_apply("spline_smooth", X, numeric(), seed)
}
...
aug_spline_x_perturb <- function(X, spline_degree = 3L,
                                 ...
                                 seed = 17) {
  .n4m_aug_apply("spline_x_perturb", X,
...
aug_spline_y_perturb <- function(X, spline_points = -1L,
                                 ...
                                 seed = 17) {
  .n4m_aug_apply("spline_y_perturb", X,
...
aug_rotate_translate <- function(X, p_range = 2.0, y_factor = 3.0,
                                 seed = 17) {
  .n4m_aug_apply("rotate_translate", X, c(p_range, y_factor), seed)
}
...
spectral_quality <- function(X, max_nan_ratio = 0.1,
                             ...
                             check_inf = TRUE) {
  X <- .n4m_check_matrix(X, "X")
  .Call(C_n4m_filter_quality_apply, X, as.double(max_nan_ratio)[1L],
