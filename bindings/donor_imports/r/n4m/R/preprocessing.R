...
snv <- function(X, with_mean = TRUE, with_std = TRUE, ddof = 0L) {
  n4m_pp_snv_transform(X, with_mean = with_mean, with_std = with_std,
                       ddof = ddof)
}
...
lsnv <- function(X, window = 11L, pad_mode = "reflect",
  ...
  n4m_pp_lsnv_transform(X, window = window, pad_mode = pad_int,
...
rnv <- function(X, with_center = TRUE, with_scale = TRUE, k = 1.4826) {
  n4m_pp_rnv_transform(X, with_center = with_center,
...
area_normalization <- function(X, method = "sum") {
  ...
  n4m_pp_area_transform(X, method = method_int)
}
...
normalize <- function(X, feature_min = -1.0, feature_max = 1.0) {
  n4m_pp_normalize_transform(X, feature_min = feature_min,
...
simple_scale <- function(X) {
  n4m_pp_simple_scale_transform(X)
}
...
msc <- function(X, X_fit = X) {
  out <- n4m_pp_msc_fit_transform(X_fit, X)
  ...
  attr(out, "n4m_msc_offsets") <- offsets
  attr(out, "n4m_msc_slopes") <- slopes
  out
}
...
msc_inverse_transform <- function(X, X_fit = X) {
  offsets <- attr(X, "n4m_msc_offsets", exact = TRUE)
  slopes <- attr(X, "n4m_msc_slopes", exact = TRUE)
...
emsc <- function(X, X_fit = X, degree = 1L) {
  n4m_pp_emsc_fit_transform(X_fit, X, degree = degree)
}
...
baseline_center <- function(X, X_fit = X) {
  n4m_pp_baseline_fit_transform(X_fit, X)
}
...
baseline_center_inverse_transform <- function(X, X_fit = X) {
  n4m_pp_baseline_fit_inverse_transform(X_fit, X)
}
...
savitzky_golay <- function(X, window_length, polyorder, deriv = 0L,
  ...
  n4m_pp_savgol_transform(X, window_length = window_length,
...
first_derivative <- function(X, delta = 1.0, edge_order = 2L) {
  n4m_pp_first_derivative_transform(X, delta = delta, edge_order = edge_order)
}
...
second_derivative <- function(X, delta = 1.0, edge_order = 2L) {
  n4m_pp_second_derivative_transform(X, delta = delta, edge_order = edge_order)
}
...
norris_williams <- function(X, segment = 5L, gap = 5L,
  ...
  n4m_pp_norris_williams_transform(X, segment = segment, gap = gap,
...
gaussian <- function(X, sigma = 1.0, order = 0L, mode = "reflect",
  ...
  n4m_pp_gaussian_transform(X, sigma = sigma, order = order,
...
to_absorbance <- function(X, is_percent = FALSE, epsilon = 1e-10,
                          clip_negative = TRUE) {
  n4m_pp_to_absorbance_transform(X, is_percent = is_percent,
                                 epsilon = epsilon,
...
from_absorbance <- function(X, is_percent = FALSE) {
  n4m_pp_from_absorbance_transform(X, is_percent = is_percent)
}
...
percent_to_fraction <- function(X) {
  n4m_pp_pct_to_frac_transform(X)
}
...
fraction_to_percent <- function(X) {
  n4m_pp_frac_to_pct_transform(X)
}
...
kubelka_munk <- function(X, is_percent = FALSE, epsilon = 1e-10) {
  n4m_pp_kubelka_munk_transform(X, is_percent = is_percent,
                                epsilon = epsilon)
}
