...
detrend <- function(X, polyorder = 1L) {
  n4m_pp_detrend_transform(X, polyorder = polyorder)
}
...
asls <- function(X, lam = 1e6, p = 0.01, max_iter = 50L, tol = 1e-3) {
  n4m_pp_asls_transform(X, lam = lam, p = p, max_iter = max_iter, tol = tol)
}
...
airpls <- function(X, lam = 1e6, max_iter = 50L, tol = 1e-3) {
  n4m_pp_airpls_transform(X, lam = lam, max_iter = max_iter, tol = tol)
}
...
arpls <- function(X, lam = 1e5, max_iter = 50L, tol = 1e-3) {
  n4m_pp_arpls_transform(X, lam = lam, max_iter = max_iter, tol = tol)
}
...
modpoly <- function(X, polyorder = 2L, max_iter = 250L, tol = 1e-3) {
  n4m_pp_modpoly_transform(X, polyorder = polyorder,
...
imodpoly <- function(X, polyorder = 2L, max_iter = 250L, tol = 1e-3) {
  n4m_pp_imodpoly_transform(X, polyorder = polyorder,
                            max_iter = max_iter, tol = tol)
}
...
snip <- function(X, max_half_window = 20L) {
  n4m_pp_snip_transform(X, max_half_window = max_half_window)
}
...
rolling_ball <- function(X, half_window = 20L, smooth_half_window = 0L) {
  n4m_pp_rolling_ball_transform(X, half_window = half_window,
...
iasls <- function(X, lam = 1e6, p = 0.01, lam_1 = 1e-4,
  ...
  n4m_pp_iasls_transform(X, lam = lam, p = p, lam_1 = lam_1,
...
beads <- function(X, lam_0 = 100.0, lam_1 = 0.5, lam_2 = 0.5,
                  max_iter = 50L, tol = 1e-3) {
  n4m_pp_beads_transform(X, lam_0 = lam_0, lam_1 = lam_1,
