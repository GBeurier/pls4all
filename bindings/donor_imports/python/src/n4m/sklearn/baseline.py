...
class Detrend(StatelessOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_pp_detrend_create(ctypes.byref(h), ctypes.c_int32(self.polyorder)),
            "n4m_pp_detrend_create",
        )
        return h
...
class AirPLS(StatelessOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_pp_airpls_create(
                ctypes.byref(h),
                ctypes.c_double(self.lam),
                ctypes.c_int32(self.max_iter),
                ctypes.c_double(self.tol),
            ),
            "n4m_pp_airpls_create",
        )
        return h
...
class ModPoly(StatelessOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_pp_modpoly_create(
                ctypes.byref(h),
                ctypes.c_int32(self.polyorder),
                ctypes.c_int32(self.max_iter),
                ctypes.c_double(self.tol),
            ),
            "n4m_pp_modpoly_create",
        )
        return h
...
class IModPoly(StatelessOperator):
    ...
    def _create_handle(self):
        h = ctypes.c_void_p()
        check(
            lib.n4m_pp_imodpoly_create(
                ctypes.byref(h),
                ctypes.c_int32(self.polyorder),
                ctypes.c_int32(self.max_iter),
                ctypes.c_double(self.tol),
            ),
            "n4m_pp_imodpoly_create",
        )
        return h
