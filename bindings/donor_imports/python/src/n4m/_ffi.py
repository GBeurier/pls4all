...
Discovery order:
  ...
  1. ``N4M_LIB_PATH`` environment variable (full path to the
...
The lookup is OS-aware: ``libn4m.so.1.9.0`` on Linux, ``libn4m.1.9.0.dylib``
on macOS, ``n4m.dll`` on Windows.
"""
...
def _candidate_library_names() -> list[str]:
    ...
    if system == "Linux":
        return [
            f"libn4m.so.{ABI_VERSION_STRING}",
            f"libn4m.so.{ABI_VERSION_MAJOR}",
            "libn4m.so",
        ]
    if system == "Darwin":
        return [
            f"libn4m.{ABI_VERSION_STRING}.dylib",
            f"libn4m.{ABI_VERSION_MAJOR}.dylib",
            "libn4m.dylib",
        ]
    if system == "Windows":
        return ["n4m.dll", "libn4m.dll"]
    return ["libn4m.so", "libn4m.dylib", "n4m.dll"]
...
def _candidate_search_paths() -> list[Path]:
    ...
    # Development fallbacks: walk upward looking for a nirs4all-methods checkout.
    here = _PKG_DIR
    for _ in range(8):
        here = here.parent
...
def _resolve_library_path() -> Path:
    ...
    raise FileNotFoundError(
        f"libn4m not found. Searched: {searched}. Set N4M_LIB_PATH "
        f"to the absolute path of libn4m.so.{ABI_VERSION_STRING} (or the "
...
def _load() -> ctypes.CDLL:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load libn4m from {path}: {exc}") from exc
    _ffi_decls.apply_argtypes(lib)
    # Verify ABI compatibility: the loaded library must expose a MAJOR ==
    # ours and a MINOR >= ours. ``n4m_check_abi_compatibility`` returns
    # N4M_OK on success.
