...
nirs4all.operators.filters.HighLeverageFilter implementation). The c4a
...
def high_leverage_leverages(X: np.ndarray,
    ...
    as the c4a engine: fit on X, then compute leverages on the same X. This is
    a one-shot call (the c4a handle separates fit and transform but the
