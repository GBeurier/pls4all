...
HEADER_SPECS = {
    ...
    "rep-selection": {
        "out": REPO_ROOT / "cpp" / "tests" / "fixtures" / "rep_selection_fixtures.hpp",
        "fixtures": (
            "synthetic_rep_pls_recursive_v1",
        ),
        "struct": "RepSelectionFixture",
        "array": "kRepSelectionFixtures",
    },
    "ipw-selection": {
        "out": REPO_ROOT / "cpp" / "tests" / "fixtures" / "ipw_selection_fixtures.hpp",
        "fixtures": (
            "synthetic_ipw_pls_reweighted_v1",
        ),
...
def generate_aom_selection(fixture_ids: Sequence[str],
                           fixture_dir: Path,
                           out: Path,
                           family: str,
                           struct_name: str,
    ...
    for fixture in fixtures:
        ...
        y_name = f"{prefix}_Y"
        op_params_name = f"{prefix}_operator_params"
        op_scores_name = f"{prefix}_operator_scores"
        curves_name = f"{prefix}_rmse_curves"
        ...
        _emit_array(lines, y_name, fixture["data"]["Y"]["values"])
        _emit_array(lines, op_params_name, expected["operator_params"]["values"])
        _emit_array(lines, op_scores_name, expected["operator_scores"]["values"])
        _emit_array(lines, curves_name, expected["rmse_curves"]["values"])
        ...
        entries.append(
            ...
            f"        {int(expected['operator_kinds']['shape'][1])},\n"
            f"        {int(params['cv'])},\n"
            f"        {int(expected['selected_operator_index'])},\n"
            f"        {int(expected['selected_n_components'])},\n"
            f"        {_format_double(float(expected['best_score']))},\n"
            f"        {_matrix_ref(x_name, fixture['data']['X'])},\n"
            f"        {_matrix_ref(y_name, fixture['data']['Y'])},\n"
            f"        {_matrix_ref(op_params_name, expected['operator_params'])},\n"
            f"        {_matrix_ref(op_scores_name, expected['operator_scores'])},\n"
            f"        {_matrix_ref(curves_name, expected['rmse_curves'])},\n"
            f"        {_matrix_ref(predictions_name, expected['predictions'])},\n"
            f"        AomSelectionIndexRef{{{kinds_name}, sizeof({kinds_name}) / sizeof(std::int64_t)}},\n"
            f"        AomSelectionIndexRef{{{param_offsets_name}, sizeof({param_offsets_name}) / sizeof(std::int64_t)}},\n"
            f"        AomSelectionIndexRef{{{train_offsets_name}, sizeof({train_offsets_name}) / sizeof(std::int64_t)}},\n"
...
def generate_rep_selection(fixture_ids: Sequence[str],
                           fixture_dir: Path,
                           out: Path,
                           family: str,
                           struct_name: str,
    ...
    lines = [
        ...
        "    MatrixRef coefficient_scores;",
        "    MatrixRef rmse_by_step;",
        "    RepSelectionIndexRef retained_counts;",
        "    RepSelectionIndexRef removed_counts;",
        "    RepSelectionIndexRef removed_indices;",
        "    RepSelectionIndexRef selected_indices;",
        "};",
        "",
    ]
    ...
    for fixture in fixtures:
        ...
        rmse_name = f"{prefix}_rmse_by_step"
        retained_name = f"{prefix}_retained_counts"
        removed_counts_name = f"{prefix}_removed_counts"
        removed_indices_name = f"{prefix}_removed_indices"
        selected_name = f"{prefix}_selected_indices"
        _emit_array(lines, x_name, fixture["data"]["X"]["values"])
        _emit_array(lines, y_name, fixture["data"]["Y"]["values"])
        _emit_array(lines, coefficients_name, expected["coefficient_scores"]["values"])
        _emit_array(lines, rmse_name, expected["rmse_by_step"]["values"])
        _emit_int_array(lines, retained_name, expected["retained_counts"]["values"])
        _emit_int_array(lines, removed_counts_name, expected["removed_counts"]["values"])
        _emit_int_array(lines, removed_indices_name, expected["removed_indices"]["values"])
        _emit_int_array(lines, selected_name, expected["selected_indices"]["values"])
        entries.append(
            "    {\n"
            f'        "{fid}",\n'
            ...
            f"        {_matrix_ref(coefficients_name, expected['coefficient_scores'])},\n"
            f"        {_matrix_ref(rmse_name, expected['rmse_by_step'])},\n"
            f"        RepSelectionIndexRef{{{retained_name}, sizeof({retained_name}) / sizeof(std::int64_t)}},\n"
            f"        RepSelectionIndexRef{{{removed_counts_name}, sizeof({removed_counts_name}) / sizeof(std::int64_t)}},\n"
            f"        RepSelectionIndexRef{{{removed_indices_name}, sizeof({removed_indices_name}) / sizeof(std::int64_t)}},\n"
            f"        RepSelectionIndexRef{{{selected_name}, sizeof({selected_name}) / sizeof(std::int64_t)}}\n"
            "    }"
        )
...
def generate_ipw_selection(fixture_ids: Sequence[str],
                           fixture_dir: Path,
                           out: Path,
                           family: str,
                           struct_name: str,
    ...
    lines = [
        ...
        "    MatrixRef X;",
        "    MatrixRef Y;",
        "    MatrixRef score_path;",
        "    MatrixRef weight_path;",
        "    MatrixRef rmse_by_iteration;",
        "    IpwSelectionIndexRef selected_by_iteration;",
        "    IpwSelectionIndexRef ranking_indices;",
        "    IpwSelectionIndexRef selected_indices;",
        "};",
        "",
    ]
    ...
    for fixture in fixtures:
        ...
        x_name = f"{prefix}_X"
        y_name = f"{prefix}_Y"
        score_name = f"{prefix}_score_path"
        weight_name = f"{prefix}_weight_path"
        rmse_name = f"{prefix}_rmse_by_iteration"
        by_iteration_name = f"{prefix}_selected_by_iteration"
        ranking_name = f"{prefix}_ranking_indices"
        selected_name = f"{prefix}_selected_indices"
        _emit_array(lines, x_name, fixture["data"]["X"]["values"])
        _emit_array(lines, y_name, fixture["data"]["Y"]["values"])
        _emit_array(lines, score_name, expected["score_path"]["values"])
        _emit_array(lines, weight_name, expected["weight_path"]["values"])
        _emit_array(lines, rmse_name, expected["rmse_by_iteration"]["values"])
        _emit_int_array(lines, by_iteration_name, expected["selected_by_iteration"]["values"])
        ...
        entries.append(
            ...
            f"        {int(params['n_iterations'])},\n"
            f"        {int(params['top_k'])},\n"
            f"        {_format_double(float(params['damping']))},\n"
            f"        {_format_double(float(params['weight_floor']))},\n"
            f"        {int(expected['best_iteration'])},\n"
            f"        {_format_double(float(expected['best_rmse']))},\n"
            f"        {_matrix_ref(x_name, fixture['data']['X'])},\n"
            f"        {_matrix_ref(y_name, fixture['data']['Y'])},\n"
            f"        {_matrix_ref(score_name, expected['score_path'])},\n"
            f"        {_matrix_ref(weight_name, expected['weight_path'])},\n"
            f"        {_matrix_ref(rmse_name, expected['rmse_by_iteration'])},\n"
            f"        IpwSelectionIndexRef{{{by_iteration_name}, sizeof({by_iteration_name}) / sizeof(std::int64_t)}},\n"
            f"        IpwSelectionIndexRef{{{ranking_name}, sizeof({ranking_name}) / sizeof(std::int64_t)}},\n"
            f"        IpwSelectionIndexRef{{{selected_name}, sizeof({selected_name}) / sizeof(std::int64_t)}}\n"
            "    }"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.family == "pipeline":
        ...
        generate_shaving_selection(fixture_ids, args.fixture_dir, out, args.family,
                                   str(spec["struct"]), str(spec["array"]))
    elif args.family == "rep-selection":
        generate_rep_selection(fixture_ids, args.fixture_dir, out, args.family,
                               str(spec["struct"]), str(spec["array"]))
