...
# Parity generator (own pyproject.toml — internal tool tracks pls4all version)
update_with_sed \
...
update_with_sed \
    "bindings/rust/pls4all/Cargo.toml" \
    "^version[[:space:]]*=[[:space:]]*\"([0-9]+\.[0-9]+\.[0-9]+)\"" \
    "s/^(version[[:space:]]*=[[:space:]]*\")[0-9]+\.[0-9]+\.[0-9]+(\")/\1${VERSION}\2/"

# Rust lock (Cargo.lock:  bump only the [[package]] entry where name = "pls4all")
# We use a small python helper because TOML arrays-of-tables are not pleasant
# to munge with sed.
update_cargo_lock() {
    local rel="bindings/rust/pls4all/Cargo.lock"
    local abs="${ROOT}/${rel}"
    ...
    # the file is *present*, we still require its [[package]] pls4all entry
...
PY
    ...
    if [[ -z "${found}" ]]; then
        echo "  DRIFT: ${rel} present but has no [[package]] entry for pls4all" >&2
        DRIFTED+=("${rel}")
        EXIT_CODE=1
        return 0
    fi
    if [[ "${MODE}" == "check" ]]; then
        if [[ "${found}" != "${VERSION}" ]]; then
            echo "  DRIFT: ${rel} (pls4all entry) '${found}' (expected '${VERSION}')" >&2
            DRIFTED+=("${rel}")
            EXIT_CODE=1
        fi
    else
...
with open(path, "w", encoding="utf-8") as f:
    f.write(txt)
PY
        echo "  updated ${rel} (pls4all entry): ${found} → ${VERSION}"
    fi
}
update_cargo_lock
...
update_with_sed \
    "bindings/dotnet/Pls4all/Pls4all.csproj" \
