...
|---|---|---|
| **GitHub Pages** (`https://gbeurier.github.io/chemometrics4all/`) | Push to `main` via `.github/workflows/docs.yml` | Live `benchmarks/cross_binding/results/full_matrix.csv` if present, else `docs/_static/bench-data.json` |
| **Read the Docs** (`chemometrics4all.readthedocs.io`) | RTD webhook on every push | `docs/_static/bench-data.json` (the committed fallback) |
