...
```bash
# After editing benchmarks/parity_timing/registry.py, regenerate the
# declarative validation snapshot consumed by the docs and the
# landing dashboard:
...
git add benchmarks/validation/registry/ docs/_static/bench-data.json docs/methods/
git commit -m "Refresh RTD-bundled benchmark snapshot + method pages"
```

The `validation/registry/*.json` snapshot is the single source of
truth for the `### Validation contract` block in each method page and
the `validation` field embedded in `docs/_static/bench-data.json`.
`build_landing.py` and `build_methods.py` read it through
`docs/_extras/validation_registry.py`; missing files degrade
gracefully (no `### Validation contract` section, `validation.available
= false` in the JSON), but CI / pre-publish must run
`python -m benchmarks.validation.validate_registry` to assert the
committed snapshot is in sync with `parity_timing.registry`.
