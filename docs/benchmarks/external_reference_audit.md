...
contracts, not to compare C++ against a second local C4A implementation.
...
| chemometrics4all bindings in the dashboard | C++ ABI, `C4A.python`, `C4A.sklearn`, R |

## New Or Previously Missing Gates

| Method | Executable reference gate | Contract notes |
|--------|---------------------------|----------------|
| Robust direct standardization | `sklearn.linear_model.LinearRegression(iterative residual trimming)` | Composite gate: scikit-learn owns the least-squares fits; the adapter applies C4A's documented residual-trimming schedule. No native robust-DS package with this exact contract was found. |
...
| Score-augmented projection standardization | `sklearn.decomposition.PCA + sklearn.linear_model.LinearRegression` | Composite gate matching C4A's PCA-score augmentation and OLS transfer contract. The SA-PBS paper is public, but no installable native package was found. |
...
| Piecewise SNV | `nirs4all.StandardNormalVariate(ddof=0) per interval` | nirs4all supplies SNV; the adapter applies it independently over C4A intervals. |
...
| Correlation optimized warping | `cowarp.warp(segment_length=16, slack=2)` | `cowarp` supplies the external dynamic-programming COW contract; C4A was changed to match that contract. |
| Interval generator | `sklearn.compose.ColumnTransformer(passthrough intervals)` | scikit-learn supplies interval column selection; the adapter returns C4A's list-of-blocks shape. |
...
| Piecewise direct standardization | `pycaltransfer.pds_pls_transfer_fit` | pycaltransfer exposes PDS-PLS; C4A's active contract is local OLS PDS and is gated by scikit-learn. |
...
| Detrend | `prospectr::detrend` | prospectr combines the NIR detrend convention with package-specific behavior; SciPy/nirs4all cover the active C4A contract. |
| Wavelet / Haar | `nirs4all` detail-resampled wavelet operators | nirs4all returns resampled detail coefficients; `PyWavelets.dwt(..., mode="periodization")` gates the active C4A output. |
| Savitzky-Golay | `nirs4all.SavitzkyGolay(default edge mode)` | nirs4all does not expose C4A's mirror edge mode; SciPy and prospectr cover the same-contract gates. |
...
- PDS-PLS references and worked examples commonly model each local window with PLS, which is why they are not the same contract as C4A's local OLS PDS gate.
