...
chemometrics4all is benchmarked across three axes:

1. **Cross-binding parity + timing** — same algorithm, same data,
   chemometrics4all vs the leading external implementation in each language
...
2. **Accelerated backend timing** — same SIMPLS, libc4a built five
   ways (`ref`, `blas`, `omp`, `blas_omp`, `cuda`). Shows the speedup
   stack at the C++ layer.

3. **Algorithm matrix** — chemometrics4all's ~60 algorithms timed at
