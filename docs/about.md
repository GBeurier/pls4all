...
- **R binding COMPLETE** — `chemometrics4all_method()` dispatcher covers all 33
...
- **MATLAB / Octave binding COMPLETE** — single dispatcher MEX exposes
  ...
  `chemometrics4all.fit(algo, X, y, …)` factory.
...
The C ABI surface is feature-complete (96 `c4a_*` symbols). Lifecycle for
