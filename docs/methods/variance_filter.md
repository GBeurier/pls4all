...
- `ref.sklearn` — sklearn.feature_selection.VarianceThreshold (Python).

### Mathematical principle

`variance_filter` follows the standard feature selection operator contract: an input matrix $\mathbf{X}\in\mathbb{R}^{n\times p}$ is transformed, scored, split, or filtered by the method-specific kernel while preserving the parity contract recorded by the benchmark snapshots. The precise numerical convention is the one exposed by the C ABI signatures and the registered external references below.
...
- ◆ **`ref.sklearn`** (Python · canonical) — `sklearn.feature_selection.VarianceThreshold` · scikit-learn 1.8.0
:::

### Validation contract

- Operation: `fit_transform` · comparator: `default_allclose` · tolerance: `rtol=1e-05`, `atol=1e-08` · quality: **strict**
- Default validation dataset: `64×128` · seed `1234567913`
...
|---------|---------|------|------------|------|
| `ref.sklearn` | `sklearn.feature_selection.VarianceThreshold` | Python / parity | `default_allclose` |  |
