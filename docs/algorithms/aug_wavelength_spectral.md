...
c4a_status_t c4a_aug_<NAME>_create(c4a_aug_<NAME>_handle_t** out,
                                    ...
                                    /* op-specific params */);
c4a_status_t c4a_aug_<NAME>_apply (const c4a_aug_<NAME>_handle_t* h,
                                    c4a_matrix_view_t X,
                                    c4a_matrix_view_t out);
void          c4a_aug_<NAME>_destroy(c4a_aug_<NAME>_handle_t* h);
```

The augmenters are stateless w.r.t. data (no `_fit`, no
`_inverse_transform`, no `_is_fitted`). Each `_apply` advances the RNG
state; tests reseed the RNG via `c4a_rng_pcg64_set_seed` immediately
...
`c4a_aug_convolve_reflect` helper that lives in
...
kernels). The kernel is built via `c4a_aug_gauss_kernel_build` — a simple
...
`c4a_pp_gaussian_state_t` engine: that path is parameterised by
...
| --------------------- | ------------------------------------------------------------ | ----------------- |
| All ten Phase 16 ops  | `parity/python_generator/src/c4a_parity_aug_spectral_ref/*` | 1e-15 abs/rel     |
...
breaking the chemometrics4all parity gates.
...
```
c4a_aug_wavelength_shift_create / _apply / _destroy
c4a_aug_wavelength_stretch_create / _apply / _destroy
c4a_aug_local_warp_create / _apply / _destroy
c4a_aug_band_perturb_create / _apply / _destroy
c4a_aug_band_mask_create / _apply / _destroy
c4a_aug_channel_dropout_create / _apply / _destroy
c4a_aug_gauss_jitter_create / _apply / _destroy
c4a_aug_unsharp_mask_create / _apply / _destroy
c4a_aug_magnitude_warp_create / _apply / _destroy
c4a_aug_local_clip_create / _apply / _destroy
```

All symbols are exported via the wildcard `c4a_*` Linux version script;
the canonical declarations land in `c4a.h` with the post-merge
