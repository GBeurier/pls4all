...
**Date**: 2026-05-18
**Scope**: 7 augmenter operators, first members of the new `c4a_aug_*` ABI category.
...
| ------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GaussianAdditiveNoise           | `c4a_aug_gaussian_noise_*`    | `out = X + N(0, 1) * std(X[i, :]) * sigma`                                                                                                            |
| MultiplicativeNoise             | `c4a_aug_multiplicative_noise_*` | `out[i, j] = X[i, j] * (1 + sigma_gain * N_i)`                                                                                                       |
| SpikeNoise                      | `c4a_aug_spike_noise_*`       | `n_spikes[i] = U[n_min, n_max]`; sort+unique random indices, add Uniform amplitudes                                                                    |
| HeteroscedasticNoiseAugmenter   | `c4a_aug_hetero_noise_*`      | `sigma[i, j] = base + dep * |X[i, j]|`; `out = X + N(0, 1) * sigma`                                                                                  |
| LinearBaselineDrift             | `c4a_aug_linear_drift_*`      | `out[i, j] = X[i, j] + offset_i + slope_i * (j - mean_j)` (uniform offset/slope draws)                                                                |
| PolynomialBaselineDrift         | `c4a_aug_poly_drift_*`        | `out[i, j] = X[i, j] + sum_k coeffs[k, i] * lambda_j^k` on `lambda = linspace(-1, 1, n_features)`                                                     |
| PathLengthAugmenter             | `c4a_aug_path_length_*`       | `L[i] = max(1 + path_length_std * N_i, min_path_length)`; `out[i, j] = L[i] * X[i, j]`                                                                |

## ABI additions

21 new symbols (7 ops × 3 symbols), all under the existing `CHEMOMETRICS4ALL_1` linker tag. The `c4a.h` public header is **not** modified in this phase — the public declarations live inside `cpp/src/c_api/c_api_augmenters.cpp` as forward-decls (per the locked task constraints), and consumers redeclare just the symbols they call. This is a deliberate exception to the "everything in c4a.h" rule for the four augmenter phases (15-18); a single consolidated banner will be added to c4a.h at the end of Phase 18 when the full 39-operator surface is finalised.
...
Every operator stores a borrowed `c4a_rng_pcg64_state_t*` pointer in its state. The RNG handle's lifetime is the caller's responsibility. The augmenter does **not** call `c4a_rng_pcg64_destroy`. To get a bit-exact reproducible stream, the caller does:

```c
c4a_rng_pcg64_set_seed(rng, seed);                  /* lock the stream */
c4a_aug_gaussian_noise_apply(handle, X, out);       /* draws N samples */
```

All operators consume the PCG64 stream through:

* `c4a_pcg64_engine_standard_normal_fill(rng, buf, n)` — for `rng.standard_normal()` parity
* `c4a_pcg64_engine_next_double(rng)`                   — for `rng.random()` parity

`rng.uniform(a, b, n)` and `rng.integers(low, high, n)` are intentionally avoided in the C engine (they would require re-implementing NumPy's Lemire rejection for bounded integers); the Python reference module mirrors this by unrolling the uniform / integer draws to `lo + (hi - lo) * u` / `floor(u * range)` mappings. The result is a bit-exact 1e-15 abs parity at the cost of producing slightly different values than `nirs4all`'s `rng.uniform` / `rng.integers` paths would yield — `c4a_parity_augmenters_ref` is the contractual reference, not nirs4all itself.

## Test coverage

A standalone test binary `chemometrics4all_augmenters_tests` (independent of the main `chemometrics4all_tests` -- per the locked task constraint not to modify `tests/main.cpp`) registers 14 tests:
...
The parity-gate runner (`run_parity_gate.py`) was updated to discover **every** test binary under `cpp/tests/` in the build directory (previously hard-coded to `chemometrics4all_tests`), so the augmenter tests are part of Stage 3 of the gate.
...
* `parity/python_generator/src/c4a_parity_augmenters_ref/__init__.py`
* `parity/python_generator/src/c4a_parity_augmenters_ref/{gaussian_noise,multiplicative_noise,spike_noise,hetero_noise,linear_drift,poly_drift,path_length}.py`
...
* `cpp/tests/CMakeLists.txt` — registered `chemometrics4all_augmenters_tests` executable + ctest entry.
...
* `docs/reviews/PHASES.md` — Phase 15 row updated to "14 / 14" with topic refined to "7 ops, `c4a_aug_*`".

### Not modified (per locked task constraints)

* `cpp/include/chemometrics4all/c4a.h`
...
```
cmake --build --preset dev-release --target chemometrics4all_c          → libc4a.so.1.7.0 built
nm -D --defined-only libc4a.so.1.7.0 | awk '$2=="T" {print $3}' | wc -l → 272 (251 + 21)
diff expected_symbols_linux.txt exported.txt                            → empty (ABI surface match)
./build/dev-release/cpp/tests/chemometrics4all_augmenters_tests         → 14 / 14 passing
./build/dev-release/cpp/tests/chemometrics4all_tests                    → 171 / 171 passing (no regressions)
...
ruff check parity/python_generator/src/c4a_parity_augmenters_ref         → clean
```

## CHANGELOG summary

```
phase-15: 7 noise + drift augmenters (c4a_aug_* category)
...
bringing libc4a from 251 -> 272 exported symbols. Parity 1e-15 abs
against the frozen NumPy reference in c4a_parity_augmenters_ref;
14 / 14 tests passing in the new chemometrics4all_augmenters_tests
binary. ABI version unchanged (1.7.0) — augmenter declarations land
in c4a.h at the end of Phase 18 once the full 39-op surface is
locked.
```
