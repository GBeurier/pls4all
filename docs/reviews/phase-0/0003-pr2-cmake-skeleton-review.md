...
- **Subject**: PR 2 — top-level `CMakeLists.txt`, `CMakePresets.json`, `cmake/*.cmake`, `cpp/include/pls4all/n4m_export.h.in`, `cpp/src/CMakeLists.txt`, `cpp/src/{core,c_api}/placeholder.cpp`.
- **Outcome**: 1 critical + 6 important + 4 minor in the first round; all addressed, second round flagged 1 new minor (LTO doc) which was also fixed. Final verdict: **approved for commit** (the only remaining "partial" item — Windows zero-export DLL risk — is acceptable because Windows CI is not enabled until PR 12, by which time PRs 3-7 have added real `n4m_*` symbols).
...
### Important
- Windows zero-export DLL risk: `n4m_c` has no `N4M_API` exports until PR 7. *Accepted as-is*: Windows MSVC CI lands in PR 12, by which time PRs 3-7 have added real `n4m_*` symbols.
...
- `n4m_c_static` excluded from install/export deviates from roadmap §3.1. → fixed by promoting `n4m_core` from `STATIC` to `OBJECT` library — both `n4m_c` and `n4m_c_static` consume it via `$<TARGET_OBJECTS:n4m_core>` so neither install needs the core artefact.
...
- `install(n4m_export.h OPTIONAL)` masks regressions. → fixed by removing `OPTIONAL`.
...
- `nm -D --defined-only build/dev-release/cpp/src/libn4m.so` → empty (correct: PR 2 has zero `n4m_*` symbols).
- `ldd build/dev-release/cpp/src/libn4m.so` → `statically linked` (no dynamic deps, confirms the zero-mandatory-deps promise).
- `cmake --install build/dev-release --prefix /tmp/p4a-install` installs:
  - `lib/libn4m.so{,.0,.0.1.0}`
  - `lib/libp4a_static.a`
  - `include/pls4all/n4m_export.h`
