...
```python
from chemometrics4all.sklearn import PLSRegression
...
with chemometrics4all.Context() as ctx, chemometrics4all.Config() as cfg:
    cfg.algorithm = chemometrics4all.Algorithm.PLS_REGRESSION
    cfg.solver    = chemometrics4all.Solver.SIMPLS
    cfg.n_components = 5
    model = chemometrics4all.Model.fit(ctx, cfg, X, y)
...
```r
# from the repo's bindings/r/chemometrics4all directory:
...
```r
res  <- chemometrics4all_method("pls", X, y, n_components = 5)
...
build_mex                          % produces +chemometrics4all/c4a_*_mex.mex
addpath(pwd);                      % make +chemometrics4all visible
```

### Fit a model — Statistics-toolbox style

```matlab
mdl  = chemometrics4all.fitrpls(X, y, "NumComponents", 5);
...
```matlab
res = chemometrics4all.fit("pls", X, y, "NumComponents", 5);
...
```javascript
import init, { Context, Config, fitPls } from '@chemometrics4all/wasm';

await init();   // loads libc4a.wasm
...
loaded <- chemometrics4all_load("model.n4a")
yhat   <- chemometrics4all_predict(loaded, X_test)
```

```matlab
% MATLAB loads the same file
loaded = chemometrics4all.load("model.n4a");
yhat   = predict(loaded, Xtest);
```

Bytes round-trip exactly. The bundle carries chemometrics4all version + ABI
