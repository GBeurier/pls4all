...
**chemometrics4all** is a portable PLS / NIRS engine written in C++17, exposed
...
multi-month integration project. chemometrics4all collapses that surface to a
...
- **Auditability** — the parity gate compares chemometrics4all predictions to
...
│    chemometrics4all.sklearn.PLSRegression(...)      (Python)         │
│    pls(y ~ ., data, ncomp=)                (R)              │
│    chemometrics4all.fitrpls(X, y, "NumComponents", k)  (MATLAB)      │
│    new chemometrics4all.PLS({nComponents: k})       (JS)             │
└────────────────────────┬─────────────────────────────────────┘
                         │
┌────────────────────────▼─────────────────────────────────────┐
│  Tier-1 raw / canonical API                                  │
│    chemometrics4all._methods.pls_fit(ctx, cfg, X, y, k)  (Python)    │
│    chemometrics4all_method("pls", X, y, n_components=k)  (R)         │
│    chemometrics4all.pls_fit(X, y, k)                     (MATLAB)    │
└────────────────────────┬─────────────────────────────────────┘
                         │
┌────────────────────────▼─────────────────────────────────────┐
│  Tier-0 — C ABI (libc4a)                                     │
│    c4a_*  symbols  (96 of them, frozen at ABI 1.x)           │
└──────────────────────────────────────────────────────────────┘
```
...
- **Acceleration matrix** — five libc4a builds (`ref`, `blas`, `omp`,
  `blas+omp`, `cuda`) so every cell can also serve as a benchmark of
  the acceleration stack itself.

## What chemometrics4all is *not*

- **Not a data-loading framework.** chemometrics4all assumes you arrive with
...
- **Not a deep-learning library.** chemometrics4all is strictly the PLS family
...
| Read chemometrics4all in your language | [Python](../bindings/python.md) · [R](../bindings/r.md) · [MATLAB](../bindings/matlab.md) · [JS](../bindings/js.md) · [Android](../bindings/android.md) |
