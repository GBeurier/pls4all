# R pls-compatible benchmark column.
#
# For `pls`, `pcr`, and `cppls` this measures the actual pls-style
# `plsr()` / `pcr()` facade. For every other registry method, it still
# enters through a formula-built data frame/design matrix and then uses
# the registry dispatcher so the dashboard has a complete R pls_compat
# column for the whole canonical method matrix.
...
if (a$algo == "on_pls") {
    cat(toJSON(list(
        ok = FALSE,
        reason = "not_available: r_pls_compat does not expose on_pls block_reconstruction_0",
        skipped = TRUE,
        versions = list(
...
fit_predict <- function(seed) {
    ...
    if (a$algo %in% c("pls", "pcr", "cppls") && n_targets == 1L) {
        df <- .formula_frame(X, y)
        if (a$algo == "pls") {
            fit <- pls4all::plsr(y ~ ., data = df, ncomp = call_nc,
                                 method = "simpls",
                                 scale = FALSE,
                                 validation = "none",
                                 fit_components = FALSE)
        } else if (a$algo == "pcr") {
            fit <- pls4all::pcr(y ~ ., data = df, ncomp = call_nc,
                                scale = FALSE,
                                validation = "none",
                                fit_components = FALSE)
        } else {
            fit <- pls4all::plsr(y ~ ., data = df, ncomp = call_nc,
                                 method = "cppls",
                                 gamma = call_params$gamma %||% 0.5,
                                 scale = FALSE,
                                 validation = "none",
                                 fit_components = FALSE)
        }
        return(as.numeric(predict(fit, newdata = df, ncomp = call_nc)))
    }
    ...
    if (a$algo %in% c("pls_diagnostic_t2", "pls_diagnostic_q",
        ...
        return(.materialize_result(res, registry_pkey, p, "r_pls_compat"))
    }
    ...
    res <- pls4all::pls4all_method(dispatch_algo, X, Y,
                                   n_components = as.integer(call_nc),
                                   params = call_params)
    .materialize_result(res, registry_pkey, p, "r_pls_compat")
}
